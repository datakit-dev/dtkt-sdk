// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.36.10
// 	protoc        (unknown)
// source: dtkt/core/v1/messages.proto

package corev1

import (
	_ "buf.build/gen/go/bufbuild/protovalidate/protocolbuffers/go/buf/validate"
	v1beta11 "github.com/datakit-dev/dtkt-sdk/sdk-go/proto/dtkt/flow/v1beta1"
	_ "github.com/datakit-dev/dtkt-sdk/sdk-go/proto/dtkt/protoform/v1beta1"
	v1beta1 "github.com/datakit-dev/dtkt-sdk/sdk-go/proto/dtkt/shared/v1beta1"
	_ "google.golang.org/genproto/googleapis/api/annotations"
	status "google.golang.org/genproto/googleapis/rpc/status"
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	descriptorpb "google.golang.org/protobuf/types/descriptorpb"
	anypb "google.golang.org/protobuf/types/known/anypb"
	durationpb "google.golang.org/protobuf/types/known/durationpb"
	fieldmaskpb "google.golang.org/protobuf/types/known/fieldmaskpb"
	timestamppb "google.golang.org/protobuf/types/known/timestamppb"
	reflect "reflect"
	sync "sync"
	unsafe "unsafe"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

// Authentication method used.
type Auth_AuthMethod int32

const (
	Auth_AUTH_METHOD_UNSPECIFIED Auth_AuthMethod = 0
	Auth_AUTH_METHOD_OAUTH       Auth_AuthMethod = 1 // OAuth 2.1 (Auth Code + PKCE)
	Auth_AUTH_METHOD_DEVICE      Auth_AuthMethod = 2 // Device Authorization (RFC 8628)
	Auth_AUTH_METHOD_BEARER      Auth_AuthMethod = 3 // Static bearer token (CI/CD)
)

// Enum value maps for Auth_AuthMethod.
var (
	Auth_AuthMethod_name = map[int32]string{
		0: "AUTH_METHOD_UNSPECIFIED",
		1: "AUTH_METHOD_OAUTH",
		2: "AUTH_METHOD_DEVICE",
		3: "AUTH_METHOD_BEARER",
	}
	Auth_AuthMethod_value = map[string]int32{
		"AUTH_METHOD_UNSPECIFIED": 0,
		"AUTH_METHOD_OAUTH":       1,
		"AUTH_METHOD_DEVICE":      2,
		"AUTH_METHOD_BEARER":      3,
	}
)

func (x Auth_AuthMethod) Enum() *Auth_AuthMethod {
	p := new(Auth_AuthMethod)
	*p = x
	return p
}

func (x Auth_AuthMethod) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (Auth_AuthMethod) Descriptor() protoreflect.EnumDescriptor {
	return file_dtkt_core_v1_messages_proto_enumTypes[0].Descriptor()
}

func (Auth_AuthMethod) Type() protoreflect.EnumType {
	return &file_dtkt_core_v1_messages_proto_enumTypes[0]
}

func (x Auth_AuthMethod) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use Auth_AuthMethod.Descriptor instead.
func (Auth_AuthMethod) EnumDescriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{1, 0}
}

type Deployment_State int32

const (
	Deployment_STATE_UNSPECIFIED Deployment_State = 0
	Deployment_PENDING           Deployment_State = 1
	Deployment_RUNNING           Deployment_State = 2
	Deployment_STOPPING          Deployment_State = 3
	Deployment_STOPPED           Deployment_State = 4
	Deployment_FAILED            Deployment_State = 5
)

// Enum value maps for Deployment_State.
var (
	Deployment_State_name = map[int32]string{
		0: "STATE_UNSPECIFIED",
		1: "PENDING",
		2: "RUNNING",
		3: "STOPPING",
		4: "STOPPED",
		5: "FAILED",
	}
	Deployment_State_value = map[string]int32{
		"STATE_UNSPECIFIED": 0,
		"PENDING":           1,
		"RUNNING":           2,
		"STOPPING":          3,
		"STOPPED":           4,
		"FAILED":            5,
	}
)

func (x Deployment_State) Enum() *Deployment_State {
	p := new(Deployment_State)
	*p = x
	return p
}

func (x Deployment_State) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (Deployment_State) Descriptor() protoreflect.EnumDescriptor {
	return file_dtkt_core_v1_messages_proto_enumTypes[1].Descriptor()
}

func (Deployment_State) Type() protoreflect.EnumType {
	return &file_dtkt_core_v1_messages_proto_enumTypes[1]
}

func (x Deployment_State) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use Deployment_State.Descriptor instead.
func (Deployment_State) EnumDescriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{7, 0}
}

type BuildOperationMetadata_State int32

const (
	BuildOperationMetadata_STATE_UNSPECIFIED BuildOperationMetadata_State = 0
	BuildOperationMetadata_STATE_PENDING     BuildOperationMetadata_State = 1
	BuildOperationMetadata_STATE_RUNNING     BuildOperationMetadata_State = 2
	BuildOperationMetadata_STATE_SUCCEEDED   BuildOperationMetadata_State = 3
	BuildOperationMetadata_STATE_FAILED      BuildOperationMetadata_State = 4
)

// Enum value maps for BuildOperationMetadata_State.
var (
	BuildOperationMetadata_State_name = map[int32]string{
		0: "STATE_UNSPECIFIED",
		1: "STATE_PENDING",
		2: "STATE_RUNNING",
		3: "STATE_SUCCEEDED",
		4: "STATE_FAILED",
	}
	BuildOperationMetadata_State_value = map[string]int32{
		"STATE_UNSPECIFIED": 0,
		"STATE_PENDING":     1,
		"STATE_RUNNING":     2,
		"STATE_SUCCEEDED":   3,
		"STATE_FAILED":      4,
	}
)

func (x BuildOperationMetadata_State) Enum() *BuildOperationMetadata_State {
	p := new(BuildOperationMetadata_State)
	*p = x
	return p
}

func (x BuildOperationMetadata_State) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (BuildOperationMetadata_State) Descriptor() protoreflect.EnumDescriptor {
	return file_dtkt_core_v1_messages_proto_enumTypes[2].Descriptor()
}

func (BuildOperationMetadata_State) Type() protoreflect.EnumType {
	return &file_dtkt_core_v1_messages_proto_enumTypes[2]
}

func (x BuildOperationMetadata_State) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use BuildOperationMetadata_State.Descriptor instead.
func (BuildOperationMetadata_State) EnumDescriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{22, 0}
}

type RunOperationMetadata_State int32

const (
	RunOperationMetadata_STATE_UNSPECIFIED RunOperationMetadata_State = 0
	RunOperationMetadata_STATE_PENDING     RunOperationMetadata_State = 1
	RunOperationMetadata_STATE_RUNNING     RunOperationMetadata_State = 2
	RunOperationMetadata_STATE_STOPPED     RunOperationMetadata_State = 3
	RunOperationMetadata_STATE_FAILED      RunOperationMetadata_State = 4
)

// Enum value maps for RunOperationMetadata_State.
var (
	RunOperationMetadata_State_name = map[int32]string{
		0: "STATE_UNSPECIFIED",
		1: "STATE_PENDING",
		2: "STATE_RUNNING",
		3: "STATE_STOPPED",
		4: "STATE_FAILED",
	}
	RunOperationMetadata_State_value = map[string]int32{
		"STATE_UNSPECIFIED": 0,
		"STATE_PENDING":     1,
		"STATE_RUNNING":     2,
		"STATE_STOPPED":     3,
		"STATE_FAILED":      4,
	}
)

func (x RunOperationMetadata_State) Enum() *RunOperationMetadata_State {
	p := new(RunOperationMetadata_State)
	*p = x
	return p
}

func (x RunOperationMetadata_State) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (RunOperationMetadata_State) Descriptor() protoreflect.EnumDescriptor {
	return file_dtkt_core_v1_messages_proto_enumTypes[3].Descriptor()
}

func (RunOperationMetadata_State) Type() protoreflect.EnumType {
	return &file_dtkt_core_v1_messages_proto_enumTypes[3]
}

func (x RunOperationMetadata_State) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use RunOperationMetadata_State.Descriptor instead.
func (RunOperationMetadata_State) EnumDescriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{23, 0}
}

type SyncOperationMetadata_State int32

const (
	SyncOperationMetadata_STATE_UNSPECIFIED SyncOperationMetadata_State = 0
	SyncOperationMetadata_STATE_PENDING     SyncOperationMetadata_State = 1
	SyncOperationMetadata_STATE_RUNNING     SyncOperationMetadata_State = 2
	SyncOperationMetadata_STATE_SUCCEEDED   SyncOperationMetadata_State = 3
	SyncOperationMetadata_STATE_FAILED      SyncOperationMetadata_State = 4
)

// Enum value maps for SyncOperationMetadata_State.
var (
	SyncOperationMetadata_State_name = map[int32]string{
		0: "STATE_UNSPECIFIED",
		1: "STATE_PENDING",
		2: "STATE_RUNNING",
		3: "STATE_SUCCEEDED",
		4: "STATE_FAILED",
	}
	SyncOperationMetadata_State_value = map[string]int32{
		"STATE_UNSPECIFIED": 0,
		"STATE_PENDING":     1,
		"STATE_RUNNING":     2,
		"STATE_SUCCEEDED":   3,
		"STATE_FAILED":      4,
	}
)

func (x SyncOperationMetadata_State) Enum() *SyncOperationMetadata_State {
	p := new(SyncOperationMetadata_State)
	*p = x
	return p
}

func (x SyncOperationMetadata_State) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (SyncOperationMetadata_State) Descriptor() protoreflect.EnumDescriptor {
	return file_dtkt_core_v1_messages_proto_enumTypes[4].Descriptor()
}

func (SyncOperationMetadata_State) Type() protoreflect.EnumType {
	return &file_dtkt_core_v1_messages_proto_enumTypes[4]
}

func (x SyncOperationMetadata_State) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use SyncOperationMetadata_State.Descriptor instead.
func (SyncOperationMetadata_State) EnumDescriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{24, 0}
}

type DialMetadata_State int32

const (
	DialMetadata_STATE_UNSPECIFIED DialMetadata_State = 0
	DialMetadata_STATE_CONNECTED   DialMetadata_State = 1
	DialMetadata_STATE_FAILED      DialMetadata_State = 2
)

// Enum value maps for DialMetadata_State.
var (
	DialMetadata_State_name = map[int32]string{
		0: "STATE_UNSPECIFIED",
		1: "STATE_CONNECTED",
		2: "STATE_FAILED",
	}
	DialMetadata_State_value = map[string]int32{
		"STATE_UNSPECIFIED": 0,
		"STATE_CONNECTED":   1,
		"STATE_FAILED":      2,
	}
)

func (x DialMetadata_State) Enum() *DialMetadata_State {
	p := new(DialMetadata_State)
	*p = x
	return p
}

func (x DialMetadata_State) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (DialMetadata_State) Descriptor() protoreflect.EnumDescriptor {
	return file_dtkt_core_v1_messages_proto_enumTypes[5].Descriptor()
}

func (DialMetadata_State) Type() protoreflect.EnumType {
	return &file_dtkt_core_v1_messages_proto_enumTypes[5]
}

func (x DialMetadata_State) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use DialMetadata_State.Descriptor instead.
func (DialMetadata_State) EnumDescriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{25, 0}
}

// Address contains a supported network and compatible target.
type Address struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Network protocol (e.g., "tcp", "unix").
	Network string `protobuf:"bytes,1,opt,name=network,proto3" json:"network,omitempty"`
	// Target address for the network.
	Target        string `protobuf:"bytes,2,opt,name=target,proto3" json:"target,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *Address) Reset() {
	*x = Address{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[0]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *Address) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Address) ProtoMessage() {}

func (x *Address) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[0]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Address.ProtoReflect.Descriptor instead.
func (*Address) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{0}
}

func (x *Address) GetNetwork() string {
	if x != nil {
		return x.Network
	}
	return ""
}

func (x *Address) GetTarget() string {
	if x != nil {
		return x.Target
	}
	return ""
}

// Auth contains authentication information for API requests.
type Auth struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Authentication method used.
	Method Auth_AuthMethod `protobuf:"varint,1,opt,name=method,proto3,enum=dtkt.core.v1.Auth_AuthMethod" json:"method,omitempty"`
	// Access token (JWT) or Personal Access Token (PAT).
	AccessToken string `protobuf:"bytes,2,opt,name=access_token,json=accessToken,proto3" json:"access_token,omitempty"`
	// Refresh token (for token renewal).
	RefreshToken string `protobuf:"bytes,3,opt,name=refresh_token,json=refreshToken,proto3" json:"refresh_token,omitempty"`
	// Token expiration timestamp.
	ExpiresAt *timestamppb.Timestamp `protobuf:"bytes,4,opt,name=expires_at,json=expiresAt,proto3" json:"expires_at,omitempty"`
	// Scope requests to an organization (if applicable).
	Org *string `protobuf:"bytes,5,opt,name=org,proto3,oneof" json:"org,omitempty"`
	// Scope requests to a space (if applicable).
	Space         *string `protobuf:"bytes,6,opt,name=space,proto3,oneof" json:"space,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *Auth) Reset() {
	*x = Auth{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[1]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *Auth) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Auth) ProtoMessage() {}

func (x *Auth) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[1]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Auth.ProtoReflect.Descriptor instead.
func (*Auth) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{1}
}

func (x *Auth) GetMethod() Auth_AuthMethod {
	if x != nil {
		return x.Method
	}
	return Auth_AUTH_METHOD_UNSPECIFIED
}

func (x *Auth) GetAccessToken() string {
	if x != nil {
		return x.AccessToken
	}
	return ""
}

func (x *Auth) GetRefreshToken() string {
	if x != nil {
		return x.RefreshToken
	}
	return ""
}

func (x *Auth) GetExpiresAt() *timestamppb.Timestamp {
	if x != nil {
		return x.ExpiresAt
	}
	return nil
}

func (x *Auth) GetOrg() string {
	if x != nil && x.Org != nil {
		return *x.Org
	}
	return ""
}

func (x *Auth) GetSpace() string {
	if x != nil && x.Space != nil {
		return *x.Space
	}
	return ""
}

// Context provides the name and address of a control plane for client
// construction and resource tracing across network boundaries. Context is used
// to resolve resources by name and provide necessary information for
// authentication, encryption, auditing, tracing, and observability when sending
// data across network boundaries.
type Context struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Name of root control plane resource (e.g., "users/bob" or "organizations/acme").
	Name string `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`
	// Address of control plane.
	Address *Address `protobuf:"bytes,2,opt,name=address,proto3" json:"address,omitempty"`
	// Authentication information for control plane.
	Auth          *Auth `protobuf:"bytes,3,opt,name=auth,proto3" json:"auth,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *Context) Reset() {
	*x = Context{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[2]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *Context) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Context) ProtoMessage() {}

func (x *Context) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[2]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Context.ProtoReflect.Descriptor instead.
func (*Context) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{2}
}

func (x *Context) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *Context) GetAddress() *Address {
	if x != nil {
		return x.Address
	}
	return nil
}

func (x *Context) GetAuth() *Auth {
	if x != nil {
		return x.Auth
	}
	return nil
}

// Resource is a named reference to resource from a context with optional
// address and associated data payload. Resource is used for tracing data origin
// across network boundaries and can represent resources such as connections,
// deployments, etc. which may contain encrypted or decrypted data payloads
// related to the resource (such as data plane message payloads, config data,
// secret data, metrics data, etc.).
type Resource struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Context of resource for tracing data origin.
	Context *Context `protobuf:"bytes,1,opt,name=context,proto3" json:"context,omitempty"`
	// Name of resource within context where data originated.
	Name string `protobuf:"bytes,2,opt,name=name,proto3" json:"name,omitempty"`
	// Address of resource (optional; may be used for direct access to resource
	// across network boundaries when available).
	Address *Address `protobuf:"bytes,3,opt,name=address,proto3" json:"address,omitempty"`
	// Data payload, either encrypted or decrypted.
	// Notes:
	//   - data is not the resource itself, but a payload related to or originating
	//     from the resource. (e.g. config data, secret data, metrics data, etc.)
	//   - information about the type of data is provided by a field of this type.
	//     (e.g. Deployment cloud field indicates data originated from a cloud deployment)
	//
	// Types that are valid to be assigned to Data:
	//
	//	*Resource_Encrypted
	//	*Resource_Decrypted
	Data          isResource_Data `protobuf_oneof:"data"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *Resource) Reset() {
	*x = Resource{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[3]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *Resource) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Resource) ProtoMessage() {}

func (x *Resource) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[3]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Resource.ProtoReflect.Descriptor instead.
func (*Resource) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{3}
}

func (x *Resource) GetContext() *Context {
	if x != nil {
		return x.Context
	}
	return nil
}

func (x *Resource) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *Resource) GetAddress() *Address {
	if x != nil {
		return x.Address
	}
	return nil
}

func (x *Resource) GetData() isResource_Data {
	if x != nil {
		return x.Data
	}
	return nil
}

func (x *Resource) GetEncrypted() *EncryptedAny {
	if x != nil {
		if x, ok := x.Data.(*Resource_Encrypted); ok {
			return x.Encrypted
		}
	}
	return nil
}

func (x *Resource) GetDecrypted() *anypb.Any {
	if x != nil {
		if x, ok := x.Data.(*Resource_Decrypted); ok {
			return x.Decrypted
		}
	}
	return nil
}

type isResource_Data interface {
	isResource_Data()
}

type Resource_Encrypted struct {
	Encrypted *EncryptedAny `protobuf:"bytes,10,opt,name=encrypted,proto3,oneof"`
}

type Resource_Decrypted struct {
	Decrypted *anypb.Any `protobuf:"bytes,11,opt,name=decrypted,proto3,oneof"`
}

func (*Resource_Encrypted) isResource_Data() {}

func (*Resource_Decrypted) isResource_Data() {}

// Automation is a runnable instance of a Flow with a tagged spec version and
// runtime configuration.
type Automation struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	Name  string                 `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`
	// Server-assigned unique ID (UUID).
	Uid string `protobuf:"bytes,2,opt,name=uid,proto3" json:"uid,omitempty"`
	// Name of flow resource to resolve spec.
	Flow string `protobuf:"bytes,3,opt,name=flow,proto3" json:"flow,omitempty"`
	// Checksum of flow spec must match when resolving spec (concurrency control).
	SpecEtag string `protobuf:"bytes,4,opt,name=spec_etag,json=specEtag,proto3" json:"spec_etag,omitempty"`
	// Executor runtime metadata of resolved Flow spec (versioned).
	Runtime *FlowRuntimeMetadata `protobuf:"bytes,5,opt,name=runtime,proto3" json:"runtime,omitempty"`
	// Execution groups metadata of resolved Flow spec (versioned).
	Groups *FlowGroupsMetadata `protobuf:"bytes,6,opt,name=groups,proto3" json:"groups,omitempty"`
	// Connections configuration as map of connection node id to connection
	// resource name and context (control plane).
	// Examples:
	// ```
	//   - "storage": {
	//     "name": "users/bob/connections/localblob",
	//     "context": {
	//     "name": "users/bob",
	//     ...
	//     },
	//     }
	//
	// ```
	Connections map[string]*FlowConnectionMetadata `protobuf:"bytes,7,rep,name=connections,proto3" json:"connections,omitempty" protobuf_key:"bytes,1,opt,name=key" protobuf_val:"bytes,2,opt,name=value"`
	// Initial inputs configuration as map of input node id to valid data of input
	// node's declared type (well-known type wrappers used for scalar values).
	// Format:
	// ```
	// - "bucketName": "my-storage-bucket"
	// - "adSettings": { "bid": 100 }
	// ```
	Inputs map[string]*anypb.Any `protobuf:"bytes,8,rep,name=inputs,proto3" json:"inputs,omitempty" protobuf_key:"bytes,1,opt,name=key" protobuf_val:"bytes,2,opt,name=value"`
	// Execution is stopped if total time exceeds given duration.
	// Default: 0 (no timeout).
	Timeout *durationpb.Duration `protobuf:"bytes,9,opt,name=timeout,proto3" json:"timeout,omitempty"`
	// Server-assigned batch name if created within a batch (optional).
	Batch *string `protobuf:"bytes,10,opt,name=batch,proto3,oneof" json:"batch,omitempty"`
	// Automation create time.
	CreateTime *timestamppb.Timestamp `protobuf:"bytes,20,opt,name=create_time,json=createTime,proto3" json:"create_time,omitempty"`
	// Automation update time.
	UpdateTime    *timestamppb.Timestamp `protobuf:"bytes,21,opt,name=update_time,json=updateTime,proto3" json:"update_time,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *Automation) Reset() {
	*x = Automation{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[4]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *Automation) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Automation) ProtoMessage() {}

func (x *Automation) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[4]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Automation.ProtoReflect.Descriptor instead.
func (*Automation) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{4}
}

func (x *Automation) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *Automation) GetUid() string {
	if x != nil {
		return x.Uid
	}
	return ""
}

func (x *Automation) GetFlow() string {
	if x != nil {
		return x.Flow
	}
	return ""
}

func (x *Automation) GetSpecEtag() string {
	if x != nil {
		return x.SpecEtag
	}
	return ""
}

func (x *Automation) GetRuntime() *FlowRuntimeMetadata {
	if x != nil {
		return x.Runtime
	}
	return nil
}

func (x *Automation) GetGroups() *FlowGroupsMetadata {
	if x != nil {
		return x.Groups
	}
	return nil
}

func (x *Automation) GetConnections() map[string]*FlowConnectionMetadata {
	if x != nil {
		return x.Connections
	}
	return nil
}

func (x *Automation) GetInputs() map[string]*anypb.Any {
	if x != nil {
		return x.Inputs
	}
	return nil
}

func (x *Automation) GetTimeout() *durationpb.Duration {
	if x != nil {
		return x.Timeout
	}
	return nil
}

func (x *Automation) GetBatch() string {
	if x != nil && x.Batch != nil {
		return *x.Batch
	}
	return ""
}

func (x *Automation) GetCreateTime() *timestamppb.Timestamp {
	if x != nil {
		return x.CreateTime
	}
	return nil
}

func (x *Automation) GetUpdateTime() *timestamppb.Timestamp {
	if x != nil {
		return x.UpdateTime
	}
	return nil
}

// Flow is a versioned workflow spec with parsed graph representation.
type Flow struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	Name  string                 `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`
	// Server-assigned unique ID (UUID).
	Uid string `protobuf:"bytes,2,opt,name=uid,proto3" json:"uid,omitempty"`
	// Flow spec URI (optional).
	SpecUri string `protobuf:"bytes,3,opt,name=spec_uri,json=specUri,proto3" json:"spec_uri,omitempty"`
	// Flow spec checksum for concurrency control.
	SpecEtag string `protobuf:"bytes,4,opt,name=spec_etag,json=specEtag,proto3" json:"spec_etag,omitempty"`
	// Flow spec metadata (versioned).
	Spec *FlowSpecMetadata `protobuf:"bytes,5,opt,name=spec,proto3" json:"spec,omitempty"`
	// Flow graph metadata (versioned).
	Graph *FlowGraphMetadata `protobuf:"bytes,6,opt,name=graph,proto3" json:"graph,omitempty"`
	// Flow create time.
	CreateTime *timestamppb.Timestamp `protobuf:"bytes,20,opt,name=create_time,json=createTime,proto3" json:"create_time,omitempty"`
	// Flow update time.
	UpdateTime    *timestamppb.Timestamp `protobuf:"bytes,21,opt,name=update_time,json=updateTime,proto3" json:"update_time,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *Flow) Reset() {
	*x = Flow{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[5]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *Flow) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Flow) ProtoMessage() {}

func (x *Flow) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[5]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Flow.ProtoReflect.Descriptor instead.
func (*Flow) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{5}
}

func (x *Flow) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *Flow) GetUid() string {
	if x != nil {
		return x.Uid
	}
	return ""
}

func (x *Flow) GetSpecUri() string {
	if x != nil {
		return x.SpecUri
	}
	return ""
}

func (x *Flow) GetSpecEtag() string {
	if x != nil {
		return x.SpecEtag
	}
	return ""
}

func (x *Flow) GetSpec() *FlowSpecMetadata {
	if x != nil {
		return x.Spec
	}
	return nil
}

func (x *Flow) GetGraph() *FlowGraphMetadata {
	if x != nil {
		return x.Graph
	}
	return nil
}

func (x *Flow) GetCreateTime() *timestamppb.Timestamp {
	if x != nil {
		return x.CreateTime
	}
	return nil
}

func (x *Flow) GetUpdateTime() *timestamppb.Timestamp {
	if x != nil {
		return x.UpdateTime
	}
	return nil
}

// Connection combines necessary components of a configured client to one of:
// - Deployment server with first-class configuration support using a DataKit SDK
// - Any reachable gRPC server with DIY configuration, one of:
//   - Custom proto file descriptors as a URI resolving to source .proto files
//   - Custom proto file descriptors using gRPC reflection (must be enabled on server)
type Connection struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	Name  string                 `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`
	// Server-assigned unique ID (UUID).
	Uid string `protobuf:"bytes,2,opt,name=uid,proto3" json:"uid,omitempty"`
	// Types that are valid to be assigned to Type:
	//
	//	*Connection_Deployment
	//	*Connection_CustomProtos
	//	*Connection_CustomGrpc
	Type isConnection_Type `protobuf_oneof:"type"`
	// Address of gRPC server.
	Address *Address `protobuf:"bytes,6,opt,name=address,proto3" json:"address,omitempty"`
	// Types that are valid to be assigned to Config:
	//
	//	*Connection_DecryptedConfig
	//	*Connection_EncryptedConfig
	Config isConnection_Config `protobuf_oneof:"config"`
	// Hash of config data.
	ConfigHash string `protobuf:"bytes,11,opt,name=config_hash,json=configHash,proto3" json:"config_hash,omitempty"`
	// Generation of config data.
	ConfigGen uint64 `protobuf:"varint,12,opt,name=config_gen,json=configGen,proto3" json:"config_gen,omitempty"`
	// Config source uri (optional).
	ConfigUri string `protobuf:"bytes,13,opt,name=config_uri,json=configUri,proto3" json:"config_uri,omitempty"`
	// Headers applied when dialing connection.
	Headers map[string]string `protobuf:"bytes,14,rep,name=headers,proto3" json:"headers,omitempty" protobuf_key:"bytes,1,opt,name=key" protobuf_val:"bytes,2,opt,name=value"`
	// Connection create time.
	CreateTime *timestamppb.Timestamp `protobuf:"bytes,20,opt,name=create_time,json=createTime,proto3" json:"create_time,omitempty"`
	// Connection update time.
	UpdateTime    *timestamppb.Timestamp `protobuf:"bytes,21,opt,name=update_time,json=updateTime,proto3" json:"update_time,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *Connection) Reset() {
	*x = Connection{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[6]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *Connection) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Connection) ProtoMessage() {}

func (x *Connection) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[6]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Connection.ProtoReflect.Descriptor instead.
func (*Connection) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{6}
}

func (x *Connection) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *Connection) GetUid() string {
	if x != nil {
		return x.Uid
	}
	return ""
}

func (x *Connection) GetType() isConnection_Type {
	if x != nil {
		return x.Type
	}
	return nil
}

func (x *Connection) GetDeployment() string {
	if x != nil {
		if x, ok := x.Type.(*Connection_Deployment); ok {
			return x.Deployment
		}
	}
	return ""
}

func (x *Connection) GetCustomProtos() string {
	if x != nil {
		if x, ok := x.Type.(*Connection_CustomProtos); ok {
			return x.CustomProtos
		}
	}
	return ""
}

func (x *Connection) GetCustomGrpc() bool {
	if x != nil {
		if x, ok := x.Type.(*Connection_CustomGrpc); ok {
			return x.CustomGrpc
		}
	}
	return false
}

func (x *Connection) GetAddress() *Address {
	if x != nil {
		return x.Address
	}
	return nil
}

func (x *Connection) GetConfig() isConnection_Config {
	if x != nil {
		return x.Config
	}
	return nil
}

func (x *Connection) GetDecryptedConfig() *anypb.Any {
	if x != nil {
		if x, ok := x.Config.(*Connection_DecryptedConfig); ok {
			return x.DecryptedConfig
		}
	}
	return nil
}

func (x *Connection) GetEncryptedConfig() *EncryptedAny {
	if x != nil {
		if x, ok := x.Config.(*Connection_EncryptedConfig); ok {
			return x.EncryptedConfig
		}
	}
	return nil
}

func (x *Connection) GetConfigHash() string {
	if x != nil {
		return x.ConfigHash
	}
	return ""
}

func (x *Connection) GetConfigGen() uint64 {
	if x != nil {
		return x.ConfigGen
	}
	return 0
}

func (x *Connection) GetConfigUri() string {
	if x != nil {
		return x.ConfigUri
	}
	return ""
}

func (x *Connection) GetHeaders() map[string]string {
	if x != nil {
		return x.Headers
	}
	return nil
}

func (x *Connection) GetCreateTime() *timestamppb.Timestamp {
	if x != nil {
		return x.CreateTime
	}
	return nil
}

func (x *Connection) GetUpdateTime() *timestamppb.Timestamp {
	if x != nil {
		return x.UpdateTime
	}
	return nil
}

type isConnection_Type interface {
	isConnection_Type()
}

type Connection_Deployment struct {
	// Name of a deployment resource.
	Deployment string `protobuf:"bytes,3,opt,name=deployment,proto3,oneof"`
}

type Connection_CustomProtos struct {
	// Custom proto descriptors URI.
	CustomProtos string `protobuf:"bytes,4,opt,name=custom_protos,json=customProtos,proto3,oneof"`
}

type Connection_CustomGrpc struct {
	// Custom using gRPC reflection.
	CustomGrpc bool `protobuf:"varint,5,opt,name=custom_grpc,json=customGrpc,proto3,oneof"`
}

func (*Connection_Deployment) isConnection_Type() {}

func (*Connection_CustomProtos) isConnection_Type() {}

func (*Connection_CustomGrpc) isConnection_Type() {}

type isConnection_Config interface {
	isConnection_Config()
}

type Connection_DecryptedConfig struct {
	// Config data compatible with integration config schema.
	DecryptedConfig *anypb.Any `protobuf:"bytes,7,opt,name=decrypted_config,json=decryptedConfig,proto3,oneof"`
}

type Connection_EncryptedConfig struct {
	// Encrypted config data.
	EncryptedConfig *EncryptedAny `protobuf:"bytes,8,opt,name=encrypted_config,json=encryptedConfig,proto3,oneof"`
}

func (*Connection_DecryptedConfig) isConnection_Config() {}

func (*Connection_EncryptedConfig) isConnection_Config() {}

// Deployment is a running instance of an Integration with context aware build
// and runtime configuration. Deployments provide a way to run and manage software
// packages represented by Integrations in compatible runtime environments, enabling
// users to easily deploy and run their software packages in a consistent and
// traceable manner within the DataKit ecosystem.
type Deployment struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Name of deployment resource.
	Name string `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`
	// Server-assigned unique ID (UUID).
	Uid   string           `protobuf:"bytes,2,opt,name=uid,proto3" json:"uid,omitempty"`
	State Deployment_State `protobuf:"varint,3,opt,name=state,proto3,enum=dtkt.core.v1.Deployment_State" json:"state,omitempty"`
	// Bind address of deployment gRPC server.
	Address *Address `protobuf:"bytes,4,opt,name=address,proto3" json:"address,omitempty"`
	// Env vars applied at runtime.
	Env map[string]string `protobuf:"bytes,5,rep,name=env,proto3" json:"env,omitempty" protobuf_key:"bytes,1,opt,name=key" protobuf_val:"bytes,2,opt,name=value"`
	// Port mappings applied at runtime.
	Ports map[string]string `protobuf:"bytes,6,rep,name=ports,proto3" json:"ports,omitempty" protobuf_key:"bytes,1,opt,name=key" protobuf_val:"bytes,2,opt,name=value"`
	// Build context for deployment, including reference to integration and build
	// metadata (must include artifact etag for a successful build). This enables
	// cross-control plane deployments where the integration resource is defined
	// in a different control plane than where it is deployed (e.g. integration
	// defined in shared cloud control plane and deployed to a user control plane,
	// or vice versa).
	Build *Deployment_Build `protobuf:"bytes,7,opt,name=build,proto3" json:"build,omitempty"`
	// Cloud context with encrypted config for binding deployment to DataKit Cloud
	// and only present when address's network is "cloud".
	Cloud *Deployment_Cloud `protobuf:"bytes,8,opt,name=cloud,proto3" json:"cloud,omitempty"`
	// Package config metadata fetched at runtime from server.
	ConfigSchema *TypeSchema `protobuf:"bytes,9,opt,name=config_schema,json=configSchema,proto3" json:"config_schema,omitempty"`
	// Package spec metadata fetched at runtime from server.
	PackageSpec *PackageSpecMetadata `protobuf:"bytes,10,opt,name=package_spec,json=packageSpec,proto3" json:"package_spec,omitempty"`
	// Runtime metadata of deployment, including runtime-specific information such as process ID or container ID.
	Runtime *Deployment_RuntimeMetadata `protobuf:"bytes,11,opt,name=runtime,proto3" json:"runtime,omitempty"`
	// Etag for concurrency control of deployment updates (must match current etag
	// to update or delete).
	Etag string `protobuf:"bytes,18,opt,name=etag,proto3" json:"etag,omitempty"`
	// Deployment create time.
	CreateTime *timestamppb.Timestamp `protobuf:"bytes,20,opt,name=create_time,json=createTime,proto3" json:"create_time,omitempty"`
	// Deployment update time.
	UpdateTime    *timestamppb.Timestamp `protobuf:"bytes,21,opt,name=update_time,json=updateTime,proto3" json:"update_time,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *Deployment) Reset() {
	*x = Deployment{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[7]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *Deployment) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Deployment) ProtoMessage() {}

func (x *Deployment) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[7]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Deployment.ProtoReflect.Descriptor instead.
func (*Deployment) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{7}
}

func (x *Deployment) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *Deployment) GetUid() string {
	if x != nil {
		return x.Uid
	}
	return ""
}

func (x *Deployment) GetState() Deployment_State {
	if x != nil {
		return x.State
	}
	return Deployment_STATE_UNSPECIFIED
}

func (x *Deployment) GetAddress() *Address {
	if x != nil {
		return x.Address
	}
	return nil
}

func (x *Deployment) GetEnv() map[string]string {
	if x != nil {
		return x.Env
	}
	return nil
}

func (x *Deployment) GetPorts() map[string]string {
	if x != nil {
		return x.Ports
	}
	return nil
}

func (x *Deployment) GetBuild() *Deployment_Build {
	if x != nil {
		return x.Build
	}
	return nil
}

func (x *Deployment) GetCloud() *Deployment_Cloud {
	if x != nil {
		return x.Cloud
	}
	return nil
}

func (x *Deployment) GetConfigSchema() *TypeSchema {
	if x != nil {
		return x.ConfigSchema
	}
	return nil
}

func (x *Deployment) GetPackageSpec() *PackageSpecMetadata {
	if x != nil {
		return x.PackageSpec
	}
	return nil
}

func (x *Deployment) GetRuntime() *Deployment_RuntimeMetadata {
	if x != nil {
		return x.Runtime
	}
	return nil
}

func (x *Deployment) GetEtag() string {
	if x != nil {
		return x.Etag
	}
	return ""
}

func (x *Deployment) GetCreateTime() *timestamppb.Timestamp {
	if x != nil {
		return x.CreateTime
	}
	return nil
}

func (x *Deployment) GetUpdateTime() *timestamppb.Timestamp {
	if x != nil {
		return x.UpdateTime
	}
	return nil
}

// Integration describes a software package which uses the DataKit SDK and can
// be deployed to a runtime environment using a Deployment resource. An Integration
// can represent a software package in development with a local runtime environment
// or a released software package in a registry with a remote runtime environment.
// Integrations provide a way to manage software packages and their deployment
// lifecycle within the DataKit ecosystem, enabling users to easily deploy and
// run their software packages in compatible runtime environments.
type Integration struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	Name  string                 `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`
	// Server-assigned unique ID (UUID).
	Uid string `protobuf:"bytes,2,opt,name=uid,proto3" json:"uid,omitempty"`
	// Package spec metadata.
	Spec *PackageSpecMetadata `protobuf:"bytes,3,opt,name=spec,proto3" json:"spec,omitempty"`
	// Package spec checksum for concurrency control.
	SpecEtag string `protobuf:"bytes,4,opt,name=spec_etag,json=specEtag,proto3" json:"spec_etag,omitempty"`
	// Package source URI (only present after successful build & update).
	SourceUri *string `protobuf:"bytes,5,opt,name=source_uri,json=sourceUri,proto3,oneof" json:"source_uri,omitempty"`
	// Package source checksum or git commit hash for concurrency control
	// (only present after successful build & update).
	SourceEtag *string `protobuf:"bytes,6,opt,name=source_etag,json=sourceEtag,proto3,oneof" json:"source_etag,omitempty"`
	// Integration create time.
	CreateTime *timestamppb.Timestamp `protobuf:"bytes,20,opt,name=create_time,json=createTime,proto3" json:"create_time,omitempty"`
	// Integration update time.
	UpdateTime    *timestamppb.Timestamp `protobuf:"bytes,21,opt,name=update_time,json=updateTime,proto3" json:"update_time,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *Integration) Reset() {
	*x = Integration{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[8]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *Integration) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Integration) ProtoMessage() {}

func (x *Integration) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[8]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Integration.ProtoReflect.Descriptor instead.
func (*Integration) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{8}
}

func (x *Integration) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *Integration) GetUid() string {
	if x != nil {
		return x.Uid
	}
	return ""
}

func (x *Integration) GetSpec() *PackageSpecMetadata {
	if x != nil {
		return x.Spec
	}
	return nil
}

func (x *Integration) GetSpecEtag() string {
	if x != nil {
		return x.SpecEtag
	}
	return ""
}

func (x *Integration) GetSourceUri() string {
	if x != nil && x.SourceUri != nil {
		return *x.SourceUri
	}
	return ""
}

func (x *Integration) GetSourceEtag() string {
	if x != nil && x.SourceEtag != nil {
		return *x.SourceEtag
	}
	return ""
}

func (x *Integration) GetCreateTime() *timestamppb.Timestamp {
	if x != nil {
		return x.CreateTime
	}
	return nil
}

func (x *Integration) GetUpdateTime() *timestamppb.Timestamp {
	if x != nil {
		return x.UpdateTime
	}
	return nil
}

// File represents runtime artifacts related to a Connection or Deployment, such
// as proto file descriptors used for dynamic client construction and reflection.
type File struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// File resource name.
	Name string `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`
	// Server-assigned unique ID (UUID).
	Uid string `protobuf:"bytes,2,opt,name=uid,proto3" json:"uid,omitempty"`
	// Checksum of file contents for concurrency control.
	Etag string `protobuf:"bytes,3,opt,name=etag,proto3" json:"etag,omitempty"`
	// URI of file contents (e.g. source .proto files for proto descriptors).
	Uri *string `protobuf:"bytes,4,opt,name=uri,proto3,oneof" json:"uri,omitempty"`
	// Types that are valid to be assigned to Type:
	//
	//	*File_Protos
	Type isFile_Type `protobuf_oneof:"type"`
	// File create time.
	CreateTime *timestamppb.Timestamp `protobuf:"bytes,20,opt,name=create_time,json=createTime,proto3" json:"create_time,omitempty"`
	// File update time.
	UpdateTime    *timestamppb.Timestamp `protobuf:"bytes,21,opt,name=update_time,json=updateTime,proto3" json:"update_time,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *File) Reset() {
	*x = File{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[9]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *File) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*File) ProtoMessage() {}

func (x *File) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[9]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use File.ProtoReflect.Descriptor instead.
func (*File) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{9}
}

func (x *File) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *File) GetUid() string {
	if x != nil {
		return x.Uid
	}
	return ""
}

func (x *File) GetEtag() string {
	if x != nil {
		return x.Etag
	}
	return ""
}

func (x *File) GetUri() string {
	if x != nil && x.Uri != nil {
		return *x.Uri
	}
	return ""
}

func (x *File) GetType() isFile_Type {
	if x != nil {
		return x.Type
	}
	return nil
}

func (x *File) GetProtos() *descriptorpb.FileDescriptorSet {
	if x != nil {
		if x, ok := x.Type.(*File_Protos); ok {
			return x.Protos
		}
	}
	return nil
}

func (x *File) GetCreateTime() *timestamppb.Timestamp {
	if x != nil {
		return x.CreateTime
	}
	return nil
}

func (x *File) GetUpdateTime() *timestamppb.Timestamp {
	if x != nil {
		return x.UpdateTime
	}
	return nil
}

type isFile_Type interface {
	isFile_Type()
}

type File_Protos struct {
	// Proto file descriptor set.
	Protos *descriptorpb.FileDescriptorSet `protobuf:"bytes,10,opt,name=protos,proto3,oneof"`
}

func (*File_Protos) isFile_Type() {}

// Service message for documentation and reflection indexing.
type Service struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Service resource name.
	Name string `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`
	// Server-assigned unique ID (UUID).
	Uid string `protobuf:"bytes,10,opt,name=uid,proto3" json:"uid,omitempty"`
	// Service description.
	Description string `protobuf:"bytes,2,opt,name=description,proto3" json:"description,omitempty"`
	// Service create time.
	CreateTime *timestamppb.Timestamp `protobuf:"bytes,20,opt,name=create_time,json=createTime,proto3" json:"create_time,omitempty"`
	// Service update time.
	UpdateTime    *timestamppb.Timestamp `protobuf:"bytes,21,opt,name=update_time,json=updateTime,proto3" json:"update_time,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *Service) Reset() {
	*x = Service{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[10]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *Service) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Service) ProtoMessage() {}

func (x *Service) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[10]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Service.ProtoReflect.Descriptor instead.
func (*Service) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{10}
}

func (x *Service) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *Service) GetUid() string {
	if x != nil {
		return x.Uid
	}
	return ""
}

func (x *Service) GetDescription() string {
	if x != nil {
		return x.Description
	}
	return ""
}

func (x *Service) GetCreateTime() *timestamppb.Timestamp {
	if x != nil {
		return x.CreateTime
	}
	return nil
}

func (x *Service) GetUpdateTime() *timestamppb.Timestamp {
	if x != nil {
		return x.UpdateTime
	}
	return nil
}

// Method message for documentation and reflection indexing.
type Method struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Method resource name.
	Name string `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`
	// Server-assigned unique ID (UUID).
	Uid string `protobuf:"bytes,10,opt,name=uid,proto3" json:"uid,omitempty"`
	// Method description.
	Description string `protobuf:"bytes,2,opt,name=description,proto3" json:"description,omitempty"`
	// Request message type.
	Request string `protobuf:"bytes,3,opt,name=request,proto3" json:"request,omitempty"`
	// Response message type.
	Response string `protobuf:"bytes,4,opt,name=response,proto3" json:"response,omitempty"`
	// Whether request is streaming.
	RequestStreaming bool `protobuf:"varint,5,opt,name=request_streaming,json=requestStreaming,proto3" json:"request_streaming,omitempty"`
	// Whether response is streaming.
	ResponseStreaming bool `protobuf:"varint,6,opt,name=response_streaming,json=responseStreaming,proto3" json:"response_streaming,omitempty"`
	// Method create time.
	CreateTime *timestamppb.Timestamp `protobuf:"bytes,20,opt,name=create_time,json=createTime,proto3" json:"create_time,omitempty"`
	// Method update time.
	UpdateTime    *timestamppb.Timestamp `protobuf:"bytes,21,opt,name=update_time,json=updateTime,proto3" json:"update_time,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *Method) Reset() {
	*x = Method{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[11]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *Method) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Method) ProtoMessage() {}

func (x *Method) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[11]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Method.ProtoReflect.Descriptor instead.
func (*Method) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{11}
}

func (x *Method) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *Method) GetUid() string {
	if x != nil {
		return x.Uid
	}
	return ""
}

func (x *Method) GetDescription() string {
	if x != nil {
		return x.Description
	}
	return ""
}

func (x *Method) GetRequest() string {
	if x != nil {
		return x.Request
	}
	return ""
}

func (x *Method) GetResponse() string {
	if x != nil {
		return x.Response
	}
	return ""
}

func (x *Method) GetRequestStreaming() bool {
	if x != nil {
		return x.RequestStreaming
	}
	return false
}

func (x *Method) GetResponseStreaming() bool {
	if x != nil {
		return x.ResponseStreaming
	}
	return false
}

func (x *Method) GetCreateTime() *timestamppb.Timestamp {
	if x != nil {
		return x.CreateTime
	}
	return nil
}

func (x *Method) GetUpdateTime() *timestamppb.Timestamp {
	if x != nil {
		return x.UpdateTime
	}
	return nil
}

// Type message for documentation and reflection indexing.
type Type struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Type resource name.
	Name string `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`
	// Server-assigned unique ID (UUID).
	Uid string `protobuf:"bytes,5,opt,name=uid,proto3" json:"uid,omitempty"`
	// Type schema.
	Schema *TypeSchema `protobuf:"bytes,2,opt,name=schema,proto3" json:"schema,omitempty"`
	// Type create time.
	CreateTime *timestamppb.Timestamp `protobuf:"bytes,20,opt,name=create_time,json=createTime,proto3" json:"create_time,omitempty"`
	// Type update time.
	UpdateTime    *timestamppb.Timestamp `protobuf:"bytes,21,opt,name=update_time,json=updateTime,proto3" json:"update_time,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *Type) Reset() {
	*x = Type{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[12]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *Type) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Type) ProtoMessage() {}

func (x *Type) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[12]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Type.ProtoReflect.Descriptor instead.
func (*Type) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{12}
}

func (x *Type) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *Type) GetUid() string {
	if x != nil {
		return x.Uid
	}
	return ""
}

func (x *Type) GetSchema() *TypeSchema {
	if x != nil {
		return x.Schema
	}
	return nil
}

func (x *Type) GetCreateTime() *timestamppb.Timestamp {
	if x != nil {
		return x.CreateTime
	}
	return nil
}

func (x *Type) GetUpdateTime() *timestamppb.Timestamp {
	if x != nil {
		return x.UpdateTime
	}
	return nil
}

// TypeSchema wraps versioned type schema definitions.
type TypeSchema struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Types that are valid to be assigned to Version:
	//
	//	*TypeSchema_V1Beta1
	Version       isTypeSchema_Version `protobuf_oneof:"version"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *TypeSchema) Reset() {
	*x = TypeSchema{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[13]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *TypeSchema) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*TypeSchema) ProtoMessage() {}

func (x *TypeSchema) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[13]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use TypeSchema.ProtoReflect.Descriptor instead.
func (*TypeSchema) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{13}
}

func (x *TypeSchema) GetVersion() isTypeSchema_Version {
	if x != nil {
		return x.Version
	}
	return nil
}

func (x *TypeSchema) GetV1Beta1() *v1beta1.TypeSchema {
	if x != nil {
		if x, ok := x.Version.(*TypeSchema_V1Beta1); ok {
			return x.V1Beta1
		}
	}
	return nil
}

type isTypeSchema_Version interface {
	isTypeSchema_Version()
}

type TypeSchema_V1Beta1 struct {
	V1Beta1 *v1beta1.TypeSchema `protobuf:"bytes,1,opt,name=v1beta1,proto3,oneof"`
}

func (*TypeSchema_V1Beta1) isTypeSchema_Version() {}

// DeploymentMetadata contains metadata for deployment operations.
type DeploymentMetadata struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Status message.
	Message string `protobuf:"bytes,10,opt,name=message,proto3" json:"message,omitempty"`
	// Start time.
	StartTime *timestamppb.Timestamp `protobuf:"bytes,20,opt,name=start_time,json=startTime,proto3" json:"start_time,omitempty"`
	// Stop time.
	StopTime      *timestamppb.Timestamp `protobuf:"bytes,21,opt,name=stop_time,json=stopTime,proto3" json:"stop_time,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *DeploymentMetadata) Reset() {
	*x = DeploymentMetadata{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[14]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *DeploymentMetadata) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*DeploymentMetadata) ProtoMessage() {}

func (x *DeploymentMetadata) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[14]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use DeploymentMetadata.ProtoReflect.Descriptor instead.
func (*DeploymentMetadata) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{14}
}

func (x *DeploymentMetadata) GetMessage() string {
	if x != nil {
		return x.Message
	}
	return ""
}

func (x *DeploymentMetadata) GetStartTime() *timestamppb.Timestamp {
	if x != nil {
		return x.StartTime
	}
	return nil
}

func (x *DeploymentMetadata) GetStopTime() *timestamppb.Timestamp {
	if x != nil {
		return x.StopTime
	}
	return nil
}

// FlowConnectionMetadata contains connection configuration for a flow automation.
type FlowConnectionMetadata struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Name of connection resource.
	Name string `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`
	// Optional context to resolve connection resource by name.
	// If unspecified, defaults to the context where the flow automation runs.
	Context       *Context `protobuf:"bytes,2,opt,name=context,proto3" json:"context,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *FlowConnectionMetadata) Reset() {
	*x = FlowConnectionMetadata{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[15]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *FlowConnectionMetadata) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*FlowConnectionMetadata) ProtoMessage() {}

func (x *FlowConnectionMetadata) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[15]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use FlowConnectionMetadata.ProtoReflect.Descriptor instead.
func (*FlowConnectionMetadata) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{15}
}

func (x *FlowConnectionMetadata) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *FlowConnectionMetadata) GetContext() *Context {
	if x != nil {
		return x.Context
	}
	return nil
}

// FlowRuntimeMetadata wraps versioned executor runtime metadata.
type FlowRuntimeMetadata struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Types that are valid to be assigned to Version:
	//
	//	*FlowRuntimeMetadata_V1Beta1
	Version       isFlowRuntimeMetadata_Version `protobuf_oneof:"version"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *FlowRuntimeMetadata) Reset() {
	*x = FlowRuntimeMetadata{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[16]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *FlowRuntimeMetadata) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*FlowRuntimeMetadata) ProtoMessage() {}

func (x *FlowRuntimeMetadata) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[16]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use FlowRuntimeMetadata.ProtoReflect.Descriptor instead.
func (*FlowRuntimeMetadata) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{16}
}

func (x *FlowRuntimeMetadata) GetVersion() isFlowRuntimeMetadata_Version {
	if x != nil {
		return x.Version
	}
	return nil
}

func (x *FlowRuntimeMetadata) GetV1Beta1() *v1beta11.Runtime {
	if x != nil {
		if x, ok := x.Version.(*FlowRuntimeMetadata_V1Beta1); ok {
			return x.V1Beta1
		}
	}
	return nil
}

type isFlowRuntimeMetadata_Version interface {
	isFlowRuntimeMetadata_Version()
}

type FlowRuntimeMetadata_V1Beta1 struct {
	V1Beta1 *v1beta11.Runtime `protobuf:"bytes,1,opt,name=v1beta1,proto3,oneof"`
}

func (*FlowRuntimeMetadata_V1Beta1) isFlowRuntimeMetadata_Version() {}

// FlowGroupsMetadata wraps versioned execution groups metadata.
type FlowGroupsMetadata struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Types that are valid to be assigned to Version:
	//
	//	*FlowGroupsMetadata_V1Beta1
	Version       isFlowGroupsMetadata_Version `protobuf_oneof:"version"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *FlowGroupsMetadata) Reset() {
	*x = FlowGroupsMetadata{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[17]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *FlowGroupsMetadata) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*FlowGroupsMetadata) ProtoMessage() {}

func (x *FlowGroupsMetadata) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[17]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use FlowGroupsMetadata.ProtoReflect.Descriptor instead.
func (*FlowGroupsMetadata) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{17}
}

func (x *FlowGroupsMetadata) GetVersion() isFlowGroupsMetadata_Version {
	if x != nil {
		return x.Version
	}
	return nil
}

func (x *FlowGroupsMetadata) GetV1Beta1() *v1beta11.Groups {
	if x != nil {
		if x, ok := x.Version.(*FlowGroupsMetadata_V1Beta1); ok {
			return x.V1Beta1
		}
	}
	return nil
}

type isFlowGroupsMetadata_Version interface {
	isFlowGroupsMetadata_Version()
}

type FlowGroupsMetadata_V1Beta1 struct {
	V1Beta1 *v1beta11.Groups `protobuf:"bytes,1,opt,name=v1beta1,proto3,oneof"`
}

func (*FlowGroupsMetadata_V1Beta1) isFlowGroupsMetadata_Version() {}

// FlowSpecMetadata wraps versioned flow specification.
type FlowSpecMetadata struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Types that are valid to be assigned to Version:
	//
	//	*FlowSpecMetadata_V1Beta1
	Version       isFlowSpecMetadata_Version `protobuf_oneof:"version"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *FlowSpecMetadata) Reset() {
	*x = FlowSpecMetadata{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[18]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *FlowSpecMetadata) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*FlowSpecMetadata) ProtoMessage() {}

func (x *FlowSpecMetadata) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[18]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use FlowSpecMetadata.ProtoReflect.Descriptor instead.
func (*FlowSpecMetadata) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{18}
}

func (x *FlowSpecMetadata) GetVersion() isFlowSpecMetadata_Version {
	if x != nil {
		return x.Version
	}
	return nil
}

func (x *FlowSpecMetadata) GetV1Beta1() *v1beta11.Flow {
	if x != nil {
		if x, ok := x.Version.(*FlowSpecMetadata_V1Beta1); ok {
			return x.V1Beta1
		}
	}
	return nil
}

type isFlowSpecMetadata_Version interface {
	isFlowSpecMetadata_Version()
}

type FlowSpecMetadata_V1Beta1 struct {
	V1Beta1 *v1beta11.Flow `protobuf:"bytes,1,opt,name=v1beta1,proto3,oneof"`
}

func (*FlowSpecMetadata_V1Beta1) isFlowSpecMetadata_Version() {}

// FlowGraphMetadata wraps versioned flow graph representation.
type FlowGraphMetadata struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Types that are valid to be assigned to Version:
	//
	//	*FlowGraphMetadata_V1Beta1
	Version       isFlowGraphMetadata_Version `protobuf_oneof:"version"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *FlowGraphMetadata) Reset() {
	*x = FlowGraphMetadata{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[19]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *FlowGraphMetadata) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*FlowGraphMetadata) ProtoMessage() {}

func (x *FlowGraphMetadata) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[19]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use FlowGraphMetadata.ProtoReflect.Descriptor instead.
func (*FlowGraphMetadata) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{19}
}

func (x *FlowGraphMetadata) GetVersion() isFlowGraphMetadata_Version {
	if x != nil {
		return x.Version
	}
	return nil
}

func (x *FlowGraphMetadata) GetV1Beta1() *v1beta11.Graph {
	if x != nil {
		if x, ok := x.Version.(*FlowGraphMetadata_V1Beta1); ok {
			return x.V1Beta1
		}
	}
	return nil
}

type isFlowGraphMetadata_Version interface {
	isFlowGraphMetadata_Version()
}

type FlowGraphMetadata_V1Beta1 struct {
	V1Beta1 *v1beta11.Graph `protobuf:"bytes,1,opt,name=v1beta1,proto3,oneof"`
}

func (*FlowGraphMetadata_V1Beta1) isFlowGraphMetadata_Version() {}

// PackageSpecMetadata wraps versioned package specification.
type PackageSpecMetadata struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Types that are valid to be assigned to Version:
	//
	//	*PackageSpecMetadata_V1Beta1
	Version       isPackageSpecMetadata_Version `protobuf_oneof:"version"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *PackageSpecMetadata) Reset() {
	*x = PackageSpecMetadata{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[20]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *PackageSpecMetadata) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*PackageSpecMetadata) ProtoMessage() {}

func (x *PackageSpecMetadata) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[20]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use PackageSpecMetadata.ProtoReflect.Descriptor instead.
func (*PackageSpecMetadata) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{20}
}

func (x *PackageSpecMetadata) GetVersion() isPackageSpecMetadata_Version {
	if x != nil {
		return x.Version
	}
	return nil
}

func (x *PackageSpecMetadata) GetV1Beta1() *v1beta1.Package {
	if x != nil {
		if x, ok := x.Version.(*PackageSpecMetadata_V1Beta1); ok {
			return x.V1Beta1
		}
	}
	return nil
}

type isPackageSpecMetadata_Version interface {
	isPackageSpecMetadata_Version()
}

type PackageSpecMetadata_V1Beta1 struct {
	V1Beta1 *v1beta1.Package `protobuf:"bytes,1,opt,name=v1beta1,proto3,oneof"`
}

func (*PackageSpecMetadata_V1Beta1) isPackageSpecMetadata_Version() {}

// PackageBuildMetadata contains build metadata for a software package, which
// may be used to build a deployment artifact for a compatible runtime environment.
type PackageBuildMetadata struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Target runtime of build artifact.
	Runtime *PackageBuildMetadata_Runtime `protobuf:"bytes,1,opt,name=runtime,proto3" json:"runtime,omitempty"`
	// Target platform of build artifact.
	Platform *PackageBuildMetadata_Platform `protobuf:"bytes,2,opt,name=platform,proto3" json:"platform,omitempty"`
	// Build environment variables.
	Env map[string]string `protobuf:"bytes,3,rep,name=env,proto3" json:"env,omitempty" protobuf_key:"bytes,1,opt,name=key" protobuf_val:"bytes,2,opt,name=value"`
	// URI of build artifact. (e.g. file://path/to/artifact or docker://image:tag)
	ArtifactUri string `protobuf:"bytes,4,opt,name=artifact_uri,json=artifactUri,proto3" json:"artifact_uri,omitempty"`
	// Checksum of build artifact (present if build is successful).
	ArtifactEtag  string `protobuf:"bytes,5,opt,name=artifact_etag,json=artifactEtag,proto3" json:"artifact_etag,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *PackageBuildMetadata) Reset() {
	*x = PackageBuildMetadata{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[21]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *PackageBuildMetadata) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*PackageBuildMetadata) ProtoMessage() {}

func (x *PackageBuildMetadata) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[21]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use PackageBuildMetadata.ProtoReflect.Descriptor instead.
func (*PackageBuildMetadata) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{21}
}

func (x *PackageBuildMetadata) GetRuntime() *PackageBuildMetadata_Runtime {
	if x != nil {
		return x.Runtime
	}
	return nil
}

func (x *PackageBuildMetadata) GetPlatform() *PackageBuildMetadata_Platform {
	if x != nil {
		return x.Platform
	}
	return nil
}

func (x *PackageBuildMetadata) GetEnv() map[string]string {
	if x != nil {
		return x.Env
	}
	return nil
}

func (x *PackageBuildMetadata) GetArtifactUri() string {
	if x != nil {
		return x.ArtifactUri
	}
	return ""
}

func (x *PackageBuildMetadata) GetArtifactEtag() string {
	if x != nil {
		return x.ArtifactEtag
	}
	return ""
}

// BuildOperationMetadata contains metadata for build operations.
type BuildOperationMetadata struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Build state.
	State BuildOperationMetadata_State `protobuf:"varint,1,opt,name=state,proto3,enum=dtkt.core.v1.BuildOperationMetadata_State" json:"state,omitempty"`
	// Build status message.
	Message string `protobuf:"bytes,2,opt,name=message,proto3" json:"message,omitempty"`
	// Build start time.
	StartTime *timestamppb.Timestamp `protobuf:"bytes,20,opt,name=start_time,json=startTime,proto3" json:"start_time,omitempty"`
	// Build finish time.
	FinishTime    *timestamppb.Timestamp `protobuf:"bytes,21,opt,name=finish_time,json=finishTime,proto3" json:"finish_time,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *BuildOperationMetadata) Reset() {
	*x = BuildOperationMetadata{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[22]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *BuildOperationMetadata) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*BuildOperationMetadata) ProtoMessage() {}

func (x *BuildOperationMetadata) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[22]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use BuildOperationMetadata.ProtoReflect.Descriptor instead.
func (*BuildOperationMetadata) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{22}
}

func (x *BuildOperationMetadata) GetState() BuildOperationMetadata_State {
	if x != nil {
		return x.State
	}
	return BuildOperationMetadata_STATE_UNSPECIFIED
}

func (x *BuildOperationMetadata) GetMessage() string {
	if x != nil {
		return x.Message
	}
	return ""
}

func (x *BuildOperationMetadata) GetStartTime() *timestamppb.Timestamp {
	if x != nil {
		return x.StartTime
	}
	return nil
}

func (x *BuildOperationMetadata) GetFinishTime() *timestamppb.Timestamp {
	if x != nil {
		return x.FinishTime
	}
	return nil
}

// RunOperationMetadata contains metadata for run operations.
type RunOperationMetadata struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Run state.
	State RunOperationMetadata_State `protobuf:"varint,1,opt,name=state,proto3,enum=dtkt.core.v1.RunOperationMetadata_State" json:"state,omitempty"`
	// Run status message.
	Message string `protobuf:"bytes,2,opt,name=message,proto3" json:"message,omitempty"`
	// Run start time.
	StartTime *timestamppb.Timestamp `protobuf:"bytes,20,opt,name=start_time,json=startTime,proto3" json:"start_time,omitempty"`
	// Run stop time.
	StopTime      *timestamppb.Timestamp `protobuf:"bytes,21,opt,name=stop_time,json=stopTime,proto3" json:"stop_time,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *RunOperationMetadata) Reset() {
	*x = RunOperationMetadata{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[23]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *RunOperationMetadata) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*RunOperationMetadata) ProtoMessage() {}

func (x *RunOperationMetadata) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[23]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use RunOperationMetadata.ProtoReflect.Descriptor instead.
func (*RunOperationMetadata) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{23}
}

func (x *RunOperationMetadata) GetState() RunOperationMetadata_State {
	if x != nil {
		return x.State
	}
	return RunOperationMetadata_STATE_UNSPECIFIED
}

func (x *RunOperationMetadata) GetMessage() string {
	if x != nil {
		return x.Message
	}
	return ""
}

func (x *RunOperationMetadata) GetStartTime() *timestamppb.Timestamp {
	if x != nil {
		return x.StartTime
	}
	return nil
}

func (x *RunOperationMetadata) GetStopTime() *timestamppb.Timestamp {
	if x != nil {
		return x.StopTime
	}
	return nil
}

// SyncOperationMetadata contains metadata for sync operations.
type SyncOperationMetadata struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Sync state.
	State SyncOperationMetadata_State `protobuf:"varint,1,opt,name=state,proto3,enum=dtkt.core.v1.SyncOperationMetadata_State" json:"state,omitempty"`
	// Sync status message.
	Message       string `protobuf:"bytes,2,opt,name=message,proto3" json:"message,omitempty"`
	ServicesCount int64  `protobuf:"varint,9,opt,name=services_count,json=servicesCount,proto3" json:"services_count,omitempty"`
	MethodsCount  int64  `protobuf:"varint,10,opt,name=methods_count,json=methodsCount,proto3" json:"methods_count,omitempty"`
	TypesCount    int64  `protobuf:"varint,11,opt,name=types_count,json=typesCount,proto3" json:"types_count,omitempty"`
	// Sync start time.
	StartTime *timestamppb.Timestamp `protobuf:"bytes,20,opt,name=start_time,json=startTime,proto3" json:"start_time,omitempty"`
	// Sync stop time.
	StopTime      *timestamppb.Timestamp `protobuf:"bytes,21,opt,name=stop_time,json=stopTime,proto3" json:"stop_time,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *SyncOperationMetadata) Reset() {
	*x = SyncOperationMetadata{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[24]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *SyncOperationMetadata) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*SyncOperationMetadata) ProtoMessage() {}

func (x *SyncOperationMetadata) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[24]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use SyncOperationMetadata.ProtoReflect.Descriptor instead.
func (*SyncOperationMetadata) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{24}
}

func (x *SyncOperationMetadata) GetState() SyncOperationMetadata_State {
	if x != nil {
		return x.State
	}
	return SyncOperationMetadata_STATE_UNSPECIFIED
}

func (x *SyncOperationMetadata) GetMessage() string {
	if x != nil {
		return x.Message
	}
	return ""
}

func (x *SyncOperationMetadata) GetServicesCount() int64 {
	if x != nil {
		return x.ServicesCount
	}
	return 0
}

func (x *SyncOperationMetadata) GetMethodsCount() int64 {
	if x != nil {
		return x.MethodsCount
	}
	return 0
}

func (x *SyncOperationMetadata) GetTypesCount() int64 {
	if x != nil {
		return x.TypesCount
	}
	return 0
}

func (x *SyncOperationMetadata) GetStartTime() *timestamppb.Timestamp {
	if x != nil {
		return x.StartTime
	}
	return nil
}

func (x *SyncOperationMetadata) GetStopTime() *timestamppb.Timestamp {
	if x != nil {
		return x.StopTime
	}
	return nil
}

type DialMetadata struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Connection dial state.
	State DialMetadata_State `protobuf:"varint,1,opt,name=state,proto3,enum=dtkt.core.v1.DialMetadata_State" json:"state,omitempty"`
	// Connection dial message.
	Message string `protobuf:"bytes,2,opt,name=message,proto3" json:"message,omitempty"`
	// Connection dial time.
	Time          *timestamppb.Timestamp `protobuf:"bytes,3,opt,name=time,proto3" json:"time,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *DialMetadata) Reset() {
	*x = DialMetadata{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[25]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *DialMetadata) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*DialMetadata) ProtoMessage() {}

func (x *DialMetadata) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[25]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use DialMetadata.ProtoReflect.Descriptor instead.
func (*DialMetadata) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{25}
}

func (x *DialMetadata) GetState() DialMetadata_State {
	if x != nil {
		return x.State
	}
	return DialMetadata_STATE_UNSPECIFIED
}

func (x *DialMetadata) GetMessage() string {
	if x != nil {
		return x.Message
	}
	return ""
}

func (x *DialMetadata) GetTime() *timestamppb.Timestamp {
	if x != nil {
		return x.Time
	}
	return nil
}

// BatchRunOperationMetadata contains metadata for batch run operations.
type BatchRunOperationMetadata struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Map of request index to failure status.
	FailedRequests map[int32]*status.Status `protobuf:"bytes,1,rep,name=failed_requests,json=failedRequests,proto3" json:"failed_requests,omitempty" protobuf_key:"varint,1,opt,name=key" protobuf_val:"bytes,2,opt,name=value"`
	unknownFields  protoimpl.UnknownFields
	sizeCache      protoimpl.SizeCache
}

func (x *BatchRunOperationMetadata) Reset() {
	*x = BatchRunOperationMetadata{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[26]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *BatchRunOperationMetadata) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*BatchRunOperationMetadata) ProtoMessage() {}

func (x *BatchRunOperationMetadata) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[26]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use BatchRunOperationMetadata.ProtoReflect.Descriptor instead.
func (*BatchRunOperationMetadata) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{26}
}

func (x *BatchRunOperationMetadata) GetFailedRequests() map[int32]*status.Status {
	if x != nil {
		return x.FailedRequests
	}
	return nil
}

// EncryptedAny is an encrypted representation of google.protobuf.Any.
type EncryptedAny struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Type URL of the encrypted message.
	TypeUrl string `protobuf:"bytes,1,opt,name=type_url,json=typeUrl,proto3" json:"type_url,omitempty"`
	// Encrypted message value.
	Value         []byte `protobuf:"bytes,2,opt,name=value,proto3" json:"value,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *EncryptedAny) Reset() {
	*x = EncryptedAny{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[27]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *EncryptedAny) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*EncryptedAny) ProtoMessage() {}

func (x *EncryptedAny) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[27]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use EncryptedAny.ProtoReflect.Descriptor instead.
func (*EncryptedAny) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{27}
}

func (x *EncryptedAny) GetTypeUrl() string {
	if x != nil {
		return x.TypeUrl
	}
	return ""
}

func (x *EncryptedAny) GetValue() []byte {
	if x != nil {
		return x.Value
	}
	return nil
}

// EncryptRequest requests encryption of a resource with decrypted data.
type EncryptRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Resource with decrypted data to encrypt.
	Resource      *Resource `protobuf:"bytes,1,opt,name=resource,proto3" json:"resource,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *EncryptRequest) Reset() {
	*x = EncryptRequest{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[28]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *EncryptRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*EncryptRequest) ProtoMessage() {}

func (x *EncryptRequest) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[28]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use EncryptRequest.ProtoReflect.Descriptor instead.
func (*EncryptRequest) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{28}
}

func (x *EncryptRequest) GetResource() *Resource {
	if x != nil {
		return x.Resource
	}
	return nil
}

// EncryptResponse returns the encrypted resource.
type EncryptResponse struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Resource with encrypted data.
	Resource      *Resource `protobuf:"bytes,1,opt,name=resource,proto3" json:"resource,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *EncryptResponse) Reset() {
	*x = EncryptResponse{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[29]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *EncryptResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*EncryptResponse) ProtoMessage() {}

func (x *EncryptResponse) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[29]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use EncryptResponse.ProtoReflect.Descriptor instead.
func (*EncryptResponse) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{29}
}

func (x *EncryptResponse) GetResource() *Resource {
	if x != nil {
		return x.Resource
	}
	return nil
}

// DecryptRequest requests decryption of a resource with encrypted data.
type DecryptRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Resource with encrypted data to decrypt.
	Resource      *Resource `protobuf:"bytes,1,opt,name=resource,proto3" json:"resource,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *DecryptRequest) Reset() {
	*x = DecryptRequest{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[30]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *DecryptRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*DecryptRequest) ProtoMessage() {}

func (x *DecryptRequest) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[30]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use DecryptRequest.ProtoReflect.Descriptor instead.
func (*DecryptRequest) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{30}
}

func (x *DecryptRequest) GetResource() *Resource {
	if x != nil {
		return x.Resource
	}
	return nil
}

// DecryptResponse returns the decrypted resource.
type DecryptResponse struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Resource with decrypted data.
	Resource      *Resource `protobuf:"bytes,1,opt,name=resource,proto3" json:"resource,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *DecryptResponse) Reset() {
	*x = DecryptResponse{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[31]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *DecryptResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*DecryptResponse) ProtoMessage() {}

func (x *DecryptResponse) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[31]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use DecryptResponse.ProtoReflect.Descriptor instead.
func (*DecryptResponse) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{31}
}

func (x *DecryptResponse) GetResource() *Resource {
	if x != nil {
		return x.Resource
	}
	return nil
}

type GetAutomationRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Name of Automation resource to get.
	Name          string `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetAutomationRequest) Reset() {
	*x = GetAutomationRequest{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[32]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetAutomationRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetAutomationRequest) ProtoMessage() {}

func (x *GetAutomationRequest) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[32]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetAutomationRequest.ProtoReflect.Descriptor instead.
func (*GetAutomationRequest) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{32}
}

func (x *GetAutomationRequest) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

type GetAutomationResponse struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// The requested automation.
	Automation    *Automation `protobuf:"bytes,1,opt,name=automation,proto3" json:"automation,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetAutomationResponse) Reset() {
	*x = GetAutomationResponse{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[33]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetAutomationResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetAutomationResponse) ProtoMessage() {}

func (x *GetAutomationResponse) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[33]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetAutomationResponse.ProtoReflect.Descriptor instead.
func (*GetAutomationResponse) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{33}
}

func (x *GetAutomationResponse) GetAutomation() *Automation {
	if x != nil {
		return x.Automation
	}
	return nil
}

type CreateAutomationRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Parent resource name.
	Parent       string `protobuf:"bytes,1,opt,name=parent,proto3" json:"parent,omitempty"`
	AutomationId string `protobuf:"bytes,2,opt,name=automation_id,json=automationId,proto3" json:"automation_id,omitempty"`
	// Automation to create.
	Automation    *Automation `protobuf:"bytes,3,opt,name=automation,proto3" json:"automation,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *CreateAutomationRequest) Reset() {
	*x = CreateAutomationRequest{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[34]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *CreateAutomationRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CreateAutomationRequest) ProtoMessage() {}

func (x *CreateAutomationRequest) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[34]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CreateAutomationRequest.ProtoReflect.Descriptor instead.
func (*CreateAutomationRequest) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{34}
}

func (x *CreateAutomationRequest) GetParent() string {
	if x != nil {
		return x.Parent
	}
	return ""
}

func (x *CreateAutomationRequest) GetAutomationId() string {
	if x != nil {
		return x.AutomationId
	}
	return ""
}

func (x *CreateAutomationRequest) GetAutomation() *Automation {
	if x != nil {
		return x.Automation
	}
	return nil
}

type CreateAutomationResponse struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// The created automation.
	Automation    *Automation `protobuf:"bytes,1,opt,name=automation,proto3" json:"automation,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *CreateAutomationResponse) Reset() {
	*x = CreateAutomationResponse{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[35]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *CreateAutomationResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CreateAutomationResponse) ProtoMessage() {}

func (x *CreateAutomationResponse) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[35]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CreateAutomationResponse.ProtoReflect.Descriptor instead.
func (*CreateAutomationResponse) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{35}
}

func (x *CreateAutomationResponse) GetAutomation() *Automation {
	if x != nil {
		return x.Automation
	}
	return nil
}

type BatchCreateAutomationsRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Parent resource name shared by all automations this batch.
	Parent        string                     `protobuf:"bytes,1,opt,name=parent,proto3" json:"parent,omitempty"`
	Requests      []*CreateAutomationRequest `protobuf:"bytes,2,rep,name=requests,proto3" json:"requests,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *BatchCreateAutomationsRequest) Reset() {
	*x = BatchCreateAutomationsRequest{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[36]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *BatchCreateAutomationsRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*BatchCreateAutomationsRequest) ProtoMessage() {}

func (x *BatchCreateAutomationsRequest) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[36]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use BatchCreateAutomationsRequest.ProtoReflect.Descriptor instead.
func (*BatchCreateAutomationsRequest) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{36}
}

func (x *BatchCreateAutomationsRequest) GetParent() string {
	if x != nil {
		return x.Parent
	}
	return ""
}

func (x *BatchCreateAutomationsRequest) GetRequests() []*CreateAutomationRequest {
	if x != nil {
		return x.Requests
	}
	return nil
}

type BatchCreateAutomationsResponse struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// The created automations.
	Automations   []*Automation `protobuf:"bytes,1,rep,name=automations,proto3" json:"automations,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *BatchCreateAutomationsResponse) Reset() {
	*x = BatchCreateAutomationsResponse{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[37]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *BatchCreateAutomationsResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*BatchCreateAutomationsResponse) ProtoMessage() {}

func (x *BatchCreateAutomationsResponse) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[37]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use BatchCreateAutomationsResponse.ProtoReflect.Descriptor instead.
func (*BatchCreateAutomationsResponse) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{37}
}

func (x *BatchCreateAutomationsResponse) GetAutomations() []*Automation {
	if x != nil {
		return x.Automations
	}
	return nil
}

type ListAutomationsRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Maximum number of automations to return.
	PageSize int32 `protobuf:"varint,1,opt,name=page_size,json=pageSize,proto3" json:"page_size,omitempty"`
	// Token for retrieving the next page.
	PageToken string `protobuf:"bytes,2,opt,name=page_token,json=pageToken,proto3" json:"page_token,omitempty"`
	// Parent resource name: `organizations/{organization}` or `users/{user}`.
	Parent *string `protobuf:"bytes,3,opt,name=parent,proto3,oneof" json:"parent,omitempty"`
	// Filter expression.
	Filter        *string `protobuf:"bytes,4,opt,name=filter,proto3,oneof" json:"filter,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListAutomationsRequest) Reset() {
	*x = ListAutomationsRequest{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[38]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListAutomationsRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListAutomationsRequest) ProtoMessage() {}

func (x *ListAutomationsRequest) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[38]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListAutomationsRequest.ProtoReflect.Descriptor instead.
func (*ListAutomationsRequest) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{38}
}

func (x *ListAutomationsRequest) GetPageSize() int32 {
	if x != nil {
		return x.PageSize
	}
	return 0
}

func (x *ListAutomationsRequest) GetPageToken() string {
	if x != nil {
		return x.PageToken
	}
	return ""
}

func (x *ListAutomationsRequest) GetParent() string {
	if x != nil && x.Parent != nil {
		return *x.Parent
	}
	return ""
}

func (x *ListAutomationsRequest) GetFilter() string {
	if x != nil && x.Filter != nil {
		return *x.Filter
	}
	return ""
}

type ListAutomationsResponse struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// The list of automations.
	Automations []*Automation `protobuf:"bytes,1,rep,name=automations,proto3" json:"automations,omitempty"`
	// Token for retrieving the next page.
	NextPageToken string `protobuf:"bytes,2,opt,name=next_page_token,json=nextPageToken,proto3" json:"next_page_token,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListAutomationsResponse) Reset() {
	*x = ListAutomationsResponse{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[39]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListAutomationsResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListAutomationsResponse) ProtoMessage() {}

func (x *ListAutomationsResponse) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[39]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListAutomationsResponse.ProtoReflect.Descriptor instead.
func (*ListAutomationsResponse) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{39}
}

func (x *ListAutomationsResponse) GetAutomations() []*Automation {
	if x != nil {
		return x.Automations
	}
	return nil
}

func (x *ListAutomationsResponse) GetNextPageToken() string {
	if x != nil {
		return x.NextPageToken
	}
	return ""
}

// SendAutomationEvent represents an event sent to an automation.
type SendAutomationEvent struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Types that are valid to be assigned to Type:
	//
	//	*SendAutomationEvent_InputsEvent_
	//	*SendAutomationEvent_UserResponseEvent_
	Type          isSendAutomationEvent_Type `protobuf_oneof:"type"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *SendAutomationEvent) Reset() {
	*x = SendAutomationEvent{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[40]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *SendAutomationEvent) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*SendAutomationEvent) ProtoMessage() {}

func (x *SendAutomationEvent) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[40]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use SendAutomationEvent.ProtoReflect.Descriptor instead.
func (*SendAutomationEvent) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{40}
}

func (x *SendAutomationEvent) GetType() isSendAutomationEvent_Type {
	if x != nil {
		return x.Type
	}
	return nil
}

func (x *SendAutomationEvent) GetInputsEvent() *SendAutomationEvent_InputsEvent {
	if x != nil {
		if x, ok := x.Type.(*SendAutomationEvent_InputsEvent_); ok {
			return x.InputsEvent
		}
	}
	return nil
}

func (x *SendAutomationEvent) GetUserResponseEvent() *SendAutomationEvent_UserResponseEvent {
	if x != nil {
		if x, ok := x.Type.(*SendAutomationEvent_UserResponseEvent_); ok {
			return x.UserResponseEvent
		}
	}
	return nil
}

type isSendAutomationEvent_Type interface {
	isSendAutomationEvent_Type()
}

type SendAutomationEvent_InputsEvent_ struct {
	InputsEvent *SendAutomationEvent_InputsEvent `protobuf:"bytes,1,opt,name=inputs_event,json=inputsEvent,proto3,oneof"`
}

type SendAutomationEvent_UserResponseEvent_ struct {
	UserResponseEvent *SendAutomationEvent_UserResponseEvent `protobuf:"bytes,2,opt,name=user_response_event,json=userResponseEvent,proto3,oneof"`
}

func (*SendAutomationEvent_InputsEvent_) isSendAutomationEvent_Type() {}

func (*SendAutomationEvent_UserResponseEvent_) isSendAutomationEvent_Type() {}

// ReceiveAutomationEvent represents an event received from an automation.
type ReceiveAutomationEvent struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Types that are valid to be assigned to Type:
	//
	//	*ReceiveAutomationEvent_OutputsEvent_
	//	*ReceiveAutomationEvent_UserRequestEvent_
	Type          isReceiveAutomationEvent_Type `protobuf_oneof:"type"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ReceiveAutomationEvent) Reset() {
	*x = ReceiveAutomationEvent{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[41]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ReceiveAutomationEvent) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ReceiveAutomationEvent) ProtoMessage() {}

func (x *ReceiveAutomationEvent) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[41]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ReceiveAutomationEvent.ProtoReflect.Descriptor instead.
func (*ReceiveAutomationEvent) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{41}
}

func (x *ReceiveAutomationEvent) GetType() isReceiveAutomationEvent_Type {
	if x != nil {
		return x.Type
	}
	return nil
}

func (x *ReceiveAutomationEvent) GetOutputsEvent() *ReceiveAutomationEvent_OutputsEvent {
	if x != nil {
		if x, ok := x.Type.(*ReceiveAutomationEvent_OutputsEvent_); ok {
			return x.OutputsEvent
		}
	}
	return nil
}

func (x *ReceiveAutomationEvent) GetUserRequestEvent() *ReceiveAutomationEvent_UserRequestEvent {
	if x != nil {
		if x, ok := x.Type.(*ReceiveAutomationEvent_UserRequestEvent_); ok {
			return x.UserRequestEvent
		}
	}
	return nil
}

type isReceiveAutomationEvent_Type interface {
	isReceiveAutomationEvent_Type()
}

type ReceiveAutomationEvent_OutputsEvent_ struct {
	OutputsEvent *ReceiveAutomationEvent_OutputsEvent `protobuf:"bytes,1,opt,name=outputs_event,json=outputsEvent,proto3,oneof"`
}

type ReceiveAutomationEvent_UserRequestEvent_ struct {
	UserRequestEvent *ReceiveAutomationEvent_UserRequestEvent `protobuf:"bytes,2,opt,name=user_request_event,json=userRequestEvent,proto3,oneof"`
}

func (*ReceiveAutomationEvent_OutputsEvent_) isReceiveAutomationEvent_Type() {}

func (*ReceiveAutomationEvent_UserRequestEvent_) isReceiveAutomationEvent_Type() {}

type SendAutomationEventRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Name of Automation resource to send event.
	Name string `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`
	// Event to send.
	Event         *SendAutomationEvent `protobuf:"bytes,2,opt,name=event,proto3" json:"event,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *SendAutomationEventRequest) Reset() {
	*x = SendAutomationEventRequest{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[42]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *SendAutomationEventRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*SendAutomationEventRequest) ProtoMessage() {}

func (x *SendAutomationEventRequest) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[42]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use SendAutomationEventRequest.ProtoReflect.Descriptor instead.
func (*SendAutomationEventRequest) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{42}
}

func (x *SendAutomationEventRequest) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *SendAutomationEventRequest) GetEvent() *SendAutomationEvent {
	if x != nil {
		return x.Event
	}
	return nil
}

type ReceiveAutomationEventsRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Name of Automation resource to receive events.
	Name          string `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ReceiveAutomationEventsRequest) Reset() {
	*x = ReceiveAutomationEventsRequest{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[43]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ReceiveAutomationEventsRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ReceiveAutomationEventsRequest) ProtoMessage() {}

func (x *ReceiveAutomationEventsRequest) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[43]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ReceiveAutomationEventsRequest.ProtoReflect.Descriptor instead.
func (*ReceiveAutomationEventsRequest) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{43}
}

func (x *ReceiveAutomationEventsRequest) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

type ReceiveAutomationEventsResponse struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// The received event.
	Event         *ReceiveAutomationEvent `protobuf:"bytes,2,opt,name=event,proto3" json:"event,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ReceiveAutomationEventsResponse) Reset() {
	*x = ReceiveAutomationEventsResponse{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[44]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ReceiveAutomationEventsResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ReceiveAutomationEventsResponse) ProtoMessage() {}

func (x *ReceiveAutomationEventsResponse) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[44]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ReceiveAutomationEventsResponse.ProtoReflect.Descriptor instead.
func (*ReceiveAutomationEventsResponse) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{44}
}

func (x *ReceiveAutomationEventsResponse) GetEvent() *ReceiveAutomationEvent {
	if x != nil {
		return x.Event
	}
	return nil
}

type StreamAutomationEventsRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Name of Automation resource to send event.
	Name string `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`
	// Event to send.
	Event         *SendAutomationEvent `protobuf:"bytes,2,opt,name=event,proto3" json:"event,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *StreamAutomationEventsRequest) Reset() {
	*x = StreamAutomationEventsRequest{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[45]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *StreamAutomationEventsRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*StreamAutomationEventsRequest) ProtoMessage() {}

func (x *StreamAutomationEventsRequest) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[45]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use StreamAutomationEventsRequest.ProtoReflect.Descriptor instead.
func (*StreamAutomationEventsRequest) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{45}
}

func (x *StreamAutomationEventsRequest) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *StreamAutomationEventsRequest) GetEvent() *SendAutomationEvent {
	if x != nil {
		return x.Event
	}
	return nil
}

type StreamAutomationEventsResponse struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// The received event.
	Event         *ReceiveAutomationEvent `protobuf:"bytes,1,opt,name=event,proto3" json:"event,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *StreamAutomationEventsResponse) Reset() {
	*x = StreamAutomationEventsResponse{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[46]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *StreamAutomationEventsResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*StreamAutomationEventsResponse) ProtoMessage() {}

func (x *StreamAutomationEventsResponse) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[46]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use StreamAutomationEventsResponse.ProtoReflect.Descriptor instead.
func (*StreamAutomationEventsResponse) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{46}
}

func (x *StreamAutomationEventsResponse) GetEvent() *ReceiveAutomationEvent {
	if x != nil {
		return x.Event
	}
	return nil
}

type ListConnectionsRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Maximum number of connections to return.
	PageSize int32 `protobuf:"varint,1,opt,name=page_size,json=pageSize,proto3" json:"page_size,omitempty"`
	// Token for retrieving the next page.
	PageToken string `protobuf:"bytes,2,opt,name=page_token,json=pageToken,proto3" json:"page_token,omitempty"`
	// Parent resource name: `organizations/{organization}` or `users/{user}`.
	Parent *string `protobuf:"bytes,3,opt,name=parent,proto3,oneof" json:"parent,omitempty"`
	// Filter expression.
	Filter        *string `protobuf:"bytes,4,opt,name=filter,proto3,oneof" json:"filter,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListConnectionsRequest) Reset() {
	*x = ListConnectionsRequest{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[47]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListConnectionsRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListConnectionsRequest) ProtoMessage() {}

func (x *ListConnectionsRequest) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[47]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListConnectionsRequest.ProtoReflect.Descriptor instead.
func (*ListConnectionsRequest) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{47}
}

func (x *ListConnectionsRequest) GetPageSize() int32 {
	if x != nil {
		return x.PageSize
	}
	return 0
}

func (x *ListConnectionsRequest) GetPageToken() string {
	if x != nil {
		return x.PageToken
	}
	return ""
}

func (x *ListConnectionsRequest) GetParent() string {
	if x != nil && x.Parent != nil {
		return *x.Parent
	}
	return ""
}

func (x *ListConnectionsRequest) GetFilter() string {
	if x != nil && x.Filter != nil {
		return *x.Filter
	}
	return ""
}

type ListConnectionsResponse struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// The list of connections.
	Connections []*Connection `protobuf:"bytes,1,rep,name=connections,proto3" json:"connections,omitempty"`
	// Token for retrieving the next page.
	NextPageToken string `protobuf:"bytes,2,opt,name=next_page_token,json=nextPageToken,proto3" json:"next_page_token,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListConnectionsResponse) Reset() {
	*x = ListConnectionsResponse{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[48]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListConnectionsResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListConnectionsResponse) ProtoMessage() {}

func (x *ListConnectionsResponse) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[48]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListConnectionsResponse.ProtoReflect.Descriptor instead.
func (*ListConnectionsResponse) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{48}
}

func (x *ListConnectionsResponse) GetConnections() []*Connection {
	if x != nil {
		return x.Connections
	}
	return nil
}

func (x *ListConnectionsResponse) GetNextPageToken() string {
	if x != nil {
		return x.NextPageToken
	}
	return ""
}

type GetConnectionRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Name of connection to get.
	Name          string `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetConnectionRequest) Reset() {
	*x = GetConnectionRequest{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[49]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetConnectionRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetConnectionRequest) ProtoMessage() {}

func (x *GetConnectionRequest) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[49]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetConnectionRequest.ProtoReflect.Descriptor instead.
func (*GetConnectionRequest) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{49}
}

func (x *GetConnectionRequest) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

type GetConnectionResponse struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// The requested connection.
	Connection    *Connection `protobuf:"bytes,1,opt,name=connection,proto3" json:"connection,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetConnectionResponse) Reset() {
	*x = GetConnectionResponse{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[50]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetConnectionResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetConnectionResponse) ProtoMessage() {}

func (x *GetConnectionResponse) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[50]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetConnectionResponse.ProtoReflect.Descriptor instead.
func (*GetConnectionResponse) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{50}
}

func (x *GetConnectionResponse) GetConnection() *Connection {
	if x != nil {
		return x.Connection
	}
	return nil
}

type CreateConnectionRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Parent resource name.
	Parent string `protobuf:"bytes,1,opt,name=parent,proto3" json:"parent,omitempty"`
	// Client-provided ID for connection resource.
	ConnectionId string `protobuf:"bytes,2,opt,name=connection_id,json=connectionId,proto3" json:"connection_id,omitempty"`
	// Connection to create.
	Connection    *Connection `protobuf:"bytes,3,opt,name=connection,proto3" json:"connection,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *CreateConnectionRequest) Reset() {
	*x = CreateConnectionRequest{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[51]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *CreateConnectionRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CreateConnectionRequest) ProtoMessage() {}

func (x *CreateConnectionRequest) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[51]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CreateConnectionRequest.ProtoReflect.Descriptor instead.
func (*CreateConnectionRequest) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{51}
}

func (x *CreateConnectionRequest) GetParent() string {
	if x != nil {
		return x.Parent
	}
	return ""
}

func (x *CreateConnectionRequest) GetConnectionId() string {
	if x != nil {
		return x.ConnectionId
	}
	return ""
}

func (x *CreateConnectionRequest) GetConnection() *Connection {
	if x != nil {
		return x.Connection
	}
	return nil
}

type CreateConnectionResponse struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// The created connection.
	Connection    *Connection `protobuf:"bytes,1,opt,name=connection,proto3" json:"connection,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *CreateConnectionResponse) Reset() {
	*x = CreateConnectionResponse{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[52]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *CreateConnectionResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CreateConnectionResponse) ProtoMessage() {}

func (x *CreateConnectionResponse) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[52]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CreateConnectionResponse.ProtoReflect.Descriptor instead.
func (*CreateConnectionResponse) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{52}
}

func (x *CreateConnectionResponse) GetConnection() *Connection {
	if x != nil {
		return x.Connection
	}
	return nil
}

type UpdateConnectionRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// The connection with updated fields. Must include `name`.
	Connection *Connection `protobuf:"bytes,1,opt,name=connection,proto3" json:"connection,omitempty"`
	// Fields to update (e.g., "config_data,integration").
	UpdateMask    *fieldmaskpb.FieldMask `protobuf:"bytes,2,opt,name=update_mask,json=updateMask,proto3" json:"update_mask,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *UpdateConnectionRequest) Reset() {
	*x = UpdateConnectionRequest{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[53]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UpdateConnectionRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UpdateConnectionRequest) ProtoMessage() {}

func (x *UpdateConnectionRequest) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[53]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UpdateConnectionRequest.ProtoReflect.Descriptor instead.
func (*UpdateConnectionRequest) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{53}
}

func (x *UpdateConnectionRequest) GetConnection() *Connection {
	if x != nil {
		return x.Connection
	}
	return nil
}

func (x *UpdateConnectionRequest) GetUpdateMask() *fieldmaskpb.FieldMask {
	if x != nil {
		return x.UpdateMask
	}
	return nil
}

type UpdateConnectionResponse struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// The updated connection.
	Connection    *Connection `protobuf:"bytes,1,opt,name=connection,proto3" json:"connection,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *UpdateConnectionResponse) Reset() {
	*x = UpdateConnectionResponse{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[54]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UpdateConnectionResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UpdateConnectionResponse) ProtoMessage() {}

func (x *UpdateConnectionResponse) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[54]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UpdateConnectionResponse.ProtoReflect.Descriptor instead.
func (*UpdateConnectionResponse) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{54}
}

func (x *UpdateConnectionResponse) GetConnection() *Connection {
	if x != nil {
		return x.Connection
	}
	return nil
}

type DeleteConnectionRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Name of connection to delete.
	Name          string `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *DeleteConnectionRequest) Reset() {
	*x = DeleteConnectionRequest{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[55]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *DeleteConnectionRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*DeleteConnectionRequest) ProtoMessage() {}

func (x *DeleteConnectionRequest) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[55]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use DeleteConnectionRequest.ProtoReflect.Descriptor instead.
func (*DeleteConnectionRequest) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{55}
}

func (x *DeleteConnectionRequest) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

type DeleteConnectionResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *DeleteConnectionResponse) Reset() {
	*x = DeleteConnectionResponse{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[56]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *DeleteConnectionResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*DeleteConnectionResponse) ProtoMessage() {}

func (x *DeleteConnectionResponse) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[56]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use DeleteConnectionResponse.ProtoReflect.Descriptor instead.
func (*DeleteConnectionResponse) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{56}
}

type DialConnectionRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Types that are valid to be assigned to Target:
	//
	//	*DialConnectionRequest_Name
	//	*DialConnectionRequest_Address
	Target        isDialConnectionRequest_Target `protobuf_oneof:"target"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *DialConnectionRequest) Reset() {
	*x = DialConnectionRequest{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[57]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *DialConnectionRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*DialConnectionRequest) ProtoMessage() {}

func (x *DialConnectionRequest) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[57]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use DialConnectionRequest.ProtoReflect.Descriptor instead.
func (*DialConnectionRequest) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{57}
}

func (x *DialConnectionRequest) GetTarget() isDialConnectionRequest_Target {
	if x != nil {
		return x.Target
	}
	return nil
}

func (x *DialConnectionRequest) GetName() string {
	if x != nil {
		if x, ok := x.Target.(*DialConnectionRequest_Name); ok {
			return x.Name
		}
	}
	return ""
}

func (x *DialConnectionRequest) GetAddress() string {
	if x != nil {
		if x, ok := x.Target.(*DialConnectionRequest_Address); ok {
			return x.Address
		}
	}
	return ""
}

type isDialConnectionRequest_Target interface {
	isDialConnectionRequest_Target()
}

type DialConnectionRequest_Name struct {
	// Name of connection to dial.
	Name string `protobuf:"bytes,1,opt,name=name,proto3,oneof"`
}

type DialConnectionRequest_Address struct {
	// Address of connection to dial.
	Address string `protobuf:"bytes,2,opt,name=address,proto3,oneof"`
}

func (*DialConnectionRequest_Name) isDialConnectionRequest_Target() {}

func (*DialConnectionRequest_Address) isDialConnectionRequest_Target() {}

type DialConnectionResponse struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Dial metadata.
	Dial          *DialMetadata `protobuf:"bytes,1,opt,name=dial,proto3" json:"dial,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *DialConnectionResponse) Reset() {
	*x = DialConnectionResponse{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[58]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *DialConnectionResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*DialConnectionResponse) ProtoMessage() {}

func (x *DialConnectionResponse) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[58]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use DialConnectionResponse.ProtoReflect.Descriptor instead.
func (*DialConnectionResponse) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{58}
}

func (x *DialConnectionResponse) GetDial() *DialMetadata {
	if x != nil {
		return x.Dial
	}
	return nil
}

type SyncDescriptorsRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Name of parent resource.
	Parent string `protobuf:"bytes,1,opt,name=parent,proto3" json:"parent,omitempty"`
	// Build etag must match current file etag for idempotent syncs and to ensure
	// descriptors are up-to-date with the latest connection or deployment config/build.
	// May be an empty string if no descriptors have been synced yet.
	Etag          string `protobuf:"bytes,2,opt,name=etag,proto3" json:"etag,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *SyncDescriptorsRequest) Reset() {
	*x = SyncDescriptorsRequest{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[59]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *SyncDescriptorsRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*SyncDescriptorsRequest) ProtoMessage() {}

func (x *SyncDescriptorsRequest) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[59]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use SyncDescriptorsRequest.ProtoReflect.Descriptor instead.
func (*SyncDescriptorsRequest) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{59}
}

func (x *SyncDescriptorsRequest) GetParent() string {
	if x != nil {
		return x.Parent
	}
	return ""
}

func (x *SyncDescriptorsRequest) GetEtag() string {
	if x != nil {
		return x.Etag
	}
	return ""
}

type SyncDescriptorsResponse struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// File containing descriptors.
	File          *File `protobuf:"bytes,1,opt,name=file,proto3" json:"file,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *SyncDescriptorsResponse) Reset() {
	*x = SyncDescriptorsResponse{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[60]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *SyncDescriptorsResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*SyncDescriptorsResponse) ProtoMessage() {}

func (x *SyncDescriptorsResponse) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[60]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use SyncDescriptorsResponse.ProtoReflect.Descriptor instead.
func (*SyncDescriptorsResponse) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{60}
}

func (x *SyncDescriptorsResponse) GetFile() *File {
	if x != nil {
		return x.File
	}
	return nil
}

type GetDescriptorsRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Name of parent resource.
	Parent        string `protobuf:"bytes,1,opt,name=parent,proto3" json:"parent,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetDescriptorsRequest) Reset() {
	*x = GetDescriptorsRequest{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[61]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetDescriptorsRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetDescriptorsRequest) ProtoMessage() {}

func (x *GetDescriptorsRequest) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[61]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetDescriptorsRequest.ProtoReflect.Descriptor instead.
func (*GetDescriptorsRequest) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{61}
}

func (x *GetDescriptorsRequest) GetParent() string {
	if x != nil {
		return x.Parent
	}
	return ""
}

type GetDescriptorsResponse struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// File containing descriptors.
	File          *File `protobuf:"bytes,1,opt,name=file,proto3" json:"file,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetDescriptorsResponse) Reset() {
	*x = GetDescriptorsResponse{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[62]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetDescriptorsResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetDescriptorsResponse) ProtoMessage() {}

func (x *GetDescriptorsResponse) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[62]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetDescriptorsResponse.ProtoReflect.Descriptor instead.
func (*GetDescriptorsResponse) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{62}
}

func (x *GetDescriptorsResponse) GetFile() *File {
	if x != nil {
		return x.File
	}
	return nil
}

type ListDeploymentsRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Maximum number of deployments to return.
	PageSize int32 `protobuf:"varint,1,opt,name=page_size,json=pageSize,proto3" json:"page_size,omitempty"`
	// Token for retrieving the next page.
	PageToken string `protobuf:"bytes,2,opt,name=page_token,json=pageToken,proto3" json:"page_token,omitempty"`
	// Parent resource name.
	Parent *string `protobuf:"bytes,3,opt,name=parent,proto3,oneof" json:"parent,omitempty"`
	// Filter expression.
	Filter        *string `protobuf:"bytes,4,opt,name=filter,proto3,oneof" json:"filter,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListDeploymentsRequest) Reset() {
	*x = ListDeploymentsRequest{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[63]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListDeploymentsRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListDeploymentsRequest) ProtoMessage() {}

func (x *ListDeploymentsRequest) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[63]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListDeploymentsRequest.ProtoReflect.Descriptor instead.
func (*ListDeploymentsRequest) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{63}
}

func (x *ListDeploymentsRequest) GetPageSize() int32 {
	if x != nil {
		return x.PageSize
	}
	return 0
}

func (x *ListDeploymentsRequest) GetPageToken() string {
	if x != nil {
		return x.PageToken
	}
	return ""
}

func (x *ListDeploymentsRequest) GetParent() string {
	if x != nil && x.Parent != nil {
		return *x.Parent
	}
	return ""
}

func (x *ListDeploymentsRequest) GetFilter() string {
	if x != nil && x.Filter != nil {
		return *x.Filter
	}
	return ""
}

type ListDeploymentsResponse struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// The list of deployments.
	Deployments []*Deployment `protobuf:"bytes,1,rep,name=deployments,proto3" json:"deployments,omitempty"`
	// Token for retrieving the next page.
	NextPageToken string `protobuf:"bytes,2,opt,name=next_page_token,json=nextPageToken,proto3" json:"next_page_token,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListDeploymentsResponse) Reset() {
	*x = ListDeploymentsResponse{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[64]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListDeploymentsResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListDeploymentsResponse) ProtoMessage() {}

func (x *ListDeploymentsResponse) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[64]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListDeploymentsResponse.ProtoReflect.Descriptor instead.
func (*ListDeploymentsResponse) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{64}
}

func (x *ListDeploymentsResponse) GetDeployments() []*Deployment {
	if x != nil {
		return x.Deployments
	}
	return nil
}

func (x *ListDeploymentsResponse) GetNextPageToken() string {
	if x != nil {
		return x.NextPageToken
	}
	return ""
}

type GetDeploymentRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Name of deployment to get.
	Name          string `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetDeploymentRequest) Reset() {
	*x = GetDeploymentRequest{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[65]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetDeploymentRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetDeploymentRequest) ProtoMessage() {}

func (x *GetDeploymentRequest) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[65]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetDeploymentRequest.ProtoReflect.Descriptor instead.
func (*GetDeploymentRequest) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{65}
}

func (x *GetDeploymentRequest) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

type GetDeploymentResponse struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// The requested deployment.
	Deployment    *Deployment `protobuf:"bytes,1,opt,name=deployment,proto3" json:"deployment,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetDeploymentResponse) Reset() {
	*x = GetDeploymentResponse{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[66]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetDeploymentResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetDeploymentResponse) ProtoMessage() {}

func (x *GetDeploymentResponse) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[66]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetDeploymentResponse.ProtoReflect.Descriptor instead.
func (*GetDeploymentResponse) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{66}
}

func (x *GetDeploymentResponse) GetDeployment() *Deployment {
	if x != nil {
		return x.Deployment
	}
	return nil
}

type CreateDeploymentRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Parent resource name.
	Parent string `protobuf:"bytes,1,opt,name=parent,proto3" json:"parent,omitempty"`
	// Client-provided ID for deployment resource.
	DeploymentId string `protobuf:"bytes,2,opt,name=deployment_id,json=deploymentId,proto3" json:"deployment_id,omitempty"`
	// Deployment to create (name ignored; server sets it).
	Deployment    *Deployment `protobuf:"bytes,3,opt,name=deployment,proto3" json:"deployment,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *CreateDeploymentRequest) Reset() {
	*x = CreateDeploymentRequest{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[67]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *CreateDeploymentRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CreateDeploymentRequest) ProtoMessage() {}

func (x *CreateDeploymentRequest) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[67]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CreateDeploymentRequest.ProtoReflect.Descriptor instead.
func (*CreateDeploymentRequest) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{67}
}

func (x *CreateDeploymentRequest) GetParent() string {
	if x != nil {
		return x.Parent
	}
	return ""
}

func (x *CreateDeploymentRequest) GetDeploymentId() string {
	if x != nil {
		return x.DeploymentId
	}
	return ""
}

func (x *CreateDeploymentRequest) GetDeployment() *Deployment {
	if x != nil {
		return x.Deployment
	}
	return nil
}

type UpdateDeploymentRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Deployment with updated fields. Must include `name`.
	Deployment *Deployment `protobuf:"bytes,1,opt,name=deployment,proto3" json:"deployment,omitempty"`
	// Fields to update (e.g., "config,address").
	UpdateMask *fieldmaskpb.FieldMask `protobuf:"bytes,2,opt,name=update_mask,json=updateMask,proto3" json:"update_mask,omitempty"`
	// Desired state of deployment after update.
	DesiredState  *Deployment_State `protobuf:"varint,3,opt,name=desired_state,json=desiredState,proto3,enum=dtkt.core.v1.Deployment_State,oneof" json:"desired_state,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *UpdateDeploymentRequest) Reset() {
	*x = UpdateDeploymentRequest{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[68]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UpdateDeploymentRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UpdateDeploymentRequest) ProtoMessage() {}

func (x *UpdateDeploymentRequest) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[68]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UpdateDeploymentRequest.ProtoReflect.Descriptor instead.
func (*UpdateDeploymentRequest) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{68}
}

func (x *UpdateDeploymentRequest) GetDeployment() *Deployment {
	if x != nil {
		return x.Deployment
	}
	return nil
}

func (x *UpdateDeploymentRequest) GetUpdateMask() *fieldmaskpb.FieldMask {
	if x != nil {
		return x.UpdateMask
	}
	return nil
}

func (x *UpdateDeploymentRequest) GetDesiredState() Deployment_State {
	if x != nil && x.DesiredState != nil {
		return *x.DesiredState
	}
	return Deployment_STATE_UNSPECIFIED
}

type DeleteDeploymentRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Name of deployment to delete.
	Name string `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`
	// Etag must match current deployment etag for delete to succeed (ensures
	// deployment is up-to-date and prevents lost deletes).
	Etag          string `protobuf:"bytes,2,opt,name=etag,proto3" json:"etag,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *DeleteDeploymentRequest) Reset() {
	*x = DeleteDeploymentRequest{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[69]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *DeleteDeploymentRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*DeleteDeploymentRequest) ProtoMessage() {}

func (x *DeleteDeploymentRequest) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[69]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use DeleteDeploymentRequest.ProtoReflect.Descriptor instead.
func (*DeleteDeploymentRequest) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{69}
}

func (x *DeleteDeploymentRequest) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *DeleteDeploymentRequest) GetEtag() string {
	if x != nil {
		return x.Etag
	}
	return ""
}

type ListFlowsRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Maximum number of flows to return.
	PageSize int32 `protobuf:"varint,1,opt,name=page_size,json=pageSize,proto3" json:"page_size,omitempty"`
	// Token for retrieving the next page.
	PageToken string `protobuf:"bytes,2,opt,name=page_token,json=pageToken,proto3" json:"page_token,omitempty"`
	// Parent resource name: `organizations/{organization}` or `users/{user}`.
	Parent *string `protobuf:"bytes,3,opt,name=parent,proto3,oneof" json:"parent,omitempty"`
	// Filter expression.
	Filter        *string `protobuf:"bytes,4,opt,name=filter,proto3,oneof" json:"filter,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListFlowsRequest) Reset() {
	*x = ListFlowsRequest{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[70]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListFlowsRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListFlowsRequest) ProtoMessage() {}

func (x *ListFlowsRequest) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[70]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListFlowsRequest.ProtoReflect.Descriptor instead.
func (*ListFlowsRequest) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{70}
}

func (x *ListFlowsRequest) GetPageSize() int32 {
	if x != nil {
		return x.PageSize
	}
	return 0
}

func (x *ListFlowsRequest) GetPageToken() string {
	if x != nil {
		return x.PageToken
	}
	return ""
}

func (x *ListFlowsRequest) GetParent() string {
	if x != nil && x.Parent != nil {
		return *x.Parent
	}
	return ""
}

func (x *ListFlowsRequest) GetFilter() string {
	if x != nil && x.Filter != nil {
		return *x.Filter
	}
	return ""
}

type ListFlowsResponse struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// The list of flows.
	Flows []*Flow `protobuf:"bytes,1,rep,name=flows,proto3" json:"flows,omitempty"`
	// Token for retrieving the next page.
	NextPageToken string `protobuf:"bytes,2,opt,name=next_page_token,json=nextPageToken,proto3" json:"next_page_token,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListFlowsResponse) Reset() {
	*x = ListFlowsResponse{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[71]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListFlowsResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListFlowsResponse) ProtoMessage() {}

func (x *ListFlowsResponse) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[71]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListFlowsResponse.ProtoReflect.Descriptor instead.
func (*ListFlowsResponse) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{71}
}

func (x *ListFlowsResponse) GetFlows() []*Flow {
	if x != nil {
		return x.Flows
	}
	return nil
}

func (x *ListFlowsResponse) GetNextPageToken() string {
	if x != nil {
		return x.NextPageToken
	}
	return ""
}

type GetFlowRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Flow resource name, one of.
	Name          string `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetFlowRequest) Reset() {
	*x = GetFlowRequest{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[72]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetFlowRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetFlowRequest) ProtoMessage() {}

func (x *GetFlowRequest) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[72]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetFlowRequest.ProtoReflect.Descriptor instead.
func (*GetFlowRequest) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{72}
}

func (x *GetFlowRequest) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

type GetFlowResponse struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// The requested flow.
	Flow          *Flow `protobuf:"bytes,1,opt,name=flow,proto3" json:"flow,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetFlowResponse) Reset() {
	*x = GetFlowResponse{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[73]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetFlowResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetFlowResponse) ProtoMessage() {}

func (x *GetFlowResponse) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[73]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetFlowResponse.ProtoReflect.Descriptor instead.
func (*GetFlowResponse) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{73}
}

func (x *GetFlowResponse) GetFlow() *Flow {
	if x != nil {
		return x.Flow
	}
	return nil
}

type CreateFlowRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Parent resource name.
	Parent string `protobuf:"bytes,1,opt,name=parent,proto3" json:"parent,omitempty"`
	// Client-provided ID for flow resource.
	FlowId string `protobuf:"bytes,2,opt,name=flow_id,json=flowId,proto3" json:"flow_id,omitempty"`
	// Flow to create.
	Flow          *Flow `protobuf:"bytes,3,opt,name=flow,proto3" json:"flow,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *CreateFlowRequest) Reset() {
	*x = CreateFlowRequest{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[74]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *CreateFlowRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CreateFlowRequest) ProtoMessage() {}

func (x *CreateFlowRequest) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[74]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CreateFlowRequest.ProtoReflect.Descriptor instead.
func (*CreateFlowRequest) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{74}
}

func (x *CreateFlowRequest) GetParent() string {
	if x != nil {
		return x.Parent
	}
	return ""
}

func (x *CreateFlowRequest) GetFlowId() string {
	if x != nil {
		return x.FlowId
	}
	return ""
}

func (x *CreateFlowRequest) GetFlow() *Flow {
	if x != nil {
		return x.Flow
	}
	return nil
}

type CreateFlowResponse struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// The created flow.
	Flow          *Flow `protobuf:"bytes,1,opt,name=flow,proto3" json:"flow,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *CreateFlowResponse) Reset() {
	*x = CreateFlowResponse{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[75]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *CreateFlowResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CreateFlowResponse) ProtoMessage() {}

func (x *CreateFlowResponse) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[75]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CreateFlowResponse.ProtoReflect.Descriptor instead.
func (*CreateFlowResponse) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{75}
}

func (x *CreateFlowResponse) GetFlow() *Flow {
	if x != nil {
		return x.Flow
	}
	return nil
}

type UpdateFlowRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Flow with updated fields. Must include `name`.
	Flow *Flow `protobuf:"bytes,1,opt,name=flow,proto3" json:"flow,omitempty"`
	// Fields to update (e.g., "spec").
	UpdateMask    *fieldmaskpb.FieldMask `protobuf:"bytes,2,opt,name=update_mask,json=updateMask,proto3" json:"update_mask,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *UpdateFlowRequest) Reset() {
	*x = UpdateFlowRequest{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[76]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UpdateFlowRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UpdateFlowRequest) ProtoMessage() {}

func (x *UpdateFlowRequest) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[76]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UpdateFlowRequest.ProtoReflect.Descriptor instead.
func (*UpdateFlowRequest) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{76}
}

func (x *UpdateFlowRequest) GetFlow() *Flow {
	if x != nil {
		return x.Flow
	}
	return nil
}

func (x *UpdateFlowRequest) GetUpdateMask() *fieldmaskpb.FieldMask {
	if x != nil {
		return x.UpdateMask
	}
	return nil
}

type UpdateFlowResponse struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// The updated flow.
	Flow          *Flow `protobuf:"bytes,1,opt,name=flow,proto3" json:"flow,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *UpdateFlowResponse) Reset() {
	*x = UpdateFlowResponse{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[77]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UpdateFlowResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UpdateFlowResponse) ProtoMessage() {}

func (x *UpdateFlowResponse) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[77]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UpdateFlowResponse.ProtoReflect.Descriptor instead.
func (*UpdateFlowResponse) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{77}
}

func (x *UpdateFlowResponse) GetFlow() *Flow {
	if x != nil {
		return x.Flow
	}
	return nil
}

type DeleteFlowRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Name of flow to delete.
	Name          string `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *DeleteFlowRequest) Reset() {
	*x = DeleteFlowRequest{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[78]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *DeleteFlowRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*DeleteFlowRequest) ProtoMessage() {}

func (x *DeleteFlowRequest) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[78]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use DeleteFlowRequest.ProtoReflect.Descriptor instead.
func (*DeleteFlowRequest) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{78}
}

func (x *DeleteFlowRequest) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

type DeleteFlowResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *DeleteFlowResponse) Reset() {
	*x = DeleteFlowResponse{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[79]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *DeleteFlowResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*DeleteFlowResponse) ProtoMessage() {}

func (x *DeleteFlowResponse) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[79]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use DeleteFlowResponse.ProtoReflect.Descriptor instead.
func (*DeleteFlowResponse) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{79}
}

type ListIntegrationsRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Maximum number of integrations to return.
	PageSize int32 `protobuf:"varint,1,opt,name=page_size,json=pageSize,proto3" json:"page_size,omitempty"`
	// Token for retrieving the next page.
	PageToken string `protobuf:"bytes,2,opt,name=page_token,json=pageToken,proto3" json:"page_token,omitempty"`
	// Parent resource name: `organizations/{organization}` or `users/{user}`.
	Parent *string `protobuf:"bytes,3,opt,name=parent,proto3,oneof" json:"parent,omitempty"`
	// Filter expression.
	Filter        *string `protobuf:"bytes,4,opt,name=filter,proto3,oneof" json:"filter,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListIntegrationsRequest) Reset() {
	*x = ListIntegrationsRequest{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[80]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListIntegrationsRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListIntegrationsRequest) ProtoMessage() {}

func (x *ListIntegrationsRequest) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[80]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListIntegrationsRequest.ProtoReflect.Descriptor instead.
func (*ListIntegrationsRequest) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{80}
}

func (x *ListIntegrationsRequest) GetPageSize() int32 {
	if x != nil {
		return x.PageSize
	}
	return 0
}

func (x *ListIntegrationsRequest) GetPageToken() string {
	if x != nil {
		return x.PageToken
	}
	return ""
}

func (x *ListIntegrationsRequest) GetParent() string {
	if x != nil && x.Parent != nil {
		return *x.Parent
	}
	return ""
}

func (x *ListIntegrationsRequest) GetFilter() string {
	if x != nil && x.Filter != nil {
		return *x.Filter
	}
	return ""
}

type ListIntegrationsResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Integrations  []*Integration         `protobuf:"bytes,1,rep,name=integrations,proto3" json:"integrations,omitempty"`
	NextPageToken string                 `protobuf:"bytes,2,opt,name=next_page_token,json=nextPageToken,proto3" json:"next_page_token,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListIntegrationsResponse) Reset() {
	*x = ListIntegrationsResponse{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[81]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListIntegrationsResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListIntegrationsResponse) ProtoMessage() {}

func (x *ListIntegrationsResponse) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[81]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListIntegrationsResponse.ProtoReflect.Descriptor instead.
func (*ListIntegrationsResponse) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{81}
}

func (x *ListIntegrationsResponse) GetIntegrations() []*Integration {
	if x != nil {
		return x.Integrations
	}
	return nil
}

func (x *ListIntegrationsResponse) GetNextPageToken() string {
	if x != nil {
		return x.NextPageToken
	}
	return ""
}

type GetIntegrationRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Integration resource name, one of:
	// - `organizations/{organization}/integrations/{integration}`
	// - `users/{user}/integrations/{integration}`
	Name          string `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetIntegrationRequest) Reset() {
	*x = GetIntegrationRequest{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[82]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetIntegrationRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetIntegrationRequest) ProtoMessage() {}

func (x *GetIntegrationRequest) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[82]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetIntegrationRequest.ProtoReflect.Descriptor instead.
func (*GetIntegrationRequest) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{82}
}

func (x *GetIntegrationRequest) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

type GetIntegrationResponse struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// The requested integration.
	Integration   *Integration `protobuf:"bytes,1,opt,name=integration,proto3" json:"integration,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetIntegrationResponse) Reset() {
	*x = GetIntegrationResponse{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[83]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetIntegrationResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetIntegrationResponse) ProtoMessage() {}

func (x *GetIntegrationResponse) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[83]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetIntegrationResponse.ProtoReflect.Descriptor instead.
func (*GetIntegrationResponse) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{83}
}

func (x *GetIntegrationResponse) GetIntegration() *Integration {
	if x != nil {
		return x.Integration
	}
	return nil
}

type CreateIntegrationRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Parent resource name.
	Parent string `protobuf:"bytes,1,opt,name=parent,proto3" json:"parent,omitempty"`
	// Client-provided ID for integration resource.
	IntegrationId string `protobuf:"bytes,2,opt,name=integration_id,json=integrationId,proto3" json:"integration_id,omitempty"`
	// Integration to create.
	Integration   *Integration `protobuf:"bytes,3,opt,name=integration,proto3" json:"integration,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *CreateIntegrationRequest) Reset() {
	*x = CreateIntegrationRequest{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[84]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *CreateIntegrationRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CreateIntegrationRequest) ProtoMessage() {}

func (x *CreateIntegrationRequest) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[84]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CreateIntegrationRequest.ProtoReflect.Descriptor instead.
func (*CreateIntegrationRequest) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{84}
}

func (x *CreateIntegrationRequest) GetParent() string {
	if x != nil {
		return x.Parent
	}
	return ""
}

func (x *CreateIntegrationRequest) GetIntegrationId() string {
	if x != nil {
		return x.IntegrationId
	}
	return ""
}

func (x *CreateIntegrationRequest) GetIntegration() *Integration {
	if x != nil {
		return x.Integration
	}
	return nil
}

type CreateIntegrationResponse struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// The created integration.
	Integration   *Integration `protobuf:"bytes,1,opt,name=integration,proto3" json:"integration,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *CreateIntegrationResponse) Reset() {
	*x = CreateIntegrationResponse{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[85]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *CreateIntegrationResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CreateIntegrationResponse) ProtoMessage() {}

func (x *CreateIntegrationResponse) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[85]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CreateIntegrationResponse.ProtoReflect.Descriptor instead.
func (*CreateIntegrationResponse) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{85}
}

func (x *CreateIntegrationResponse) GetIntegration() *Integration {
	if x != nil {
		return x.Integration
	}
	return nil
}

type BuildIntegrationRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Name of integration resource to build.
	Name string `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`
	// URI of package source to build integration from (e.g. git://repo_url or file://path/to/source).
	SourceUri string `protobuf:"bytes,2,opt,name=source_uri,json=sourceUri,proto3" json:"source_uri,omitempty"`
	// Package source checksum or git commit hash.
	SourceEtag string `protobuf:"bytes,3,opt,name=source_etag,json=sourceEtag,proto3" json:"source_etag,omitempty"`
	// Build metadata for this build operation.
	Build         *PackageBuildMetadata `protobuf:"bytes,4,opt,name=build,proto3" json:"build,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *BuildIntegrationRequest) Reset() {
	*x = BuildIntegrationRequest{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[86]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *BuildIntegrationRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*BuildIntegrationRequest) ProtoMessage() {}

func (x *BuildIntegrationRequest) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[86]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use BuildIntegrationRequest.ProtoReflect.Descriptor instead.
func (*BuildIntegrationRequest) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{86}
}

func (x *BuildIntegrationRequest) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *BuildIntegrationRequest) GetSourceUri() string {
	if x != nil {
		return x.SourceUri
	}
	return ""
}

func (x *BuildIntegrationRequest) GetSourceEtag() string {
	if x != nil {
		return x.SourceEtag
	}
	return ""
}

func (x *BuildIntegrationRequest) GetBuild() *PackageBuildMetadata {
	if x != nil {
		return x.Build
	}
	return nil
}

type BuildIntegrationResponse struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Integration resource of build.
	Integration *Integration `protobuf:"bytes,1,opt,name=integration,proto3" json:"integration,omitempty"`
	// Package build metadata (must include artifact etag for a successful build).
	Build         *PackageBuildMetadata `protobuf:"bytes,2,opt,name=build,proto3" json:"build,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *BuildIntegrationResponse) Reset() {
	*x = BuildIntegrationResponse{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[87]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *BuildIntegrationResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*BuildIntegrationResponse) ProtoMessage() {}

func (x *BuildIntegrationResponse) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[87]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use BuildIntegrationResponse.ProtoReflect.Descriptor instead.
func (*BuildIntegrationResponse) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{87}
}

func (x *BuildIntegrationResponse) GetIntegration() *Integration {
	if x != nil {
		return x.Integration
	}
	return nil
}

func (x *BuildIntegrationResponse) GetBuild() *PackageBuildMetadata {
	if x != nil {
		return x.Build
	}
	return nil
}

type UpdateIntegrationRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Integration with updated fields. Must include `name`.
	Integration *Integration `protobuf:"bytes,1,opt,name=integration,proto3" json:"integration,omitempty"`
	// Fields to update (e.g., "config,spec").
	UpdateMask    *fieldmaskpb.FieldMask `protobuf:"bytes,2,opt,name=update_mask,json=updateMask,proto3" json:"update_mask,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *UpdateIntegrationRequest) Reset() {
	*x = UpdateIntegrationRequest{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[88]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UpdateIntegrationRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UpdateIntegrationRequest) ProtoMessage() {}

func (x *UpdateIntegrationRequest) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[88]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UpdateIntegrationRequest.ProtoReflect.Descriptor instead.
func (*UpdateIntegrationRequest) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{88}
}

func (x *UpdateIntegrationRequest) GetIntegration() *Integration {
	if x != nil {
		return x.Integration
	}
	return nil
}

func (x *UpdateIntegrationRequest) GetUpdateMask() *fieldmaskpb.FieldMask {
	if x != nil {
		return x.UpdateMask
	}
	return nil
}

type UpdateIntegrationResponse struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// The updated integration.
	Integration   *Integration `protobuf:"bytes,1,opt,name=integration,proto3" json:"integration,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *UpdateIntegrationResponse) Reset() {
	*x = UpdateIntegrationResponse{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[89]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UpdateIntegrationResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UpdateIntegrationResponse) ProtoMessage() {}

func (x *UpdateIntegrationResponse) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[89]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UpdateIntegrationResponse.ProtoReflect.Descriptor instead.
func (*UpdateIntegrationResponse) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{89}
}

func (x *UpdateIntegrationResponse) GetIntegration() *Integration {
	if x != nil {
		return x.Integration
	}
	return nil
}

type DeleteIntegrationRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Name of integration to delete.
	Name          string `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *DeleteIntegrationRequest) Reset() {
	*x = DeleteIntegrationRequest{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[90]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *DeleteIntegrationRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*DeleteIntegrationRequest) ProtoMessage() {}

func (x *DeleteIntegrationRequest) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[90]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use DeleteIntegrationRequest.ProtoReflect.Descriptor instead.
func (*DeleteIntegrationRequest) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{90}
}

func (x *DeleteIntegrationRequest) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

type DeleteIntegrationResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *DeleteIntegrationResponse) Reset() {
	*x = DeleteIntegrationResponse{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[91]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *DeleteIntegrationResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*DeleteIntegrationResponse) ProtoMessage() {}

func (x *DeleteIntegrationResponse) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[91]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use DeleteIntegrationResponse.ProtoReflect.Descriptor instead.
func (*DeleteIntegrationResponse) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{91}
}

type GetTypeRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Type resource name, one of:
	// - System: "types/{type}"
	// - Organization/User: e.g. "organizations/bar/types/{type}" or "users/foo/types/{type}"
	// - Connection: e.g. "users/foo/connections/foobar/types/{type}"
	// - Deployment: e.g. "users/foo/deployments/default/types/{type}"
	Name          string `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetTypeRequest) Reset() {
	*x = GetTypeRequest{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[92]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetTypeRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetTypeRequest) ProtoMessage() {}

func (x *GetTypeRequest) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[92]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetTypeRequest.ProtoReflect.Descriptor instead.
func (*GetTypeRequest) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{92}
}

func (x *GetTypeRequest) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

type GetTypeResponse struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// The requested type.
	Type          *Type `protobuf:"bytes,1,opt,name=type,proto3" json:"type,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetTypeResponse) Reset() {
	*x = GetTypeResponse{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[93]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetTypeResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetTypeResponse) ProtoMessage() {}

func (x *GetTypeResponse) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[93]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetTypeResponse.ProtoReflect.Descriptor instead.
func (*GetTypeResponse) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{93}
}

func (x *GetTypeResponse) GetType() *Type {
	if x != nil {
		return x.Type
	}
	return nil
}

type ListTypesRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Maximum number of types to return.
	PageSize int32 `protobuf:"varint,1,opt,name=page_size,json=pageSize,proto3" json:"page_size,omitempty"`
	// Token for retrieving the next page.
	PageToken string `protobuf:"bytes,2,opt,name=page_token,json=pageToken,proto3" json:"page_token,omitempty"`
	// Parent resource name, one of:
	// - System (no parent): nil | ""
	// - Organization/User: e.g. "organizations/bar" or "users/foo"
	// - Connection: e.g. "users/foo/connections/foobar"
	// - Deployment: e.g. "users/foo/deployments/default"
	// - Integration (aggregates deployments): e.g. "users/foo/integrations/bar"
	Parent *string `protobuf:"bytes,3,opt,name=parent,proto3,oneof" json:"parent,omitempty"`
	// Filter expression.
	Filter        *string `protobuf:"bytes,4,opt,name=filter,proto3,oneof" json:"filter,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListTypesRequest) Reset() {
	*x = ListTypesRequest{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[94]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListTypesRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListTypesRequest) ProtoMessage() {}

func (x *ListTypesRequest) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[94]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListTypesRequest.ProtoReflect.Descriptor instead.
func (*ListTypesRequest) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{94}
}

func (x *ListTypesRequest) GetPageSize() int32 {
	if x != nil {
		return x.PageSize
	}
	return 0
}

func (x *ListTypesRequest) GetPageToken() string {
	if x != nil {
		return x.PageToken
	}
	return ""
}

func (x *ListTypesRequest) GetParent() string {
	if x != nil && x.Parent != nil {
		return *x.Parent
	}
	return ""
}

func (x *ListTypesRequest) GetFilter() string {
	if x != nil && x.Filter != nil {
		return *x.Filter
	}
	return ""
}

type ListTypesResponse struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// The list of types.
	Types []*Type `protobuf:"bytes,1,rep,name=types,proto3" json:"types,omitempty"`
	// Token for retrieving the next page.
	NextPageToken string `protobuf:"bytes,2,opt,name=next_page_token,json=nextPageToken,proto3" json:"next_page_token,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListTypesResponse) Reset() {
	*x = ListTypesResponse{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[95]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListTypesResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListTypesResponse) ProtoMessage() {}

func (x *ListTypesResponse) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[95]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListTypesResponse.ProtoReflect.Descriptor instead.
func (*ListTypesResponse) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{95}
}

func (x *ListTypesResponse) GetTypes() []*Type {
	if x != nil {
		return x.Types
	}
	return nil
}

func (x *ListTypesResponse) GetNextPageToken() string {
	if x != nil {
		return x.NextPageToken
	}
	return ""
}

type GetMethodRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Method resource name, one of:
	// - System: "methods/{method}"
	// - Organization/User: e.g. "organizations/bar/methods/{method}" or "users/foo/methods/{method}"
	// - Connection: e.g. "users/foo/connections/foobar/methods/{method}"
	// - Deployment: e.g. "users/foo/deployments/default/methods/{method}"
	Name          string `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetMethodRequest) Reset() {
	*x = GetMethodRequest{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[96]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetMethodRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetMethodRequest) ProtoMessage() {}

func (x *GetMethodRequest) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[96]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetMethodRequest.ProtoReflect.Descriptor instead.
func (*GetMethodRequest) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{96}
}

func (x *GetMethodRequest) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

type GetMethodResponse struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// The requested method.
	Method        *Method `protobuf:"bytes,1,opt,name=method,proto3" json:"method,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetMethodResponse) Reset() {
	*x = GetMethodResponse{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[97]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetMethodResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetMethodResponse) ProtoMessage() {}

func (x *GetMethodResponse) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[97]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetMethodResponse.ProtoReflect.Descriptor instead.
func (*GetMethodResponse) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{97}
}

func (x *GetMethodResponse) GetMethod() *Method {
	if x != nil {
		return x.Method
	}
	return nil
}

type GetServiceRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Service resource name, one of:
	// - System: "services/{service}"
	// - Organization/User: e.g. "organizations/bar/services/{service}" or "users/foo/services/{service}"
	// - Connection: e.g. "users/foo/connections/foobar/services/{service}"
	// - Deployment: e.g. "users/foo/deployments/default/services/{service}"
	Name          string `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetServiceRequest) Reset() {
	*x = GetServiceRequest{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[98]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetServiceRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetServiceRequest) ProtoMessage() {}

func (x *GetServiceRequest) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[98]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetServiceRequest.ProtoReflect.Descriptor instead.
func (*GetServiceRequest) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{98}
}

func (x *GetServiceRequest) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

type GetServiceResponse struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// The requested service.
	Service       *Service `protobuf:"bytes,1,opt,name=service,proto3" json:"service,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetServiceResponse) Reset() {
	*x = GetServiceResponse{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[99]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetServiceResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetServiceResponse) ProtoMessage() {}

func (x *GetServiceResponse) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[99]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetServiceResponse.ProtoReflect.Descriptor instead.
func (*GetServiceResponse) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{99}
}

func (x *GetServiceResponse) GetService() *Service {
	if x != nil {
		return x.Service
	}
	return nil
}

type ListServicesRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Maximum number of services to return.
	PageSize int32 `protobuf:"varint,1,opt,name=page_size,json=pageSize,proto3" json:"page_size,omitempty"`
	// Token for retrieving the next page.
	PageToken string `protobuf:"bytes,2,opt,name=page_token,json=pageToken,proto3" json:"page_token,omitempty"`
	// Parent resource name, one of (blank will return all):
	// - Return all: nil
	// - Organization/User: e.g. "organizations/bar" or "users/foo"
	// - Connection: e.g. "users/foo/connections/foobar"
	// - Deployment: e.g. "users/foo/deployments/default"
	// - Integration (aggregates deployments): e.g. "users/foo/integrations/bar"
	Parent *string `protobuf:"bytes,3,opt,name=parent,proto3,oneof" json:"parent,omitempty"`
	// Filter expression.
	Filter        *string `protobuf:"bytes,4,opt,name=filter,proto3,oneof" json:"filter,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListServicesRequest) Reset() {
	*x = ListServicesRequest{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[100]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListServicesRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListServicesRequest) ProtoMessage() {}

func (x *ListServicesRequest) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[100]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListServicesRequest.ProtoReflect.Descriptor instead.
func (*ListServicesRequest) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{100}
}

func (x *ListServicesRequest) GetPageSize() int32 {
	if x != nil {
		return x.PageSize
	}
	return 0
}

func (x *ListServicesRequest) GetPageToken() string {
	if x != nil {
		return x.PageToken
	}
	return ""
}

func (x *ListServicesRequest) GetParent() string {
	if x != nil && x.Parent != nil {
		return *x.Parent
	}
	return ""
}

func (x *ListServicesRequest) GetFilter() string {
	if x != nil && x.Filter != nil {
		return *x.Filter
	}
	return ""
}

type ListServicesResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Services      []*Service             `protobuf:"bytes,1,rep,name=services,proto3" json:"services,omitempty"`
	NextPageToken string                 `protobuf:"bytes,2,opt,name=next_page_token,json=nextPageToken,proto3" json:"next_page_token,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListServicesResponse) Reset() {
	*x = ListServicesResponse{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[101]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListServicesResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListServicesResponse) ProtoMessage() {}

func (x *ListServicesResponse) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[101]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListServicesResponse.ProtoReflect.Descriptor instead.
func (*ListServicesResponse) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{101}
}

func (x *ListServicesResponse) GetServices() []*Service {
	if x != nil {
		return x.Services
	}
	return nil
}

func (x *ListServicesResponse) GetNextPageToken() string {
	if x != nil {
		return x.NextPageToken
	}
	return ""
}

type ListMethodsRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Maximum number of methods to return.
	PageSize int32 `protobuf:"varint,1,opt,name=page_size,json=pageSize,proto3" json:"page_size,omitempty"`
	// Token for retrieving the next page.
	PageToken string `protobuf:"bytes,2,opt,name=page_token,json=pageToken,proto3" json:"page_token,omitempty"`
	// Parent resource name, one of (blank will return all):
	// - Return all: nil
	// - Organization/User: e.g. "organizations/bar" or "users/foo"
	// - Connection: e.g. "users/foo/connections/foobar"
	// - Deployment: e.g. "users/foo/deployments/default"
	// - Integration (aggregates deployments): e.g. "users/foo/integrations/bar"
	Parent *string `protobuf:"bytes,3,opt,name=parent,proto3,oneof" json:"parent,omitempty"`
	// Filter expression.
	Filter        *string `protobuf:"bytes,4,opt,name=filter,proto3,oneof" json:"filter,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListMethodsRequest) Reset() {
	*x = ListMethodsRequest{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[102]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListMethodsRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListMethodsRequest) ProtoMessage() {}

func (x *ListMethodsRequest) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[102]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListMethodsRequest.ProtoReflect.Descriptor instead.
func (*ListMethodsRequest) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{102}
}

func (x *ListMethodsRequest) GetPageSize() int32 {
	if x != nil {
		return x.PageSize
	}
	return 0
}

func (x *ListMethodsRequest) GetPageToken() string {
	if x != nil {
		return x.PageToken
	}
	return ""
}

func (x *ListMethodsRequest) GetParent() string {
	if x != nil && x.Parent != nil {
		return *x.Parent
	}
	return ""
}

func (x *ListMethodsRequest) GetFilter() string {
	if x != nil && x.Filter != nil {
		return *x.Filter
	}
	return ""
}

type ListMethodsResponse struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Methods       []*Method              `protobuf:"bytes,1,rep,name=methods,proto3" json:"methods,omitempty"`
	NextPageToken string                 `protobuf:"bytes,2,opt,name=next_page_token,json=nextPageToken,proto3" json:"next_page_token,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ListMethodsResponse) Reset() {
	*x = ListMethodsResponse{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[103]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ListMethodsResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListMethodsResponse) ProtoMessage() {}

func (x *ListMethodsResponse) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[103]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListMethodsResponse.ProtoReflect.Descriptor instead.
func (*ListMethodsResponse) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{103}
}

func (x *ListMethodsResponse) GetMethods() []*Method {
	if x != nil {
		return x.Methods
	}
	return nil
}

func (x *ListMethodsResponse) GetNextPageToken() string {
	if x != nil {
		return x.NextPageToken
	}
	return ""
}

type Deployment_Build struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Name of integration resource this build belongs to.
	Integration string `protobuf:"bytes,1,opt,name=integration,proto3" json:"integration,omitempty"`
	// Build metadata including artifact, platform, and runtime information.
	Metadata *PackageBuildMetadata `protobuf:"bytes,2,opt,name=metadata,proto3" json:"metadata,omitempty"`
	// Context of where the build was performed (optional if build performed
	// within the same control plane as deployment).
	Context       *Context `protobuf:"bytes,3,opt,name=context,proto3" json:"context,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *Deployment_Build) Reset() {
	*x = Deployment_Build{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[109]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *Deployment_Build) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Deployment_Build) ProtoMessage() {}

func (x *Deployment_Build) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[109]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Deployment_Build.ProtoReflect.Descriptor instead.
func (*Deployment_Build) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{7, 2}
}

func (x *Deployment_Build) GetIntegration() string {
	if x != nil {
		return x.Integration
	}
	return ""
}

func (x *Deployment_Build) GetMetadata() *PackageBuildMetadata {
	if x != nil {
		return x.Metadata
	}
	return nil
}

func (x *Deployment_Build) GetContext() *Context {
	if x != nil {
		return x.Context
	}
	return nil
}

type Deployment_Cloud struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Context       *Context               `protobuf:"bytes,1,opt,name=context,proto3" json:"context,omitempty"`
	Config        *EncryptedAny          `protobuf:"bytes,3,opt,name=config,proto3" json:"config,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *Deployment_Cloud) Reset() {
	*x = Deployment_Cloud{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[110]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *Deployment_Cloud) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Deployment_Cloud) ProtoMessage() {}

func (x *Deployment_Cloud) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[110]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Deployment_Cloud.ProtoReflect.Descriptor instead.
func (*Deployment_Cloud) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{7, 3}
}

func (x *Deployment_Cloud) GetContext() *Context {
	if x != nil {
		return x.Context
	}
	return nil
}

func (x *Deployment_Cloud) GetConfig() *EncryptedAny {
	if x != nil {
		return x.Config
	}
	return nil
}

type Deployment_RuntimeMetadata struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Types that are valid to be assigned to Type:
	//
	//	*Deployment_RuntimeMetadata_Native_
	//	*Deployment_RuntimeMetadata_Docker_
	Type          isDeployment_RuntimeMetadata_Type `protobuf_oneof:"type"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *Deployment_RuntimeMetadata) Reset() {
	*x = Deployment_RuntimeMetadata{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[111]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *Deployment_RuntimeMetadata) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Deployment_RuntimeMetadata) ProtoMessage() {}

func (x *Deployment_RuntimeMetadata) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[111]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Deployment_RuntimeMetadata.ProtoReflect.Descriptor instead.
func (*Deployment_RuntimeMetadata) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{7, 4}
}

func (x *Deployment_RuntimeMetadata) GetType() isDeployment_RuntimeMetadata_Type {
	if x != nil {
		return x.Type
	}
	return nil
}

func (x *Deployment_RuntimeMetadata) GetNative() *Deployment_RuntimeMetadata_Native {
	if x != nil {
		if x, ok := x.Type.(*Deployment_RuntimeMetadata_Native_); ok {
			return x.Native
		}
	}
	return nil
}

func (x *Deployment_RuntimeMetadata) GetDocker() *Deployment_RuntimeMetadata_Docker {
	if x != nil {
		if x, ok := x.Type.(*Deployment_RuntimeMetadata_Docker_); ok {
			return x.Docker
		}
	}
	return nil
}

type isDeployment_RuntimeMetadata_Type interface {
	isDeployment_RuntimeMetadata_Type()
}

type Deployment_RuntimeMetadata_Native_ struct {
	Native *Deployment_RuntimeMetadata_Native `protobuf:"bytes,1,opt,name=native,proto3,oneof"`
}

type Deployment_RuntimeMetadata_Docker_ struct {
	Docker *Deployment_RuntimeMetadata_Docker `protobuf:"bytes,2,opt,name=docker,proto3,oneof"`
}

func (*Deployment_RuntimeMetadata_Native_) isDeployment_RuntimeMetadata_Type() {}

func (*Deployment_RuntimeMetadata_Docker_) isDeployment_RuntimeMetadata_Type() {}

type Deployment_RuntimeMetadata_Native struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// PID of native process.
	ProcessId     int64 `protobuf:"varint,1,opt,name=process_id,json=processId,proto3" json:"process_id,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *Deployment_RuntimeMetadata_Native) Reset() {
	*x = Deployment_RuntimeMetadata_Native{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[112]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *Deployment_RuntimeMetadata_Native) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Deployment_RuntimeMetadata_Native) ProtoMessage() {}

func (x *Deployment_RuntimeMetadata_Native) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[112]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Deployment_RuntimeMetadata_Native.ProtoReflect.Descriptor instead.
func (*Deployment_RuntimeMetadata_Native) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{7, 4, 0}
}

func (x *Deployment_RuntimeMetadata_Native) GetProcessId() int64 {
	if x != nil {
		return x.ProcessId
	}
	return 0
}

type Deployment_RuntimeMetadata_Docker struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Container ID of docker container.
	ContainerId   string `protobuf:"bytes,1,opt,name=container_id,json=containerId,proto3" json:"container_id,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *Deployment_RuntimeMetadata_Docker) Reset() {
	*x = Deployment_RuntimeMetadata_Docker{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[113]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *Deployment_RuntimeMetadata_Docker) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Deployment_RuntimeMetadata_Docker) ProtoMessage() {}

func (x *Deployment_RuntimeMetadata_Docker) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[113]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Deployment_RuntimeMetadata_Docker.ProtoReflect.Descriptor instead.
func (*Deployment_RuntimeMetadata_Docker) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{7, 4, 1}
}

func (x *Deployment_RuntimeMetadata_Docker) GetContainerId() string {
	if x != nil {
		return x.ContainerId
	}
	return ""
}

type PackageBuildMetadata_Runtime struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Types that are valid to be assigned to Version:
	//
	//	*PackageBuildMetadata_Runtime_V1Beta1
	Version       isPackageBuildMetadata_Runtime_Version `protobuf_oneof:"version"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *PackageBuildMetadata_Runtime) Reset() {
	*x = PackageBuildMetadata_Runtime{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[114]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *PackageBuildMetadata_Runtime) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*PackageBuildMetadata_Runtime) ProtoMessage() {}

func (x *PackageBuildMetadata_Runtime) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[114]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use PackageBuildMetadata_Runtime.ProtoReflect.Descriptor instead.
func (*PackageBuildMetadata_Runtime) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{21, 0}
}

func (x *PackageBuildMetadata_Runtime) GetVersion() isPackageBuildMetadata_Runtime_Version {
	if x != nil {
		return x.Version
	}
	return nil
}

func (x *PackageBuildMetadata_Runtime) GetV1Beta1() v1beta1.Runtime {
	if x != nil {
		if x, ok := x.Version.(*PackageBuildMetadata_Runtime_V1Beta1); ok {
			return x.V1Beta1
		}
	}
	return v1beta1.Runtime(0)
}

type isPackageBuildMetadata_Runtime_Version interface {
	isPackageBuildMetadata_Runtime_Version()
}

type PackageBuildMetadata_Runtime_V1Beta1 struct {
	V1Beta1 v1beta1.Runtime `protobuf:"varint,1,opt,name=v1beta1,proto3,enum=dtkt.shared.v1beta1.Runtime,oneof"`
}

func (*PackageBuildMetadata_Runtime_V1Beta1) isPackageBuildMetadata_Runtime_Version() {}

type PackageBuildMetadata_Platform struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Types that are valid to be assigned to Version:
	//
	//	*PackageBuildMetadata_Platform_V1Beta1
	Version       isPackageBuildMetadata_Platform_Version `protobuf_oneof:"version"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *PackageBuildMetadata_Platform) Reset() {
	*x = PackageBuildMetadata_Platform{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[115]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *PackageBuildMetadata_Platform) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*PackageBuildMetadata_Platform) ProtoMessage() {}

func (x *PackageBuildMetadata_Platform) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[115]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use PackageBuildMetadata_Platform.ProtoReflect.Descriptor instead.
func (*PackageBuildMetadata_Platform) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{21, 1}
}

func (x *PackageBuildMetadata_Platform) GetVersion() isPackageBuildMetadata_Platform_Version {
	if x != nil {
		return x.Version
	}
	return nil
}

func (x *PackageBuildMetadata_Platform) GetV1Beta1() *v1beta1.Platform {
	if x != nil {
		if x, ok := x.Version.(*PackageBuildMetadata_Platform_V1Beta1); ok {
			return x.V1Beta1
		}
	}
	return nil
}

type isPackageBuildMetadata_Platform_Version interface {
	isPackageBuildMetadata_Platform_Version()
}

type PackageBuildMetadata_Platform_V1Beta1 struct {
	V1Beta1 *v1beta1.Platform `protobuf:"bytes,1,opt,name=v1beta1,proto3,oneof"`
}

func (*PackageBuildMetadata_Platform_V1Beta1) isPackageBuildMetadata_Platform_Version() {}

// InputsEvent provides initial input values for a flow.
type SendAutomationEvent_InputsEvent struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Map of flow input node id => any proto wrapper (e.g., "ping" => Any("pong")).
	Values        map[string]*anypb.Any `protobuf:"bytes,1,rep,name=values,proto3" json:"values,omitempty" protobuf_key:"bytes,1,opt,name=key" protobuf_val:"bytes,2,opt,name=value"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *SendAutomationEvent_InputsEvent) Reset() {
	*x = SendAutomationEvent_InputsEvent{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[118]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *SendAutomationEvent_InputsEvent) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*SendAutomationEvent_InputsEvent) ProtoMessage() {}

func (x *SendAutomationEvent_InputsEvent) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[118]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use SendAutomationEvent_InputsEvent.ProtoReflect.Descriptor instead.
func (*SendAutomationEvent_InputsEvent) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{40, 0}
}

func (x *SendAutomationEvent_InputsEvent) GetValues() map[string]*anypb.Any {
	if x != nil {
		return x.Values
	}
	return nil
}

// UserResponseEvent is received upon a fulfilled user elicitation.
type SendAutomationEvent_UserResponseEvent struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Flow action node id containing user action.
	Id string `protobuf:"bytes,1,opt,name=id,proto3" json:"id,omitempty"`
	// Map of user action input id => any proto wrapper (e.g., "ping" => Any("pong")).
	Values        map[string]*anypb.Any `protobuf:"bytes,2,rep,name=values,proto3" json:"values,omitempty" protobuf_key:"bytes,1,opt,name=key" protobuf_val:"bytes,2,opt,name=value"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *SendAutomationEvent_UserResponseEvent) Reset() {
	*x = SendAutomationEvent_UserResponseEvent{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[119]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *SendAutomationEvent_UserResponseEvent) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*SendAutomationEvent_UserResponseEvent) ProtoMessage() {}

func (x *SendAutomationEvent_UserResponseEvent) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[119]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use SendAutomationEvent_UserResponseEvent.ProtoReflect.Descriptor instead.
func (*SendAutomationEvent_UserResponseEvent) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{40, 1}
}

func (x *SendAutomationEvent_UserResponseEvent) GetId() string {
	if x != nil {
		return x.Id
	}
	return ""
}

func (x *SendAutomationEvent_UserResponseEvent) GetValues() map[string]*anypb.Any {
	if x != nil {
		return x.Values
	}
	return nil
}

// OutputsEvent provides output values from a flow.
type ReceiveAutomationEvent_OutputsEvent struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Map of output node id => any proto wrapper.
	Values        map[string]*anypb.Any `protobuf:"bytes,1,rep,name=values,proto3" json:"values,omitempty" protobuf_key:"bytes,1,opt,name=key" protobuf_val:"bytes,2,opt,name=value"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ReceiveAutomationEvent_OutputsEvent) Reset() {
	*x = ReceiveAutomationEvent_OutputsEvent{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[122]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ReceiveAutomationEvent_OutputsEvent) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ReceiveAutomationEvent_OutputsEvent) ProtoMessage() {}

func (x *ReceiveAutomationEvent_OutputsEvent) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[122]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ReceiveAutomationEvent_OutputsEvent.ProtoReflect.Descriptor instead.
func (*ReceiveAutomationEvent_OutputsEvent) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{41, 0}
}

func (x *ReceiveAutomationEvent_OutputsEvent) GetValues() map[string]*anypb.Any {
	if x != nil {
		return x.Values
	}
	return nil
}

// UserRequestEvent is emitted upon execution requiring elicitation from a user.
type ReceiveAutomationEvent_UserRequestEvent struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Flow action node id containing user action.
	Id string `protobuf:"bytes,1,opt,name=id,proto3" json:"id,omitempty"`
	// User action node which defines the data being requested.
	UserAction    *v1beta11.UserAction `protobuf:"bytes,2,opt,name=user_action,json=userAction,proto3" json:"user_action,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *ReceiveAutomationEvent_UserRequestEvent) Reset() {
	*x = ReceiveAutomationEvent_UserRequestEvent{}
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[123]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *ReceiveAutomationEvent_UserRequestEvent) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ReceiveAutomationEvent_UserRequestEvent) ProtoMessage() {}

func (x *ReceiveAutomationEvent_UserRequestEvent) ProtoReflect() protoreflect.Message {
	mi := &file_dtkt_core_v1_messages_proto_msgTypes[123]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ReceiveAutomationEvent_UserRequestEvent.ProtoReflect.Descriptor instead.
func (*ReceiveAutomationEvent_UserRequestEvent) Descriptor() ([]byte, []int) {
	return file_dtkt_core_v1_messages_proto_rawDescGZIP(), []int{41, 1}
}

func (x *ReceiveAutomationEvent_UserRequestEvent) GetId() string {
	if x != nil {
		return x.Id
	}
	return ""
}

func (x *ReceiveAutomationEvent_UserRequestEvent) GetUserAction() *v1beta11.UserAction {
	if x != nil {
		return x.UserAction
	}
	return nil
}

var File_dtkt_core_v1_messages_proto protoreflect.FileDescriptor

const file_dtkt_core_v1_messages_proto_rawDesc = "" +
	"\n" +
	"\x1bdtkt/core/v1/messages.proto\x12\fdtkt.core.v1\x1a\x1bbuf/validate/validate.proto\x1a\x1cdtkt/flow/v1beta1/eval.proto\x1a\x1cdtkt/flow/v1beta1/spec.proto\x1a&dtkt/protoform/v1beta1/protoform.proto\x1a\x1fdtkt/shared/v1beta1/enums.proto\x1a\"dtkt/shared/v1beta1/messages.proto\x1a\x1fgoogle/api/field_behavior.proto\x1a\x19google/api/resource.proto\x1a\x19google/protobuf/any.proto\x1a google/protobuf/descriptor.proto\x1a\x1egoogle/protobuf/duration.proto\x1a google/protobuf/field_mask.proto\x1a\x1fgoogle/protobuf/timestamp.proto\x1a\x17google/rpc/status.proto\"K\n" +
	"\aAddress\x12 \n" +
	"\anetwork\x18\x01 \x01(\tB\x06\xbaH\x03\xc8\x01\x01R\anetwork\x12\x1e\n" +
	"\x06target\x18\x02 \x01(\tB\x06\xbaH\x03\xc8\x01\x01R\x06target\"\x9a\x03\n" +
	"\x04Auth\x12=\n" +
	"\x06method\x18\x01 \x01(\x0e2\x1d.dtkt.core.v1.Auth.AuthMethodB\x06\xbaH\x03\xc8\x01\x01R\x06method\x12&\n" +
	"\faccess_token\x18\x02 \x01(\tB\x03\x80\x01\x01R\vaccessToken\x12(\n" +
	"\rrefresh_token\x18\x03 \x01(\tB\x03\x80\x01\x01R\frefreshToken\x129\n" +
	"\n" +
	"expires_at\x18\x04 \x01(\v2\x1a.google.protobuf.TimestampR\texpiresAt\x12\x1e\n" +
	"\x03org\x18\x05 \x01(\tB\a\xbaH\x04r\x02\x10\x01H\x00R\x03org\x88\x01\x01\x12\"\n" +
	"\x05space\x18\x06 \x01(\tB\a\xbaH\x04r\x02\x10\x01H\x01R\x05space\x88\x01\x01\"p\n" +
	"\n" +
	"AuthMethod\x12\x1b\n" +
	"\x17AUTH_METHOD_UNSPECIFIED\x10\x00\x12\x15\n" +
	"\x11AUTH_METHOD_OAUTH\x10\x01\x12\x16\n" +
	"\x12AUTH_METHOD_DEVICE\x10\x02\x12\x16\n" +
	"\x12AUTH_METHOD_BEARER\x10\x03B\x06\n" +
	"\x04_orgB\b\n" +
	"\x06_space\"\x86\x01\n" +
	"\aContext\x12\x1a\n" +
	"\x04name\x18\x01 \x01(\tB\x06\xbaH\x03\xc8\x01\x01R\x04name\x127\n" +
	"\aaddress\x18\x02 \x01(\v2\x15.dtkt.core.v1.AddressB\x06\xbaH\x03\xc8\x01\x01R\aaddress\x12&\n" +
	"\x04auth\x18\x03 \x01(\v2\x12.dtkt.core.v1.AuthR\x04auth\"\x9b\x02\n" +
	"\bResource\x127\n" +
	"\acontext\x18\x01 \x01(\v2\x15.dtkt.core.v1.ContextB\x06\xbaH\x03\xc8\x01\x01R\acontext\x12\x1a\n" +
	"\x04name\x18\x02 \x01(\tB\x06\xbaH\x03\xc8\x01\x01R\x04name\x12/\n" +
	"\aaddress\x18\x03 \x01(\v2\x15.dtkt.core.v1.AddressR\aaddress\x12?\n" +
	"\tencrypted\x18\n" +
	" \x01(\v2\x1a.dtkt.core.v1.EncryptedAnyB\x03\x80\x01\x01H\x00R\tencrypted\x129\n" +
	"\tdecrypted\x18\v \x01(\v2\x14.google.protobuf.AnyB\x03\x80\x01\x01H\x00R\tdecryptedB\r\n" +
	"\x04data\x12\x05\xbaH\x02\b\x01\"\xfb\b\n" +
	"\n" +
	"Automation\x12#\n" +
	"\x04name\x18\x01 \x01(\tB\x0f\xe0A\b\xbaH\x03\xc8\x01\x01\x82\xb5\x18\x02\x18\x01R\x04name\x12&\n" +
	"\x03uid\x18\x02 \x01(\tB\x14\xe0A\x03\xbaH\b\xc8\x01\x01r\x03\xb0\x01\x01\x82\xb5\x18\x02\x18\x01R\x03uid\x12\x9a\x01\n" +
	"\x04flow\x18\x03 \x01(\tB\x85\x01\xfaA\x06\n" +
	"\x04Flow\xbaH\x03\xc8\x01\x01\x82\xb5\x18r\n" +
	"\x04Flow\x12\x0eSelect a flow.:Z\n" +
	"\"dtkt.core.v1.FlowService.ListFlows\x12\x19this.flows.map(i, i.name)\x1a\x19this.flows.map(i, i.name)R\x04flow\x12,\n" +
	"\tspec_etag\x18\x04 \x01(\tB\x0f\xe0A\x04\xbaH\x03\xc8\x01\x01\x82\xb5\x18\x02\x18\x01R\bspecEtag\x12F\n" +
	"\aruntime\x18\x05 \x01(\v2!.dtkt.core.v1.FlowRuntimeMetadataB\t\xe0A\x03\xbaH\x03\xc8\x01\x01R\aruntime\x12C\n" +
	"\x06groups\x18\x06 \x01(\v2 .dtkt.core.v1.FlowGroupsMetadataB\t\xe0A\x03\xbaH\x03\xc8\x01\x01R\x06groups\x12K\n" +
	"\vconnections\x18\a \x03(\v2).dtkt.core.v1.Automation.ConnectionsEntryR\vconnections\x12<\n" +
	"\x06inputs\x18\b \x03(\v2$.dtkt.core.v1.Automation.InputsEntryR\x06inputs\x123\n" +
	"\atimeout\x18\t \x01(\v2\x19.google.protobuf.DurationR\atimeout\x12!\n" +
	"\x05batch\x18\n" +
	" \x01(\tB\x06\x82\xb5\x18\x02\x18\x01H\x00R\x05batch\x88\x01\x01\x12L\n" +
	"\vcreate_time\x18\x14 \x01(\v2\x1a.google.protobuf.TimestampB\x0f\xe0A\x03\xbaH\x03\xc8\x01\x01\x82\xb5\x18\x02\x18\x01R\n" +
	"createTime\x12L\n" +
	"\vupdate_time\x18\x15 \x01(\v2\x1a.google.protobuf.TimestampB\x0f\xe0A\x03\xbaH\x03\xc8\x01\x01\x82\xb5\x18\x02\x18\x01R\n" +
	"updateTime\x1ad\n" +
	"\x10ConnectionsEntry\x12\x10\n" +
	"\x03key\x18\x01 \x01(\tR\x03key\x12:\n" +
	"\x05value\x18\x02 \x01(\v2$.dtkt.core.v1.FlowConnectionMetadataR\x05value:\x028\x01\x1aO\n" +
	"\vInputsEntry\x12\x10\n" +
	"\x03key\x18\x01 \x01(\tR\x03key\x12*\n" +
	"\x05value\x18\x02 \x01(\v2\x14.google.protobuf.AnyR\x05value:\x028\x01:\x87\x01\xeaA\x83\x01\n" +
	"\n" +
	"Automation\x125organizations/{organization}/automations/{automation}\x12%users/{user}/automations/{automation}*\vautomations2\n" +
	"automationB\b\n" +
	"\x06_batch\"\x92\x04\n" +
	"\x04Flow\x12#\n" +
	"\x04name\x18\x01 \x01(\tB\x0f\xe0A\b\xbaH\x03\xc8\x01\x01\x82\xb5\x18\x02\x18\x01R\x04name\x12&\n" +
	"\x03uid\x18\x02 \x01(\tB\x14\xe0A\x03\xbaH\b\xc8\x01\x01r\x03\xb0\x01\x01\x82\xb5\x18\x02\x18\x01R\x03uid\x12&\n" +
	"\bspec_uri\x18\x03 \x01(\tB\v\xbaH\b\xd8\x01\x01r\x03\x88\x01\x01R\aspecUri\x12#\n" +
	"\tspec_etag\x18\x04 \x01(\tB\x06\xbaH\x03\xd8\x01\x01R\bspecEtag\x12:\n" +
	"\x04spec\x18\x05 \x01(\v2\x1e.dtkt.core.v1.FlowSpecMetadataB\x06\xbaH\x03\xc8\x01\x01R\x04spec\x12F\n" +
	"\x05graph\x18\x06 \x01(\v2\x1f.dtkt.core.v1.FlowGraphMetadataB\x0f\xe0A\x03\xbaH\x03\xc8\x01\x01\x82\xb5\x18\x02\x18\x01R\x05graph\x12F\n" +
	"\vcreate_time\x18\x14 \x01(\v2\x1a.google.protobuf.TimestampB\t\xe0A\x03\xbaH\x03\xc8\x01\x01R\n" +
	"createTime\x12F\n" +
	"\vupdate_time\x18\x15 \x01(\v2\x1a.google.protobuf.TimestampB\t\xe0A\x03\xbaH\x03\xc8\x01\x01R\n" +
	"updateTime:\\\xeaAY\n" +
	"\x04Flow\x12)organizations/{organization}/flows/{flow}\x12\x19users/{user}/flows/{flow}*\x05flows2\x04flow\"\xb4\r\n" +
	"\n" +
	"Connection\x12#\n" +
	"\x04name\x18\x01 \x01(\tB\x0f\xe0A\b\xbaH\x03\xc8\x01\x01\x82\xb5\x18\x02\x18\x01R\x04name\x12&\n" +
	"\x03uid\x18\x02 \x01(\tB\x14\xe0A\x03\xbaH\b\xc8\x01\x01r\x03\xb0\x01\x01\x82\xb5\x18\x02\x18\x01R\x03uid\x12\xcd\x01\n" +
	"\n" +
	"deployment\x18\x03 \x01(\tB\xaa\x01\xfaA\f\n" +
	"\n" +
	"Deployment\x82\xb5\x18\x96\x01\n" +
	"\n" +
	"Deployment\x12\x14Select a deployment.:r\n" +
	".dtkt.core.v1.DeploymentService.ListDeployments\x12\x1fthis.deployments.map(i, i.name)\x1a\x1fthis.deployments.map(i, i.name)H\x00R\n" +
	"deployment\x12\x9f\x01\n" +
	"\rcustom_protos\x18\x04 \x01(\tBx\xbaH\x05r\x03\x88\x01\x01\x82\xb5\x18l\n" +
	"\x18Custom Proto Descriptors\x12NCustom connection to a server using a file path or URI to .proto source files.*\x00H\x00R\fcustomProtos\x12\x9f\x01\n" +
	"\vcustom_grpc\x18\x05 \x01(\bB|\x82\xb5\x18x\n" +
	"\x16Custom gRPC Reflection\x12;Custom connection to a server with gRPC reflection service.\"!\n" +
	"\x03Yes\x12\x18No/Unsure (check for me)\x18\x01H\x00R\n" +
	"customGrpc\x12/\n" +
	"\aaddress\x18\x06 \x01(\v2\x15.dtkt.core.v1.AddressR\aaddress\x12I\n" +
	"\x10decrypted_config\x18\a \x01(\v2\x14.google.protobuf.AnyB\x06\xe0A\x04\x80\x01\x01H\x01R\x0fdecryptedConfig\x12R\n" +
	"\x10encrypted_config\x18\b \x01(\v2\x1a.dtkt.core.v1.EncryptedAnyB\t\xe0A\x03\x82\xb5\x18\x02\x18\x01H\x01R\x0fencryptedConfig\x12-\n" +
	"\vconfig_hash\x18\v \x01(\tB\f\xe0A\x03\x82\xb5\x18\x02\x18\x01\x80\x01\x01R\n" +
	"configHash\x12+\n" +
	"\n" +
	"config_gen\x18\f \x01(\x04B\f\xe0A\x03\x82\xb5\x18\x02\x18\x01\x80\x01\x01R\tconfigGen\x12O\n" +
	"\n" +
	"config_uri\x18\r \x01(\tB0\xe0A\x04\xbaH\b\xd8\x01\x01r\x03\x88\x01\x01\x82\xb5\x18\x1e\x12\x1aConfig data URI (optional)2\x00R\tconfigUri\x12D\n" +
	"\aheaders\x18\x0e \x03(\v2%.dtkt.core.v1.Connection.HeadersEntryB\x03\x80\x01\x01R\aheaders\x12L\n" +
	"\vcreate_time\x18\x14 \x01(\v2\x1a.google.protobuf.TimestampB\x0f\xe0A\x03\xbaH\x03\xc8\x01\x01\x82\xb5\x18\x02\x18\x01R\n" +
	"createTime\x12L\n" +
	"\vupdate_time\x18\x15 \x01(\v2\x1a.google.protobuf.TimestampB\x0f\xe0A\x03\xbaH\x03\xc8\x01\x01\x82\xb5\x18\x02\x18\x01R\n" +
	"updateTime\x1a:\n" +
	"\fHeadersEntry\x12\x10\n" +
	"\x03key\x18\x01 \x01(\tR\x03key\x12\x14\n" +
	"\x05value\x18\x02 \x01(\tR\x05value:\x028\x01:\x90\x03\xeaA\x83\x01\n" +
	"\n" +
	"Connection\x125organizations/{organization}/connections/{connection}\x12%users/{user}/connections/{connection}*\vconnections2\n" +
	"connection\xbaH\x85\x02*](has(this.custom_protos) || has(this.custom_grpc)) ? has(this.address) : has(this.deployment)*Vhas(this.deployment) ? has(this.decrypted_config) || has(this.encrypted_config) : true*Lhas(this.headers) ? has(this.custom_protos) || has(this.custom_grpc) : falseB\r\n" +
	"\x04type\x12\x05\xbaH\x02\b\x01B\b\n" +
	"\x06config\"\xc7\x0f\n" +
	"\n" +
	"Deployment\x12#\n" +
	"\x04name\x18\x01 \x01(\tB\x0f\xe0A\b\xbaH\x03\xc8\x01\x01\x82\xb5\x18\x02\x18\x01R\x04name\x12&\n" +
	"\x03uid\x18\x02 \x01(\tB\x14\xe0A\x03\xbaH\b\xc8\x01\x01r\x03\xb0\x01\x01\x82\xb5\x18\x02\x18\x01R\x03uid\x12D\n" +
	"\x05state\x18\x03 \x01(\x0e2\x1e.dtkt.core.v1.Deployment.StateB\x0e\xe0A\x03\xbaH\b\xc8\x01\x01\x82\x01\x02\x10\x01R\x05state\x127\n" +
	"\aaddress\x18\x04 \x01(\v2\x15.dtkt.core.v1.AddressB\x06\xbaH\x03\xc8\x01\x01R\aaddress\x128\n" +
	"\x03env\x18\x05 \x03(\v2!.dtkt.core.v1.Deployment.EnvEntryB\x03\x80\x01\x01R\x03env\x129\n" +
	"\x05ports\x18\x06 \x03(\v2#.dtkt.core.v1.Deployment.PortsEntryR\x05ports\x12<\n" +
	"\x05build\x18\a \x01(\v2\x1e.dtkt.core.v1.Deployment.BuildB\x06\xbaH\x03\xc8\x01\x01R\x05build\x124\n" +
	"\x05cloud\x18\b \x01(\v2\x1e.dtkt.core.v1.Deployment.CloudR\x05cloud\x12H\n" +
	"\rconfig_schema\x18\t \x01(\v2\x18.dtkt.core.v1.TypeSchemaB\t\xe0A\x03\xbaH\x03\xc8\x01\x01R\fconfigSchema\x12O\n" +
	"\fpackage_spec\x18\n" +
	" \x01(\v2!.dtkt.core.v1.PackageSpecMetadataB\t\xe0A\x03\xbaH\x03\xc8\x01\x01R\vpackageSpec\x12G\n" +
	"\aruntime\x18\v \x01(\v2(.dtkt.core.v1.Deployment.RuntimeMetadataB\x03\xe0A\x03R\aruntime\x12&\n" +
	"\x04etag\x18\x12 \x01(\tB\x12\xe0A\x03\xbaH\x03\xc8\x01\x01\x82\xb5\x18\x02\x18\x01\x80\x01\x01R\x04etag\x12F\n" +
	"\vcreate_time\x18\x14 \x01(\v2\x1a.google.protobuf.TimestampB\t\xe0A\x03\xbaH\x03\xc8\x01\x01R\n" +
	"createTime\x12F\n" +
	"\vupdate_time\x18\x15 \x01(\v2\x1a.google.protobuf.TimestampB\t\xe0A\x03\xbaH\x03\xc8\x01\x01R\n" +
	"updateTime\x1a6\n" +
	"\bEnvEntry\x12\x10\n" +
	"\x03key\x18\x01 \x01(\tR\x03key\x12\x14\n" +
	"\x05value\x18\x02 \x01(\tR\x05value:\x028\x01\x1a8\n" +
	"\n" +
	"PortsEntry\x12\x10\n" +
	"\x03key\x18\x01 \x01(\tR\x03key\x12\x14\n" +
	"\x05value\x18\x02 \x01(\tR\x05value:\x028\x01\x1a\x86\x02\n" +
	"\x05Build\x128\n" +
	"\vintegration\x18\x01 \x01(\tB\x16\xfaA\r\n" +
	"\vIntegration\xbaH\x03\xc8\x01\x01R\vintegration\x12F\n" +
	"\bmetadata\x18\x02 \x01(\v2\".dtkt.core.v1.PackageBuildMetadataB\x06\xbaH\x03\xc8\x01\x01R\bmetadata\x12/\n" +
	"\acontext\x18\x03 \x01(\v2\x15.dtkt.core.v1.ContextR\acontext:J\xbaHG*Ehas(this.metadata.artifact_etag) && this.metadata.artifact_etag != ''\x1a\xe2\x01\n" +
	"\x05Cloud\x12@\n" +
	"\acontext\x18\x01 \x01(\v2\x15.dtkt.core.v1.ContextB\x0f\xe0A\x04\xbaH\x03\xc8\x01\x01\x82\xb5\x18\x02\x18\x01R\acontext\x12@\n" +
	"\x06config\x18\x03 \x01(\v2\x1a.dtkt.core.v1.EncryptedAnyB\f\xe0A\x03\xbaH\x03\xc8\x01\x01\x80\x01\x01R\x06config:U\xbaHR*Phas(this.config) && this.config.type_url.endsWith('/google.protobuf.BytesValue')\x1a\x9c\x02\n" +
	"\x0fRuntimeMetadata\x12I\n" +
	"\x06native\x18\x01 \x01(\v2/.dtkt.core.v1.Deployment.RuntimeMetadata.NativeH\x00R\x06native\x12I\n" +
	"\x06docker\x18\x02 \x01(\v2/.dtkt.core.v1.Deployment.RuntimeMetadata.DockerH\x00R\x06docker\x1a/\n" +
	"\x06Native\x12%\n" +
	"\n" +
	"process_id\x18\x01 \x01(\x03B\x06\xbaH\x03\xc8\x01\x01R\tprocessId\x1a3\n" +
	"\x06Docker\x12)\n" +
	"\fcontainer_id\x18\x01 \x01(\tB\x06\xbaH\x03\xc8\x01\x01R\vcontainerIdB\r\n" +
	"\x04type\x12\x05\xbaH\x02\b\x01\"_\n" +
	"\x05State\x12\x15\n" +
	"\x11STATE_UNSPECIFIED\x10\x00\x12\v\n" +
	"\aPENDING\x10\x01\x12\v\n" +
	"\aRUNNING\x10\x02\x12\f\n" +
	"\bSTOPPING\x10\x03\x12\v\n" +
	"\aSTOPPED\x10\x04\x12\n" +
	"\n" +
	"\x06FAILED\x10\x05:\x87\x01\xeaA\x83\x01\n" +
	"\n" +
	"Deployment\x125organizations/{organization}/deployments/{deployment}\x12%users/{user}/deployments/{deployment}*\vdeployments2\n" +
	"deployment\"\xd5\x04\n" +
	"\vIntegration\x12#\n" +
	"\x04name\x18\x01 \x01(\tB\x0f\xe0A\b\xbaH\x03\xc8\x01\x01\x82\xb5\x18\x02\x18\x01R\x04name\x12&\n" +
	"\x03uid\x18\x02 \x01(\tB\x14\xe0A\x03\xbaH\b\xc8\x01\x01r\x03\xb0\x01\x01\x82\xb5\x18\x02\x18\x01R\x03uid\x12=\n" +
	"\x04spec\x18\x03 \x01(\v2!.dtkt.core.v1.PackageSpecMetadataB\x06\xbaH\x03\xc8\x01\x01R\x04spec\x12&\n" +
	"\tspec_etag\x18\x04 \x01(\tB\t\xe0A\x03\xbaH\x03\xc8\x01\x01R\bspecEtag\x12,\n" +
	"\n" +
	"source_uri\x18\x05 \x01(\tB\b\xbaH\x05r\x03\x88\x01\x01H\x00R\tsourceUri\x88\x01\x01\x12$\n" +
	"\vsource_etag\x18\x06 \x01(\tH\x01R\n" +
	"sourceEtag\x88\x01\x01\x12F\n" +
	"\vcreate_time\x18\x14 \x01(\v2\x1a.google.protobuf.TimestampB\t\xe0A\x03\xbaH\x03\xc8\x01\x01R\n" +
	"createTime\x12F\n" +
	"\vupdate_time\x18\x15 \x01(\v2\x1a.google.protobuf.TimestampB\t\xe0A\x03\xbaH\x03\xc8\x01\x01R\n" +
	"updateTime:\x8e\x01\xeaA\x8a\x01\n" +
	"\vIntegration\x127organizations/{organization}/integrations/{integration}\x12'users/{user}/integrations/{integration}*\fintegrations2\vintegrationB\r\n" +
	"\v_source_uriB\x0e\n" +
	"\f_source_etag\"\xff\x04\n" +
	"\x04File\x12#\n" +
	"\x04name\x18\x01 \x01(\tB\x0f\xe0A\b\xbaH\x03\xc8\x01\x01\x82\xb5\x18\x02\x18\x01R\x04name\x12&\n" +
	"\x03uid\x18\x02 \x01(\tB\x14\xe0A\x03\xbaH\b\xc8\x01\x01r\x03\xb0\x01\x01\x82\xb5\x18\x02\x18\x01R\x03uid\x12\x1a\n" +
	"\x04etag\x18\x03 \x01(\tB\x06\xbaH\x03\xc8\x01\x01R\x04etag\x12\x1f\n" +
	"\x03uri\x18\x04 \x01(\tB\b\xbaH\x05r\x03\x88\x01\x01H\x01R\x03uri\x88\x01\x01\x12<\n" +
	"\x06protos\x18\n" +
	" \x01(\v2\".google.protobuf.FileDescriptorSetH\x00R\x06protos\x12F\n" +
	"\vcreate_time\x18\x14 \x01(\v2\x1a.google.protobuf.TimestampB\t\xe0A\x03\xbaH\x03\xc8\x01\x01R\n" +
	"createTime\x12F\n" +
	"\vupdate_time\x18\x15 \x01(\v2\x1a.google.protobuf.TimestampB\t\xe0A\x03\xbaH\x03\xc8\x01\x01R\n" +
	"updateTime:\x87\x02\xeaA\x83\x02\n" +
	"\x04File\x12Borganizations/{organization}/connections/{connection}/files/{file}\x122users/{user}/connections/{connection}/files/{file}\x12Borganizations/{organization}/deployments/{deployment}/files/{file}\x122users/{user}/deployments/{deployment}/files/{file}*\x05files2\x04fileB\r\n" +
	"\x04type\x12\x05\xbaH\x02\b\x01B\x06\n" +
	"\x04_uri\"\xb5\x04\n" +
	"\aService\x12\x1a\n" +
	"\x04name\x18\x01 \x01(\tB\x06\xbaH\x03\xc8\x01\x01R\x04name\x12\x1d\n" +
	"\x03uid\x18\n" +
	" \x01(\tB\v\xbaH\b\xc8\x01\x01r\x03\xb0\x01\x01R\x03uid\x12 \n" +
	"\vdescription\x18\x02 \x01(\tR\vdescription\x12F\n" +
	"\vcreate_time\x18\x14 \x01(\v2\x1a.google.protobuf.TimestampB\t\xe0A\x03\xbaH\x03\xc8\x01\x01R\n" +
	"createTime\x12F\n" +
	"\vupdate_time\x18\x15 \x01(\v2\x1a.google.protobuf.TimestampB\t\xe0A\x03\xbaH\x03\xc8\x01\x01R\n" +
	"updateTime:\xbc\x02\xeaA\xb8\x02\n" +
	"\aService\x12\x12services/{service}\x12Horganizations/{organization}/connections/{connection}/services/{service}\x128users/{user}/connections/{connection}/services/{service}\x12Horganizations/{organization}/deployments/{deployment}/services/{service}\x128users/{user}/deployments/{deployment}/services/{service}*\bservices2\aservice\"\xc9\x05\n" +
	"\x06Method\x12\x1a\n" +
	"\x04name\x18\x01 \x01(\tB\x06\xbaH\x03\xc8\x01\x01R\x04name\x12\x1d\n" +
	"\x03uid\x18\n" +
	" \x01(\tB\v\xbaH\b\xc8\x01\x01r\x03\xb0\x01\x01R\x03uid\x12 \n" +
	"\vdescription\x18\x02 \x01(\tR\vdescription\x12 \n" +
	"\arequest\x18\x03 \x01(\tB\x06\xbaH\x03\xc8\x01\x01R\arequest\x12\"\n" +
	"\bresponse\x18\x04 \x01(\tB\x06\xbaH\x03\xc8\x01\x01R\bresponse\x12+\n" +
	"\x11request_streaming\x18\x05 \x01(\bR\x10requestStreaming\x12-\n" +
	"\x12response_streaming\x18\x06 \x01(\bR\x11responseStreaming\x12F\n" +
	"\vcreate_time\x18\x14 \x01(\v2\x1a.google.protobuf.TimestampB\t\xe0A\x03\xbaH\x03\xc8\x01\x01R\n" +
	"createTime\x12F\n" +
	"\vupdate_time\x18\x15 \x01(\v2\x1a.google.protobuf.TimestampB\t\xe0A\x03\xbaH\x03\xc8\x01\x01R\n" +
	"updateTime:\xaf\x02\xeaA\xab\x02\n" +
	"\x06Method\x12\x10methods/{method}\x12Forganizations/{organization}/connections/{connection}/methods/{method}\x126users/{user}/connections/{connection}/methods/{method}\x12Forganizations/{organization}/deployments/{deployment}/methods/{method}\x126users/{user}/deployments/{deployment}/methods/{method}*\amethods2\x06method\"\xa3\x04\n" +
	"\x04Type\x12\x1a\n" +
	"\x04name\x18\x01 \x01(\tB\x06\xbaH\x03\xc8\x01\x01R\x04name\x12\x1d\n" +
	"\x03uid\x18\x05 \x01(\tB\v\xbaH\b\xc8\x01\x01r\x03\xb0\x01\x01R\x03uid\x128\n" +
	"\x06schema\x18\x02 \x01(\v2\x18.dtkt.core.v1.TypeSchemaB\x06\xbaH\x03\xc8\x01\x01R\x06schema\x12F\n" +
	"\vcreate_time\x18\x14 \x01(\v2\x1a.google.protobuf.TimestampB\t\xe0A\x03\xbaH\x03\xc8\x01\x01R\n" +
	"createTime\x12F\n" +
	"\vupdate_time\x18\x15 \x01(\v2\x1a.google.protobuf.TimestampB\t\xe0A\x03\xbaH\x03\xc8\x01\x01R\n" +
	"updateTime:\x95\x02\xeaA\x91\x02\n" +
	"\x04Type\x12\ftypes/{type}\x12Borganizations/{organization}/connections/{connection}/types/{type}\x122users/{user}/connections/{connection}/types/{type}\x12Borganizations/{organization}/deployments/{deployment}/types/{type}\x122users/{user}/deployments/{deployment}/types/{type}*\x05types2\x04type\"[\n" +
	"\n" +
	"TypeSchema\x12;\n" +
	"\av1beta1\x18\x01 \x01(\v2\x1f.dtkt.shared.v1beta1.TypeSchemaH\x00R\av1beta1B\x10\n" +
	"\aversion\x12\x05\xbaH\x02\b\x01\"\xa2\x01\n" +
	"\x12DeploymentMetadata\x12\x18\n" +
	"\amessage\x18\n" +
	" \x01(\tR\amessage\x129\n" +
	"\n" +
	"start_time\x18\x14 \x01(\v2\x1a.google.protobuf.TimestampR\tstartTime\x127\n" +
	"\tstop_time\x18\x15 \x01(\v2\x1a.google.protobuf.TimestampR\bstopTime\"\x93\x02\n" +
	"\x16FlowConnectionMetadata\x12\xc7\x01\n" +
	"\x04name\x18\x01 \x01(\tB\xb2\x01\xbaH\x14\xc8\x01\x01r\x0fJ\r/connections/\x82\xb5\x18\x96\x01\n" +
	"\n" +
	"Connection\x12\x14Select a connection.:r\n" +
	".dtkt.core.v1.ConnectionService.ListConnections\x12\x1fthis.connections.map(i, i.name)\x1a\x1fthis.connections.map(i, i.name)R\x04name\x12/\n" +
	"\acontext\x18\x02 \x01(\v2\x15.dtkt.core.v1.ContextR\acontext\"_\n" +
	"\x13FlowRuntimeMetadata\x126\n" +
	"\av1beta1\x18\x01 \x01(\v2\x1a.dtkt.flow.v1beta1.RuntimeH\x00R\av1beta1B\x10\n" +
	"\aversion\x12\x05\xbaH\x02\b\x01\"]\n" +
	"\x12FlowGroupsMetadata\x125\n" +
	"\av1beta1\x18\x01 \x01(\v2\x19.dtkt.flow.v1beta1.GroupsH\x00R\av1beta1B\x10\n" +
	"\aversion\x12\x05\xbaH\x02\b\x01\"Y\n" +
	"\x10FlowSpecMetadata\x123\n" +
	"\av1beta1\x18\x01 \x01(\v2\x17.dtkt.flow.v1beta1.FlowH\x00R\av1beta1B\x10\n" +
	"\aversion\x12\x05\xbaH\x02\b\x01\"[\n" +
	"\x11FlowGraphMetadata\x124\n" +
	"\av1beta1\x18\x01 \x01(\v2\x18.dtkt.flow.v1beta1.GraphH\x00R\av1beta1B\x10\n" +
	"\aversion\x12\x05\xbaH\x02\b\x01\"a\n" +
	"\x13PackageSpecMetadata\x128\n" +
	"\av1beta1\x18\x01 \x01(\v2\x1c.dtkt.shared.v1beta1.PackageH\x00R\av1beta1B\x10\n" +
	"\aversion\x12\x05\xbaH\x02\b\x01\"\xc4\x04\n" +
	"\x14PackageBuildMetadata\x12L\n" +
	"\aruntime\x18\x01 \x01(\v2*.dtkt.core.v1.PackageBuildMetadata.RuntimeB\x06\xbaH\x03\xc8\x01\x01R\aruntime\x12O\n" +
	"\bplatform\x18\x02 \x01(\v2+.dtkt.core.v1.PackageBuildMetadata.PlatformB\x06\xbaH\x03\xc8\x01\x01R\bplatform\x12B\n" +
	"\x03env\x18\x03 \x03(\v2+.dtkt.core.v1.PackageBuildMetadata.EnvEntryB\x03\x80\x01\x01R\x03env\x121\n" +
	"\fartifact_uri\x18\x04 \x01(\tB\x0e\xe0A\x03\xbaH\b\xc8\x01\x01r\x03\x88\x01\x01R\vartifactUri\x12.\n" +
	"\rartifact_etag\x18\x05 \x01(\tB\t\xe0A\x03\xbaH\x03\xc8\x01\x01R\fartifactEtag\x1aU\n" +
	"\aRuntime\x128\n" +
	"\av1beta1\x18\x01 \x01(\x0e2\x1c.dtkt.shared.v1beta1.RuntimeH\x00R\av1beta1B\x10\n" +
	"\aversion\x12\x05\xbaH\x02\b\x01\x1aW\n" +
	"\bPlatform\x129\n" +
	"\av1beta1\x18\x01 \x01(\v2\x1d.dtkt.shared.v1beta1.PlatformH\x00R\av1beta1B\x10\n" +
	"\aversion\x12\x05\xbaH\x02\b\x01\x1a6\n" +
	"\bEnvEntry\x12\x10\n" +
	"\x03key\x18\x01 \x01(\tR\x03key\x12\x14\n" +
	"\x05value\x18\x02 \x01(\tR\x05value:\x028\x01\"\xe1\x02\n" +
	"\x16BuildOperationMetadata\x12H\n" +
	"\x05state\x18\x01 \x01(\x0e2*.dtkt.core.v1.BuildOperationMetadata.StateB\x06\xbaH\x03\xc8\x01\x01R\x05state\x12\x18\n" +
	"\amessage\x18\x02 \x01(\tR\amessage\x129\n" +
	"\n" +
	"start_time\x18\x14 \x01(\v2\x1a.google.protobuf.TimestampR\tstartTime\x12;\n" +
	"\vfinish_time\x18\x15 \x01(\v2\x1a.google.protobuf.TimestampR\n" +
	"finishTime\"k\n" +
	"\x05State\x12\x15\n" +
	"\x11STATE_UNSPECIFIED\x10\x00\x12\x11\n" +
	"\rSTATE_PENDING\x10\x01\x12\x11\n" +
	"\rSTATE_RUNNING\x10\x02\x12\x13\n" +
	"\x0fSTATE_SUCCEEDED\x10\x03\x12\x10\n" +
	"\fSTATE_FAILED\x10\x04\"\xd7\x02\n" +
	"\x14RunOperationMetadata\x12F\n" +
	"\x05state\x18\x01 \x01(\x0e2(.dtkt.core.v1.RunOperationMetadata.StateB\x06\xbaH\x03\xc8\x01\x01R\x05state\x12\x18\n" +
	"\amessage\x18\x02 \x01(\tR\amessage\x129\n" +
	"\n" +
	"start_time\x18\x14 \x01(\v2\x1a.google.protobuf.TimestampR\tstartTime\x127\n" +
	"\tstop_time\x18\x15 \x01(\v2\x1a.google.protobuf.TimestampR\bstopTime\"i\n" +
	"\x05State\x12\x15\n" +
	"\x11STATE_UNSPECIFIED\x10\x00\x12\x11\n" +
	"\rSTATE_PENDING\x10\x01\x12\x11\n" +
	"\rSTATE_RUNNING\x10\x02\x12\x11\n" +
	"\rSTATE_STOPPED\x10\x03\x12\x10\n" +
	"\fSTATE_FAILED\x10\x04\"\xc8\x03\n" +
	"\x15SyncOperationMetadata\x12G\n" +
	"\x05state\x18\x01 \x01(\x0e2).dtkt.core.v1.SyncOperationMetadata.StateB\x06\xbaH\x03\xc8\x01\x01R\x05state\x12\x18\n" +
	"\amessage\x18\x02 \x01(\tR\amessage\x12%\n" +
	"\x0eservices_count\x18\t \x01(\x03R\rservicesCount\x12#\n" +
	"\rmethods_count\x18\n" +
	" \x01(\x03R\fmethodsCount\x12\x1f\n" +
	"\vtypes_count\x18\v \x01(\x03R\n" +
	"typesCount\x129\n" +
	"\n" +
	"start_time\x18\x14 \x01(\v2\x1a.google.protobuf.TimestampR\tstartTime\x127\n" +
	"\tstop_time\x18\x15 \x01(\v2\x1a.google.protobuf.TimestampR\bstopTime\"k\n" +
	"\x05State\x12\x15\n" +
	"\x11STATE_UNSPECIFIED\x10\x00\x12\x11\n" +
	"\rSTATE_PENDING\x10\x01\x12\x11\n" +
	"\rSTATE_RUNNING\x10\x02\x12\x13\n" +
	"\x0fSTATE_SUCCEEDED\x10\x03\x12\x10\n" +
	"\fSTATE_FAILED\x10\x04\"\xd7\x01\n" +
	"\fDialMetadata\x126\n" +
	"\x05state\x18\x01 \x01(\x0e2 .dtkt.core.v1.DialMetadata.StateR\x05state\x12\x18\n" +
	"\amessage\x18\x02 \x01(\tR\amessage\x12.\n" +
	"\x04time\x18\x03 \x01(\v2\x1a.google.protobuf.TimestampR\x04time\"E\n" +
	"\x05State\x12\x15\n" +
	"\x11STATE_UNSPECIFIED\x10\x00\x12\x13\n" +
	"\x0fSTATE_CONNECTED\x10\x01\x12\x10\n" +
	"\fSTATE_FAILED\x10\x02\"\xd8\x01\n" +
	"\x19BatchRunOperationMetadata\x12d\n" +
	"\x0ffailed_requests\x18\x01 \x03(\v2;.dtkt.core.v1.BatchRunOperationMetadata.FailedRequestsEntryR\x0efailedRequests\x1aU\n" +
	"\x13FailedRequestsEntry\x12\x10\n" +
	"\x03key\x18\x01 \x01(\x05R\x03key\x12(\n" +
	"\x05value\x18\x02 \x01(\v2\x12.google.rpc.StatusR\x05value:\x028\x01\"O\n" +
	"\fEncryptedAny\x12!\n" +
	"\btype_url\x18\x01 \x01(\tB\x06\xbaH\x03\xc8\x01\x01R\atypeUrl\x12\x1c\n" +
	"\x05value\x18\x02 \x01(\fB\x06\xbaH\x03\xc8\x01\x01R\x05value\"b\n" +
	"\x0eEncryptRequest\x12P\n" +
	"\bresource\x18\x01 \x01(\v2\x16.dtkt.core.v1.ResourceB\x1c\xbaH\x19\xc8\x01\x01\xea\x01\x13has(this.decrypted)R\bresource\"c\n" +
	"\x0fEncryptResponse\x12P\n" +
	"\bresource\x18\x01 \x01(\v2\x16.dtkt.core.v1.ResourceB\x1c\xbaH\x19\xc8\x01\x01\xea\x01\x13has(this.encrypted)R\bresource\"b\n" +
	"\x0eDecryptRequest\x12P\n" +
	"\bresource\x18\x01 \x01(\v2\x16.dtkt.core.v1.ResourceB\x1c\xbaH\x19\xc8\x01\x01\xea\x01\x13has(this.encrypted)R\bresource\"c\n" +
	"\x0fDecryptResponse\x12P\n" +
	"\bresource\x18\x01 \x01(\v2\x16.dtkt.core.v1.ResourceB\x1c\xbaH\x19\xc8\x01\x01\xea\x01\x13has(this.decrypted)R\bresource\"2\n" +
	"\x14GetAutomationRequest\x12\x1a\n" +
	"\x04name\x18\x01 \x01(\tB\x06\xbaH\x03\xc8\x01\x01R\x04name\"Y\n" +
	"\x15GetAutomationResponse\x12@\n" +
	"\n" +
	"automation\x18\x01 \x01(\v2\x18.dtkt.core.v1.AutomationB\x06\xbaH\x03\xc8\x01\x01R\n" +
	"automation\"\xd1\x01\n" +
	"\x17CreateAutomationRequest\x12$\n" +
	"\x06parent\x18\x01 \x01(\tB\f\xbaH\x03\xc8\x01\x01\x82\xb5\x18\x02\x18\x01R\x06parent\x12N\n" +
	"\rautomation_id\x18\x02 \x01(\tB)\xbaH\x13\xc8\x01\x01r\x0e2\f^[a-z0-9-]+$\x82\xb5\x18\x0f\x12\rAutomation IDR\fautomationId\x12@\n" +
	"\n" +
	"automation\x18\x03 \x01(\v2\x18.dtkt.core.v1.AutomationB\x06\xbaH\x03\xc8\x01\x01R\n" +
	"automation\"\\\n" +
	"\x18CreateAutomationResponse\x12@\n" +
	"\n" +
	"automation\x18\x01 \x01(\v2\x18.dtkt.core.v1.AutomationB\x06\xbaH\x03\xc8\x01\x01R\n" +
	"automation\"\xb1\x02\n" +
	"\x1dBatchCreateAutomationsRequest\x12$\n" +
	"\x06parent\x18\x01 \x01(\tB\f\xbaH\x03\xc8\x01\x01\x82\xb5\x18\x02\x18\x01R\x06parent\x12K\n" +
	"\brequests\x18\x02 \x03(\v2%.dtkt.core.v1.CreateAutomationRequestB\b\xbaH\x05\x92\x01\x02\b\x02R\brequests:\x9c\x01\xbaH\x98\x01\x1a\x95\x01\n" +
	"!batch_create_automations.requests\x12!requests must use the same parent\x1aMsize(this.requests.filter(r, r.parent == this.parent)) == size(this.requests)\"d\n" +
	"\x1eBatchCreateAutomationsResponse\x12B\n" +
	"\vautomations\x18\x01 \x03(\v2\x18.dtkt.core.v1.AutomationB\x06\xbaH\x03\xc8\x01\x01R\vautomations\"\xa4\x01\n" +
	"\x16ListAutomationsRequest\x12\x1b\n" +
	"\tpage_size\x18\x01 \x01(\x05R\bpageSize\x12\x1d\n" +
	"\n" +
	"page_token\x18\x02 \x01(\tR\tpageToken\x12\x1b\n" +
	"\x06parent\x18\x03 \x01(\tH\x00R\x06parent\x88\x01\x01\x12\x1b\n" +
	"\x06filter\x18\x04 \x01(\tH\x01R\x06filter\x88\x01\x01B\t\n" +
	"\a_parentB\t\n" +
	"\a_filter\"\x85\x01\n" +
	"\x17ListAutomationsResponse\x12B\n" +
	"\vautomations\x18\x01 \x03(\v2\x18.dtkt.core.v1.AutomationB\x06\xbaH\x03\xc8\x01\x01R\vautomations\x12&\n" +
	"\x0fnext_page_token\x18\x02 \x01(\tR\rnextPageToken\"\xec\x04\n" +
	"\x13SendAutomationEvent\x12R\n" +
	"\finputs_event\x18\x01 \x01(\v2-.dtkt.core.v1.SendAutomationEvent.InputsEventH\x00R\vinputsEvent\x12e\n" +
	"\x13user_response_event\x18\x02 \x01(\v23.dtkt.core.v1.SendAutomationEvent.UserResponseEventH\x00R\x11userResponseEvent\x1a\xb1\x01\n" +
	"\vInputsEvent\x12Q\n" +
	"\x06values\x18\x01 \x03(\v29.dtkt.core.v1.SendAutomationEvent.InputsEvent.ValuesEntryR\x06values\x1aO\n" +
	"\vValuesEntry\x12\x10\n" +
	"\x03key\x18\x01 \x01(\tR\x03key\x12*\n" +
	"\x05value\x18\x02 \x01(\v2\x14.google.protobuf.AnyR\x05value:\x028\x01\x1a\xdd\x01\n" +
	"\x11UserResponseEvent\x12\x16\n" +
	"\x02id\x18\x01 \x01(\tB\x06\xbaH\x03\xc8\x01\x01R\x02id\x12_\n" +
	"\x06values\x18\x02 \x03(\v2?.dtkt.core.v1.SendAutomationEvent.UserResponseEvent.ValuesEntryB\x06\xbaH\x03\xc8\x01\x01R\x06values\x1aO\n" +
	"\vValuesEntry\x12\x10\n" +
	"\x03key\x18\x01 \x01(\tR\x03key\x12*\n" +
	"\x05value\x18\x02 \x01(\v2\x14.google.protobuf.AnyR\x05value:\x028\x01B\x06\n" +
	"\x04type\"\x96\x04\n" +
	"\x16ReceiveAutomationEvent\x12X\n" +
	"\routputs_event\x18\x01 \x01(\v21.dtkt.core.v1.ReceiveAutomationEvent.OutputsEventH\x00R\foutputsEvent\x12e\n" +
	"\x12user_request_event\x18\x02 \x01(\v25.dtkt.core.v1.ReceiveAutomationEvent.UserRequestEventH\x00R\x10userRequestEvent\x1a\xbe\x01\n" +
	"\fOutputsEvent\x12]\n" +
	"\x06values\x18\x01 \x03(\v2=.dtkt.core.v1.ReceiveAutomationEvent.OutputsEvent.ValuesEntryB\x06\xbaH\x03\xc8\x01\x01R\x06values\x1aO\n" +
	"\vValuesEntry\x12\x10\n" +
	"\x03key\x18\x01 \x01(\tR\x03key\x12*\n" +
	"\x05value\x18\x02 \x01(\v2\x14.google.protobuf.AnyR\x05value:\x028\x01\x1ar\n" +
	"\x10UserRequestEvent\x12\x16\n" +
	"\x02id\x18\x01 \x01(\tB\x06\xbaH\x03\xc8\x01\x01R\x02id\x12F\n" +
	"\vuser_action\x18\x02 \x01(\v2\x1d.dtkt.flow.v1beta1.UserActionB\x06\xbaH\x03\xc8\x01\x01R\n" +
	"userActionB\x06\n" +
	"\x04type\"q\n" +
	"\x1aSendAutomationEventRequest\x12\x1a\n" +
	"\x04name\x18\x01 \x01(\tB\x06\xbaH\x03\xc8\x01\x01R\x04name\x127\n" +
	"\x05event\x18\x02 \x01(\v2!.dtkt.core.v1.SendAutomationEventR\x05event\"<\n" +
	"\x1eReceiveAutomationEventsRequest\x12\x1a\n" +
	"\x04name\x18\x01 \x01(\tB\x06\xbaH\x03\xc8\x01\x01R\x04name\"e\n" +
	"\x1fReceiveAutomationEventsResponse\x12B\n" +
	"\x05event\x18\x02 \x01(\v2$.dtkt.core.v1.ReceiveAutomationEventB\x06\xbaH\x03\xc8\x01\x01R\x05event\"t\n" +
	"\x1dStreamAutomationEventsRequest\x12\x1a\n" +
	"\x04name\x18\x01 \x01(\tB\x06\xbaH\x03\xc8\x01\x01R\x04name\x127\n" +
	"\x05event\x18\x02 \x01(\v2!.dtkt.core.v1.SendAutomationEventR\x05event\"d\n" +
	"\x1eStreamAutomationEventsResponse\x12B\n" +
	"\x05event\x18\x01 \x01(\v2$.dtkt.core.v1.ReceiveAutomationEventB\x06\xbaH\x03\xc8\x01\x01R\x05event\"\xa4\x01\n" +
	"\x16ListConnectionsRequest\x12\x1b\n" +
	"\tpage_size\x18\x01 \x01(\x05R\bpageSize\x12\x1d\n" +
	"\n" +
	"page_token\x18\x02 \x01(\tR\tpageToken\x12\x1b\n" +
	"\x06parent\x18\x03 \x01(\tH\x00R\x06parent\x88\x01\x01\x12\x1b\n" +
	"\x06filter\x18\x04 \x01(\tH\x01R\x06filter\x88\x01\x01B\t\n" +
	"\a_parentB\t\n" +
	"\a_filter\"}\n" +
	"\x17ListConnectionsResponse\x12:\n" +
	"\vconnections\x18\x01 \x03(\v2\x18.dtkt.core.v1.ConnectionR\vconnections\x12&\n" +
	"\x0fnext_page_token\x18\x02 \x01(\tR\rnextPageToken\"2\n" +
	"\x14GetConnectionRequest\x12\x1a\n" +
	"\x04name\x18\x01 \x01(\tB\x06\xbaH\x03\xc8\x01\x01R\x04name\"Y\n" +
	"\x15GetConnectionResponse\x12@\n" +
	"\n" +
	"connection\x18\x01 \x01(\v2\x18.dtkt.core.v1.ConnectionB\x06\xbaH\x03\xc8\x01\x01R\n" +
	"connection\"\xd1\x01\n" +
	"\x17CreateConnectionRequest\x12$\n" +
	"\x06parent\x18\x01 \x01(\tB\f\xbaH\x03\xc8\x01\x01\x82\xb5\x18\x02\x18\x01R\x06parent\x12N\n" +
	"\rconnection_id\x18\x02 \x01(\tB)\xbaH\x13\xc8\x01\x01r\x0e2\f^[a-z0-9-]+$\x82\xb5\x18\x0f\x12\rConnection IDR\fconnectionId\x12@\n" +
	"\n" +
	"connection\x18\x03 \x01(\v2\x18.dtkt.core.v1.ConnectionB\x06\xbaH\x03\xc8\x01\x01R\n" +
	"connection\"\\\n" +
	"\x18CreateConnectionResponse\x12@\n" +
	"\n" +
	"connection\x18\x01 \x01(\v2\x18.dtkt.core.v1.ConnectionB\x06\xbaH\x03\xc8\x01\x01R\n" +
	"connection\"\x98\x01\n" +
	"\x17UpdateConnectionRequest\x12@\n" +
	"\n" +
	"connection\x18\x01 \x01(\v2\x18.dtkt.core.v1.ConnectionB\x06\xbaH\x03\xc8\x01\x01R\n" +
	"connection\x12;\n" +
	"\vupdate_mask\x18\x02 \x01(\v2\x1a.google.protobuf.FieldMaskR\n" +
	"updateMask\"\\\n" +
	"\x18UpdateConnectionResponse\x12@\n" +
	"\n" +
	"connection\x18\x01 \x01(\v2\x18.dtkt.core.v1.ConnectionB\x06\xbaH\x03\xc8\x01\x01R\n" +
	"connection\"5\n" +
	"\x17DeleteConnectionRequest\x12\x1a\n" +
	"\x04name\x18\x01 \x01(\tB\x06\xbaH\x03\xc8\x01\x01R\x04name\"\x1a\n" +
	"\x18DeleteConnectionResponse\"]\n" +
	"\x15DialConnectionRequest\x12\x14\n" +
	"\x04name\x18\x01 \x01(\tH\x00R\x04name\x12$\n" +
	"\aaddress\x18\x02 \x01(\tB\b\xbaH\x05r\x03\x88\x01\x01H\x00R\aaddressB\b\n" +
	"\x06target\"P\n" +
	"\x16DialConnectionResponse\x126\n" +
	"\x04dial\x18\x01 \x01(\v2\x1a.dtkt.core.v1.DialMetadataB\x06\xbaH\x03\xc8\x01\x01R\x04dial\"L\n" +
	"\x16SyncDescriptorsRequest\x12\x1e\n" +
	"\x06parent\x18\x01 \x01(\tB\x06\xbaH\x03\xc8\x01\x01R\x06parent\x12\x12\n" +
	"\x04etag\x18\x02 \x01(\tR\x04etag\"I\n" +
	"\x17SyncDescriptorsResponse\x12.\n" +
	"\x04file\x18\x01 \x01(\v2\x12.dtkt.core.v1.FileB\x06\xbaH\x03\xc8\x01\x01R\x04file\"7\n" +
	"\x15GetDescriptorsRequest\x12\x1e\n" +
	"\x06parent\x18\x01 \x01(\tB\x06\xbaH\x03\xc8\x01\x01R\x06parent\"H\n" +
	"\x16GetDescriptorsResponse\x12.\n" +
	"\x04file\x18\x01 \x01(\v2\x12.dtkt.core.v1.FileB\x06\xbaH\x03\xc8\x01\x01R\x04file\"\xa4\x01\n" +
	"\x16ListDeploymentsRequest\x12\x1b\n" +
	"\tpage_size\x18\x01 \x01(\x05R\bpageSize\x12\x1d\n" +
	"\n" +
	"page_token\x18\x02 \x01(\tR\tpageToken\x12\x1b\n" +
	"\x06parent\x18\x03 \x01(\tH\x00R\x06parent\x88\x01\x01\x12\x1b\n" +
	"\x06filter\x18\x04 \x01(\tH\x01R\x06filter\x88\x01\x01B\t\n" +
	"\a_parentB\t\n" +
	"\a_filter\"}\n" +
	"\x17ListDeploymentsResponse\x12:\n" +
	"\vdeployments\x18\x01 \x03(\v2\x18.dtkt.core.v1.DeploymentR\vdeployments\x12&\n" +
	"\x0fnext_page_token\x18\x02 \x01(\tR\rnextPageToken\"2\n" +
	"\x14GetDeploymentRequest\x12\x1a\n" +
	"\x04name\x18\x01 \x01(\tB\x06\xbaH\x03\xc8\x01\x01R\x04name\"Y\n" +
	"\x15GetDeploymentResponse\x12@\n" +
	"\n" +
	"deployment\x18\x01 \x01(\v2\x18.dtkt.core.v1.DeploymentB\x06\xbaH\x03\xc8\x01\x01R\n" +
	"deployment\"\xbe\x01\n" +
	"\x17CreateDeploymentRequest\x12$\n" +
	"\x06parent\x18\x01 \x01(\tB\f\xbaH\x03\xc8\x01\x01\x82\xb5\x18\x02\x18\x01R\x06parent\x12;\n" +
	"\rdeployment_id\x18\x02 \x01(\tB\x16\xbaH\x13\xc8\x01\x01r\x0e2\f^[a-z0-9-]+$R\fdeploymentId\x12@\n" +
	"\n" +
	"deployment\x18\x03 \x01(\v2\x18.dtkt.core.v1.DeploymentB\x06\xbaH\x03\xc8\x01\x01R\n" +
	"deployment\"\x80\x02\n" +
	"\x17UpdateDeploymentRequest\x12@\n" +
	"\n" +
	"deployment\x18\x01 \x01(\v2\x18.dtkt.core.v1.DeploymentB\x06\xbaH\x03\xc8\x01\x01R\n" +
	"deployment\x12;\n" +
	"\vupdate_mask\x18\x02 \x01(\v2\x1a.google.protobuf.FieldMaskR\n" +
	"updateMask\x12T\n" +
	"\rdesired_state\x18\x03 \x01(\x0e2\x1e.dtkt.core.v1.Deployment.StateB\n" +
	"\xbaH\a\x82\x01\x04\x18\x02\x18\x04H\x00R\fdesiredState\x88\x01\x01B\x10\n" +
	"\x0e_desired_state\"Q\n" +
	"\x17DeleteDeploymentRequest\x12\x1a\n" +
	"\x04name\x18\x01 \x01(\tB\x06\xbaH\x03\xc8\x01\x01R\x04name\x12\x1a\n" +
	"\x04etag\x18\x02 \x01(\tB\x06\xbaH\x03\xc8\x01\x01R\x04etag\"\x9e\x01\n" +
	"\x10ListFlowsRequest\x12\x1b\n" +
	"\tpage_size\x18\x01 \x01(\x05R\bpageSize\x12\x1d\n" +
	"\n" +
	"page_token\x18\x02 \x01(\tR\tpageToken\x12\x1b\n" +
	"\x06parent\x18\x03 \x01(\tH\x00R\x06parent\x88\x01\x01\x12\x1b\n" +
	"\x06filter\x18\x04 \x01(\tH\x01R\x06filter\x88\x01\x01B\t\n" +
	"\a_parentB\t\n" +
	"\a_filter\"e\n" +
	"\x11ListFlowsResponse\x12(\n" +
	"\x05flows\x18\x01 \x03(\v2\x12.dtkt.core.v1.FlowR\x05flows\x12&\n" +
	"\x0fnext_page_token\x18\x02 \x01(\tR\rnextPageToken\",\n" +
	"\x0eGetFlowRequest\x12\x1a\n" +
	"\x04name\x18\x01 \x01(\tB\x06\xbaH\x03\xc8\x01\x01R\x04name\"A\n" +
	"\x0fGetFlowResponse\x12.\n" +
	"\x04flow\x18\x01 \x01(\v2\x12.dtkt.core.v1.FlowB\x06\xbaH\x03\xc8\x01\x01R\x04flow\"\x9a\x01\n" +
	"\x11CreateFlowRequest\x12$\n" +
	"\x06parent\x18\x01 \x01(\tB\f\xbaH\x03\xc8\x01\x01\x82\xb5\x18\x02\x18\x01R\x06parent\x12/\n" +
	"\aflow_id\x18\x02 \x01(\tB\x16\xbaH\x13\xc8\x01\x01r\x0e2\f^[a-z0-9-]+$R\x06flowId\x12.\n" +
	"\x04flow\x18\x03 \x01(\v2\x12.dtkt.core.v1.FlowB\x06\xbaH\x03\xc8\x01\x01R\x04flow\"D\n" +
	"\x12CreateFlowResponse\x12.\n" +
	"\x04flow\x18\x01 \x01(\v2\x12.dtkt.core.v1.FlowB\x06\xbaH\x03\xc8\x01\x01R\x04flow\"\x80\x01\n" +
	"\x11UpdateFlowRequest\x12.\n" +
	"\x04flow\x18\x01 \x01(\v2\x12.dtkt.core.v1.FlowB\x06\xbaH\x03\xc8\x01\x01R\x04flow\x12;\n" +
	"\vupdate_mask\x18\x02 \x01(\v2\x1a.google.protobuf.FieldMaskR\n" +
	"updateMask\"D\n" +
	"\x12UpdateFlowResponse\x12.\n" +
	"\x04flow\x18\x01 \x01(\v2\x12.dtkt.core.v1.FlowB\x06\xbaH\x03\xc8\x01\x01R\x04flow\"/\n" +
	"\x11DeleteFlowRequest\x12\x1a\n" +
	"\x04name\x18\x01 \x01(\tB\x06\xbaH\x03\xc8\x01\x01R\x04name\"\x14\n" +
	"\x12DeleteFlowResponse\"\xa5\x01\n" +
	"\x17ListIntegrationsRequest\x12\x1b\n" +
	"\tpage_size\x18\x01 \x01(\x05R\bpageSize\x12\x1d\n" +
	"\n" +
	"page_token\x18\x02 \x01(\tR\tpageToken\x12\x1b\n" +
	"\x06parent\x18\x03 \x01(\tH\x00R\x06parent\x88\x01\x01\x12\x1b\n" +
	"\x06filter\x18\x04 \x01(\tH\x01R\x06filter\x88\x01\x01B\t\n" +
	"\a_parentB\t\n" +
	"\a_filter\"\x81\x01\n" +
	"\x18ListIntegrationsResponse\x12=\n" +
	"\fintegrations\x18\x01 \x03(\v2\x19.dtkt.core.v1.IntegrationR\fintegrations\x12&\n" +
	"\x0fnext_page_token\x18\x02 \x01(\tR\rnextPageToken\"3\n" +
	"\x15GetIntegrationRequest\x12\x1a\n" +
	"\x04name\x18\x01 \x01(\tB\x06\xbaH\x03\xc8\x01\x01R\x04name\"]\n" +
	"\x16GetIntegrationResponse\x12C\n" +
	"\vintegration\x18\x01 \x01(\v2\x19.dtkt.core.v1.IntegrationB\x06\xbaH\x03\xc8\x01\x01R\vintegration\"\xc4\x01\n" +
	"\x18CreateIntegrationRequest\x12$\n" +
	"\x06parent\x18\x01 \x01(\tB\f\xbaH\x03\xc8\x01\x01\x82\xb5\x18\x02\x18\x01R\x06parent\x12=\n" +
	"\x0eintegration_id\x18\x02 \x01(\tB\x16\xbaH\x13\xc8\x01\x01r\x0e2\f^[a-z0-9-]+$R\rintegrationId\x12C\n" +
	"\vintegration\x18\x03 \x01(\v2\x19.dtkt.core.v1.IntegrationB\x06\xbaH\x03\xc8\x01\x01R\vintegration\"`\n" +
	"\x19CreateIntegrationResponse\x12C\n" +
	"\vintegration\x18\x01 \x01(\v2\x19.dtkt.core.v1.IntegrationB\x06\xbaH\x03\xc8\x01\x01R\vintegration\"\xcc\x01\n" +
	"\x17BuildIntegrationRequest\x12\x1a\n" +
	"\x04name\x18\x01 \x01(\tB\x06\xbaH\x03\xc8\x01\x01R\x04name\x12*\n" +
	"\n" +
	"source_uri\x18\x02 \x01(\tB\v\xbaH\b\xc8\x01\x01r\x03\x88\x01\x01R\tsourceUri\x12'\n" +
	"\vsource_etag\x18\x03 \x01(\tB\x06\xbaH\x03\xc8\x01\x01R\n" +
	"sourceEtag\x12@\n" +
	"\x05build\x18\x04 \x01(\v2\".dtkt.core.v1.PackageBuildMetadataB\x06\xbaH\x03\xc8\x01\x01R\x05build\"\xe7\x01\n" +
	"\x18BuildIntegrationResponse\x12C\n" +
	"\vintegration\x18\x01 \x01(\v2\x19.dtkt.core.v1.IntegrationB\x06\xbaH\x03\xc8\x01\x01R\vintegration\x12@\n" +
	"\x05build\x18\x02 \x01(\v2\".dtkt.core.v1.PackageBuildMetadataB\x06\xbaH\x03\xc8\x01\x01R\x05build:D\xbaHA*?has(this.build.artifact_etag) && this.build.artifact_etag != ''\"\x9c\x01\n" +
	"\x18UpdateIntegrationRequest\x12C\n" +
	"\vintegration\x18\x01 \x01(\v2\x19.dtkt.core.v1.IntegrationB\x06\xbaH\x03\xc8\x01\x01R\vintegration\x12;\n" +
	"\vupdate_mask\x18\x02 \x01(\v2\x1a.google.protobuf.FieldMaskR\n" +
	"updateMask\"`\n" +
	"\x19UpdateIntegrationResponse\x12C\n" +
	"\vintegration\x18\x01 \x01(\v2\x19.dtkt.core.v1.IntegrationB\x06\xbaH\x03\xc8\x01\x01R\vintegration\"6\n" +
	"\x18DeleteIntegrationRequest\x12\x1a\n" +
	"\x04name\x18\x01 \x01(\tB\x06\xbaH\x03\xc8\x01\x01R\x04name\"\x1b\n" +
	"\x19DeleteIntegrationResponse\",\n" +
	"\x0eGetTypeRequest\x12\x1a\n" +
	"\x04name\x18\x01 \x01(\tB\x06\xbaH\x03\xc8\x01\x01R\x04name\"A\n" +
	"\x0fGetTypeResponse\x12.\n" +
	"\x04type\x18\x01 \x01(\v2\x12.dtkt.core.v1.TypeB\x06\xbaH\x03\xc8\x01\x01R\x04type\"\x9e\x01\n" +
	"\x10ListTypesRequest\x12\x1b\n" +
	"\tpage_size\x18\x01 \x01(\x05R\bpageSize\x12\x1d\n" +
	"\n" +
	"page_token\x18\x02 \x01(\tR\tpageToken\x12\x1b\n" +
	"\x06parent\x18\x03 \x01(\tH\x00R\x06parent\x88\x01\x01\x12\x1b\n" +
	"\x06filter\x18\x04 \x01(\tH\x01R\x06filter\x88\x01\x01B\t\n" +
	"\a_parentB\t\n" +
	"\a_filter\"e\n" +
	"\x11ListTypesResponse\x12(\n" +
	"\x05types\x18\x01 \x03(\v2\x12.dtkt.core.v1.TypeR\x05types\x12&\n" +
	"\x0fnext_page_token\x18\x02 \x01(\tR\rnextPageToken\".\n" +
	"\x10GetMethodRequest\x12\x1a\n" +
	"\x04name\x18\x01 \x01(\tB\x06\xbaH\x03\xc8\x01\x01R\x04name\"I\n" +
	"\x11GetMethodResponse\x124\n" +
	"\x06method\x18\x01 \x01(\v2\x14.dtkt.core.v1.MethodB\x06\xbaH\x03\xc8\x01\x01R\x06method\"/\n" +
	"\x11GetServiceRequest\x12\x1a\n" +
	"\x04name\x18\x01 \x01(\tB\x06\xbaH\x03\xc8\x01\x01R\x04name\"M\n" +
	"\x12GetServiceResponse\x127\n" +
	"\aservice\x18\x01 \x01(\v2\x15.dtkt.core.v1.ServiceB\x06\xbaH\x03\xc8\x01\x01R\aservice\"\xa1\x01\n" +
	"\x13ListServicesRequest\x12\x1b\n" +
	"\tpage_size\x18\x01 \x01(\x05R\bpageSize\x12\x1d\n" +
	"\n" +
	"page_token\x18\x02 \x01(\tR\tpageToken\x12\x1b\n" +
	"\x06parent\x18\x03 \x01(\tH\x00R\x06parent\x88\x01\x01\x12\x1b\n" +
	"\x06filter\x18\x04 \x01(\tH\x01R\x06filter\x88\x01\x01B\t\n" +
	"\a_parentB\t\n" +
	"\a_filter\"q\n" +
	"\x14ListServicesResponse\x121\n" +
	"\bservices\x18\x01 \x03(\v2\x15.dtkt.core.v1.ServiceR\bservices\x12&\n" +
	"\x0fnext_page_token\x18\x02 \x01(\tR\rnextPageToken\"\xa0\x01\n" +
	"\x12ListMethodsRequest\x12\x1b\n" +
	"\tpage_size\x18\x01 \x01(\x05R\bpageSize\x12\x1d\n" +
	"\n" +
	"page_token\x18\x02 \x01(\tR\tpageToken\x12\x1b\n" +
	"\x06parent\x18\x03 \x01(\tH\x00R\x06parent\x88\x01\x01\x12\x1b\n" +
	"\x06filter\x18\x04 \x01(\tH\x01R\x06filter\x88\x01\x01B\t\n" +
	"\a_parentB\t\n" +
	"\a_filter\"m\n" +
	"\x13ListMethodsResponse\x12.\n" +
	"\amethods\x18\x01 \x03(\v2\x14.dtkt.core.v1.MethodR\amethods\x12&\n" +
	"\x0fnext_page_token\x18\x02 \x01(\tR\rnextPageTokenB\xb7\x01\n" +
	"\x12proto.dtkt.core.v1B\rMessagesProtoP\x01Z@github.com/datakit-dev/dtkt-sdk/sdk-go/proto/dtkt/core/v1;corev1\xa2\x02\x03DCX\xaa\x02\fDtkt.Core.V1\xca\x02\fDtkt\\Core\\V1\xe2\x02\x18Dtkt\\Core\\V1\\GPBMetadata\xea\x02\x0eDtkt::Core::V1b\x06proto3"

var (
	file_dtkt_core_v1_messages_proto_rawDescOnce sync.Once
	file_dtkt_core_v1_messages_proto_rawDescData []byte
)

func file_dtkt_core_v1_messages_proto_rawDescGZIP() []byte {
	file_dtkt_core_v1_messages_proto_rawDescOnce.Do(func() {
		file_dtkt_core_v1_messages_proto_rawDescData = protoimpl.X.CompressGZIP(unsafe.Slice(unsafe.StringData(file_dtkt_core_v1_messages_proto_rawDesc), len(file_dtkt_core_v1_messages_proto_rawDesc)))
	})
	return file_dtkt_core_v1_messages_proto_rawDescData
}

var file_dtkt_core_v1_messages_proto_enumTypes = make([]protoimpl.EnumInfo, 6)
var file_dtkt_core_v1_messages_proto_msgTypes = make([]protoimpl.MessageInfo, 125)
var file_dtkt_core_v1_messages_proto_goTypes = []any{
	(Auth_AuthMethod)(0),                          // 0: dtkt.core.v1.Auth.AuthMethod
	(Deployment_State)(0),                         // 1: dtkt.core.v1.Deployment.State
	(BuildOperationMetadata_State)(0),             // 2: dtkt.core.v1.BuildOperationMetadata.State
	(RunOperationMetadata_State)(0),               // 3: dtkt.core.v1.RunOperationMetadata.State
	(SyncOperationMetadata_State)(0),              // 4: dtkt.core.v1.SyncOperationMetadata.State
	(DialMetadata_State)(0),                       // 5: dtkt.core.v1.DialMetadata.State
	(*Address)(nil),                               // 6: dtkt.core.v1.Address
	(*Auth)(nil),                                  // 7: dtkt.core.v1.Auth
	(*Context)(nil),                               // 8: dtkt.core.v1.Context
	(*Resource)(nil),                              // 9: dtkt.core.v1.Resource
	(*Automation)(nil),                            // 10: dtkt.core.v1.Automation
	(*Flow)(nil),                                  // 11: dtkt.core.v1.Flow
	(*Connection)(nil),                            // 12: dtkt.core.v1.Connection
	(*Deployment)(nil),                            // 13: dtkt.core.v1.Deployment
	(*Integration)(nil),                           // 14: dtkt.core.v1.Integration
	(*File)(nil),                                  // 15: dtkt.core.v1.File
	(*Service)(nil),                               // 16: dtkt.core.v1.Service
	(*Method)(nil),                                // 17: dtkt.core.v1.Method
	(*Type)(nil),                                  // 18: dtkt.core.v1.Type
	(*TypeSchema)(nil),                            // 19: dtkt.core.v1.TypeSchema
	(*DeploymentMetadata)(nil),                    // 20: dtkt.core.v1.DeploymentMetadata
	(*FlowConnectionMetadata)(nil),                // 21: dtkt.core.v1.FlowConnectionMetadata
	(*FlowRuntimeMetadata)(nil),                   // 22: dtkt.core.v1.FlowRuntimeMetadata
	(*FlowGroupsMetadata)(nil),                    // 23: dtkt.core.v1.FlowGroupsMetadata
	(*FlowSpecMetadata)(nil),                      // 24: dtkt.core.v1.FlowSpecMetadata
	(*FlowGraphMetadata)(nil),                     // 25: dtkt.core.v1.FlowGraphMetadata
	(*PackageSpecMetadata)(nil),                   // 26: dtkt.core.v1.PackageSpecMetadata
	(*PackageBuildMetadata)(nil),                  // 27: dtkt.core.v1.PackageBuildMetadata
	(*BuildOperationMetadata)(nil),                // 28: dtkt.core.v1.BuildOperationMetadata
	(*RunOperationMetadata)(nil),                  // 29: dtkt.core.v1.RunOperationMetadata
	(*SyncOperationMetadata)(nil),                 // 30: dtkt.core.v1.SyncOperationMetadata
	(*DialMetadata)(nil),                          // 31: dtkt.core.v1.DialMetadata
	(*BatchRunOperationMetadata)(nil),             // 32: dtkt.core.v1.BatchRunOperationMetadata
	(*EncryptedAny)(nil),                          // 33: dtkt.core.v1.EncryptedAny
	(*EncryptRequest)(nil),                        // 34: dtkt.core.v1.EncryptRequest
	(*EncryptResponse)(nil),                       // 35: dtkt.core.v1.EncryptResponse
	(*DecryptRequest)(nil),                        // 36: dtkt.core.v1.DecryptRequest
	(*DecryptResponse)(nil),                       // 37: dtkt.core.v1.DecryptResponse
	(*GetAutomationRequest)(nil),                  // 38: dtkt.core.v1.GetAutomationRequest
	(*GetAutomationResponse)(nil),                 // 39: dtkt.core.v1.GetAutomationResponse
	(*CreateAutomationRequest)(nil),               // 40: dtkt.core.v1.CreateAutomationRequest
	(*CreateAutomationResponse)(nil),              // 41: dtkt.core.v1.CreateAutomationResponse
	(*BatchCreateAutomationsRequest)(nil),         // 42: dtkt.core.v1.BatchCreateAutomationsRequest
	(*BatchCreateAutomationsResponse)(nil),        // 43: dtkt.core.v1.BatchCreateAutomationsResponse
	(*ListAutomationsRequest)(nil),                // 44: dtkt.core.v1.ListAutomationsRequest
	(*ListAutomationsResponse)(nil),               // 45: dtkt.core.v1.ListAutomationsResponse
	(*SendAutomationEvent)(nil),                   // 46: dtkt.core.v1.SendAutomationEvent
	(*ReceiveAutomationEvent)(nil),                // 47: dtkt.core.v1.ReceiveAutomationEvent
	(*SendAutomationEventRequest)(nil),            // 48: dtkt.core.v1.SendAutomationEventRequest
	(*ReceiveAutomationEventsRequest)(nil),        // 49: dtkt.core.v1.ReceiveAutomationEventsRequest
	(*ReceiveAutomationEventsResponse)(nil),       // 50: dtkt.core.v1.ReceiveAutomationEventsResponse
	(*StreamAutomationEventsRequest)(nil),         // 51: dtkt.core.v1.StreamAutomationEventsRequest
	(*StreamAutomationEventsResponse)(nil),        // 52: dtkt.core.v1.StreamAutomationEventsResponse
	(*ListConnectionsRequest)(nil),                // 53: dtkt.core.v1.ListConnectionsRequest
	(*ListConnectionsResponse)(nil),               // 54: dtkt.core.v1.ListConnectionsResponse
	(*GetConnectionRequest)(nil),                  // 55: dtkt.core.v1.GetConnectionRequest
	(*GetConnectionResponse)(nil),                 // 56: dtkt.core.v1.GetConnectionResponse
	(*CreateConnectionRequest)(nil),               // 57: dtkt.core.v1.CreateConnectionRequest
	(*CreateConnectionResponse)(nil),              // 58: dtkt.core.v1.CreateConnectionResponse
	(*UpdateConnectionRequest)(nil),               // 59: dtkt.core.v1.UpdateConnectionRequest
	(*UpdateConnectionResponse)(nil),              // 60: dtkt.core.v1.UpdateConnectionResponse
	(*DeleteConnectionRequest)(nil),               // 61: dtkt.core.v1.DeleteConnectionRequest
	(*DeleteConnectionResponse)(nil),              // 62: dtkt.core.v1.DeleteConnectionResponse
	(*DialConnectionRequest)(nil),                 // 63: dtkt.core.v1.DialConnectionRequest
	(*DialConnectionResponse)(nil),                // 64: dtkt.core.v1.DialConnectionResponse
	(*SyncDescriptorsRequest)(nil),                // 65: dtkt.core.v1.SyncDescriptorsRequest
	(*SyncDescriptorsResponse)(nil),               // 66: dtkt.core.v1.SyncDescriptorsResponse
	(*GetDescriptorsRequest)(nil),                 // 67: dtkt.core.v1.GetDescriptorsRequest
	(*GetDescriptorsResponse)(nil),                // 68: dtkt.core.v1.GetDescriptorsResponse
	(*ListDeploymentsRequest)(nil),                // 69: dtkt.core.v1.ListDeploymentsRequest
	(*ListDeploymentsResponse)(nil),               // 70: dtkt.core.v1.ListDeploymentsResponse
	(*GetDeploymentRequest)(nil),                  // 71: dtkt.core.v1.GetDeploymentRequest
	(*GetDeploymentResponse)(nil),                 // 72: dtkt.core.v1.GetDeploymentResponse
	(*CreateDeploymentRequest)(nil),               // 73: dtkt.core.v1.CreateDeploymentRequest
	(*UpdateDeploymentRequest)(nil),               // 74: dtkt.core.v1.UpdateDeploymentRequest
	(*DeleteDeploymentRequest)(nil),               // 75: dtkt.core.v1.DeleteDeploymentRequest
	(*ListFlowsRequest)(nil),                      // 76: dtkt.core.v1.ListFlowsRequest
	(*ListFlowsResponse)(nil),                     // 77: dtkt.core.v1.ListFlowsResponse
	(*GetFlowRequest)(nil),                        // 78: dtkt.core.v1.GetFlowRequest
	(*GetFlowResponse)(nil),                       // 79: dtkt.core.v1.GetFlowResponse
	(*CreateFlowRequest)(nil),                     // 80: dtkt.core.v1.CreateFlowRequest
	(*CreateFlowResponse)(nil),                    // 81: dtkt.core.v1.CreateFlowResponse
	(*UpdateFlowRequest)(nil),                     // 82: dtkt.core.v1.UpdateFlowRequest
	(*UpdateFlowResponse)(nil),                    // 83: dtkt.core.v1.UpdateFlowResponse
	(*DeleteFlowRequest)(nil),                     // 84: dtkt.core.v1.DeleteFlowRequest
	(*DeleteFlowResponse)(nil),                    // 85: dtkt.core.v1.DeleteFlowResponse
	(*ListIntegrationsRequest)(nil),               // 86: dtkt.core.v1.ListIntegrationsRequest
	(*ListIntegrationsResponse)(nil),              // 87: dtkt.core.v1.ListIntegrationsResponse
	(*GetIntegrationRequest)(nil),                 // 88: dtkt.core.v1.GetIntegrationRequest
	(*GetIntegrationResponse)(nil),                // 89: dtkt.core.v1.GetIntegrationResponse
	(*CreateIntegrationRequest)(nil),              // 90: dtkt.core.v1.CreateIntegrationRequest
	(*CreateIntegrationResponse)(nil),             // 91: dtkt.core.v1.CreateIntegrationResponse
	(*BuildIntegrationRequest)(nil),               // 92: dtkt.core.v1.BuildIntegrationRequest
	(*BuildIntegrationResponse)(nil),              // 93: dtkt.core.v1.BuildIntegrationResponse
	(*UpdateIntegrationRequest)(nil),              // 94: dtkt.core.v1.UpdateIntegrationRequest
	(*UpdateIntegrationResponse)(nil),             // 95: dtkt.core.v1.UpdateIntegrationResponse
	(*DeleteIntegrationRequest)(nil),              // 96: dtkt.core.v1.DeleteIntegrationRequest
	(*DeleteIntegrationResponse)(nil),             // 97: dtkt.core.v1.DeleteIntegrationResponse
	(*GetTypeRequest)(nil),                        // 98: dtkt.core.v1.GetTypeRequest
	(*GetTypeResponse)(nil),                       // 99: dtkt.core.v1.GetTypeResponse
	(*ListTypesRequest)(nil),                      // 100: dtkt.core.v1.ListTypesRequest
	(*ListTypesResponse)(nil),                     // 101: dtkt.core.v1.ListTypesResponse
	(*GetMethodRequest)(nil),                      // 102: dtkt.core.v1.GetMethodRequest
	(*GetMethodResponse)(nil),                     // 103: dtkt.core.v1.GetMethodResponse
	(*GetServiceRequest)(nil),                     // 104: dtkt.core.v1.GetServiceRequest
	(*GetServiceResponse)(nil),                    // 105: dtkt.core.v1.GetServiceResponse
	(*ListServicesRequest)(nil),                   // 106: dtkt.core.v1.ListServicesRequest
	(*ListServicesResponse)(nil),                  // 107: dtkt.core.v1.ListServicesResponse
	(*ListMethodsRequest)(nil),                    // 108: dtkt.core.v1.ListMethodsRequest
	(*ListMethodsResponse)(nil),                   // 109: dtkt.core.v1.ListMethodsResponse
	nil,                                           // 110: dtkt.core.v1.Automation.ConnectionsEntry
	nil,                                           // 111: dtkt.core.v1.Automation.InputsEntry
	nil,                                           // 112: dtkt.core.v1.Connection.HeadersEntry
	nil,                                           // 113: dtkt.core.v1.Deployment.EnvEntry
	nil,                                           // 114: dtkt.core.v1.Deployment.PortsEntry
	(*Deployment_Build)(nil),                      // 115: dtkt.core.v1.Deployment.Build
	(*Deployment_Cloud)(nil),                      // 116: dtkt.core.v1.Deployment.Cloud
	(*Deployment_RuntimeMetadata)(nil),            // 117: dtkt.core.v1.Deployment.RuntimeMetadata
	(*Deployment_RuntimeMetadata_Native)(nil),     // 118: dtkt.core.v1.Deployment.RuntimeMetadata.Native
	(*Deployment_RuntimeMetadata_Docker)(nil),     // 119: dtkt.core.v1.Deployment.RuntimeMetadata.Docker
	(*PackageBuildMetadata_Runtime)(nil),          // 120: dtkt.core.v1.PackageBuildMetadata.Runtime
	(*PackageBuildMetadata_Platform)(nil),         // 121: dtkt.core.v1.PackageBuildMetadata.Platform
	nil,                                           // 122: dtkt.core.v1.PackageBuildMetadata.EnvEntry
	nil,                                           // 123: dtkt.core.v1.BatchRunOperationMetadata.FailedRequestsEntry
	(*SendAutomationEvent_InputsEvent)(nil),       // 124: dtkt.core.v1.SendAutomationEvent.InputsEvent
	(*SendAutomationEvent_UserResponseEvent)(nil), // 125: dtkt.core.v1.SendAutomationEvent.UserResponseEvent
	nil, // 126: dtkt.core.v1.SendAutomationEvent.InputsEvent.ValuesEntry
	nil, // 127: dtkt.core.v1.SendAutomationEvent.UserResponseEvent.ValuesEntry
	(*ReceiveAutomationEvent_OutputsEvent)(nil),     // 128: dtkt.core.v1.ReceiveAutomationEvent.OutputsEvent
	(*ReceiveAutomationEvent_UserRequestEvent)(nil), // 129: dtkt.core.v1.ReceiveAutomationEvent.UserRequestEvent
	nil,                                    // 130: dtkt.core.v1.ReceiveAutomationEvent.OutputsEvent.ValuesEntry
	(*timestamppb.Timestamp)(nil),          // 131: google.protobuf.Timestamp
	(*anypb.Any)(nil),                      // 132: google.protobuf.Any
	(*durationpb.Duration)(nil),            // 133: google.protobuf.Duration
	(*descriptorpb.FileDescriptorSet)(nil), // 134: google.protobuf.FileDescriptorSet
	(*v1beta1.TypeSchema)(nil),             // 135: dtkt.shared.v1beta1.TypeSchema
	(*v1beta11.Runtime)(nil),               // 136: dtkt.flow.v1beta1.Runtime
	(*v1beta11.Groups)(nil),                // 137: dtkt.flow.v1beta1.Groups
	(*v1beta11.Flow)(nil),                  // 138: dtkt.flow.v1beta1.Flow
	(*v1beta11.Graph)(nil),                 // 139: dtkt.flow.v1beta1.Graph
	(*v1beta1.Package)(nil),                // 140: dtkt.shared.v1beta1.Package
	(*fieldmaskpb.FieldMask)(nil),          // 141: google.protobuf.FieldMask
	(v1beta1.Runtime)(0),                   // 142: dtkt.shared.v1beta1.Runtime
	(*v1beta1.Platform)(nil),               // 143: dtkt.shared.v1beta1.Platform
	(*status.Status)(nil),                  // 144: google.rpc.Status
	(*v1beta11.UserAction)(nil),            // 145: dtkt.flow.v1beta1.UserAction
}
var file_dtkt_core_v1_messages_proto_depIdxs = []int32{
	0,   // 0: dtkt.core.v1.Auth.method:type_name -> dtkt.core.v1.Auth.AuthMethod
	131, // 1: dtkt.core.v1.Auth.expires_at:type_name -> google.protobuf.Timestamp
	6,   // 2: dtkt.core.v1.Context.address:type_name -> dtkt.core.v1.Address
	7,   // 3: dtkt.core.v1.Context.auth:type_name -> dtkt.core.v1.Auth
	8,   // 4: dtkt.core.v1.Resource.context:type_name -> dtkt.core.v1.Context
	6,   // 5: dtkt.core.v1.Resource.address:type_name -> dtkt.core.v1.Address
	33,  // 6: dtkt.core.v1.Resource.encrypted:type_name -> dtkt.core.v1.EncryptedAny
	132, // 7: dtkt.core.v1.Resource.decrypted:type_name -> google.protobuf.Any
	22,  // 8: dtkt.core.v1.Automation.runtime:type_name -> dtkt.core.v1.FlowRuntimeMetadata
	23,  // 9: dtkt.core.v1.Automation.groups:type_name -> dtkt.core.v1.FlowGroupsMetadata
	110, // 10: dtkt.core.v1.Automation.connections:type_name -> dtkt.core.v1.Automation.ConnectionsEntry
	111, // 11: dtkt.core.v1.Automation.inputs:type_name -> dtkt.core.v1.Automation.InputsEntry
	133, // 12: dtkt.core.v1.Automation.timeout:type_name -> google.protobuf.Duration
	131, // 13: dtkt.core.v1.Automation.create_time:type_name -> google.protobuf.Timestamp
	131, // 14: dtkt.core.v1.Automation.update_time:type_name -> google.protobuf.Timestamp
	24,  // 15: dtkt.core.v1.Flow.spec:type_name -> dtkt.core.v1.FlowSpecMetadata
	25,  // 16: dtkt.core.v1.Flow.graph:type_name -> dtkt.core.v1.FlowGraphMetadata
	131, // 17: dtkt.core.v1.Flow.create_time:type_name -> google.protobuf.Timestamp
	131, // 18: dtkt.core.v1.Flow.update_time:type_name -> google.protobuf.Timestamp
	6,   // 19: dtkt.core.v1.Connection.address:type_name -> dtkt.core.v1.Address
	132, // 20: dtkt.core.v1.Connection.decrypted_config:type_name -> google.protobuf.Any
	33,  // 21: dtkt.core.v1.Connection.encrypted_config:type_name -> dtkt.core.v1.EncryptedAny
	112, // 22: dtkt.core.v1.Connection.headers:type_name -> dtkt.core.v1.Connection.HeadersEntry
	131, // 23: dtkt.core.v1.Connection.create_time:type_name -> google.protobuf.Timestamp
	131, // 24: dtkt.core.v1.Connection.update_time:type_name -> google.protobuf.Timestamp
	1,   // 25: dtkt.core.v1.Deployment.state:type_name -> dtkt.core.v1.Deployment.State
	6,   // 26: dtkt.core.v1.Deployment.address:type_name -> dtkt.core.v1.Address
	113, // 27: dtkt.core.v1.Deployment.env:type_name -> dtkt.core.v1.Deployment.EnvEntry
	114, // 28: dtkt.core.v1.Deployment.ports:type_name -> dtkt.core.v1.Deployment.PortsEntry
	115, // 29: dtkt.core.v1.Deployment.build:type_name -> dtkt.core.v1.Deployment.Build
	116, // 30: dtkt.core.v1.Deployment.cloud:type_name -> dtkt.core.v1.Deployment.Cloud
	19,  // 31: dtkt.core.v1.Deployment.config_schema:type_name -> dtkt.core.v1.TypeSchema
	26,  // 32: dtkt.core.v1.Deployment.package_spec:type_name -> dtkt.core.v1.PackageSpecMetadata
	117, // 33: dtkt.core.v1.Deployment.runtime:type_name -> dtkt.core.v1.Deployment.RuntimeMetadata
	131, // 34: dtkt.core.v1.Deployment.create_time:type_name -> google.protobuf.Timestamp
	131, // 35: dtkt.core.v1.Deployment.update_time:type_name -> google.protobuf.Timestamp
	26,  // 36: dtkt.core.v1.Integration.spec:type_name -> dtkt.core.v1.PackageSpecMetadata
	131, // 37: dtkt.core.v1.Integration.create_time:type_name -> google.protobuf.Timestamp
	131, // 38: dtkt.core.v1.Integration.update_time:type_name -> google.protobuf.Timestamp
	134, // 39: dtkt.core.v1.File.protos:type_name -> google.protobuf.FileDescriptorSet
	131, // 40: dtkt.core.v1.File.create_time:type_name -> google.protobuf.Timestamp
	131, // 41: dtkt.core.v1.File.update_time:type_name -> google.protobuf.Timestamp
	131, // 42: dtkt.core.v1.Service.create_time:type_name -> google.protobuf.Timestamp
	131, // 43: dtkt.core.v1.Service.update_time:type_name -> google.protobuf.Timestamp
	131, // 44: dtkt.core.v1.Method.create_time:type_name -> google.protobuf.Timestamp
	131, // 45: dtkt.core.v1.Method.update_time:type_name -> google.protobuf.Timestamp
	19,  // 46: dtkt.core.v1.Type.schema:type_name -> dtkt.core.v1.TypeSchema
	131, // 47: dtkt.core.v1.Type.create_time:type_name -> google.protobuf.Timestamp
	131, // 48: dtkt.core.v1.Type.update_time:type_name -> google.protobuf.Timestamp
	135, // 49: dtkt.core.v1.TypeSchema.v1beta1:type_name -> dtkt.shared.v1beta1.TypeSchema
	131, // 50: dtkt.core.v1.DeploymentMetadata.start_time:type_name -> google.protobuf.Timestamp
	131, // 51: dtkt.core.v1.DeploymentMetadata.stop_time:type_name -> google.protobuf.Timestamp
	8,   // 52: dtkt.core.v1.FlowConnectionMetadata.context:type_name -> dtkt.core.v1.Context
	136, // 53: dtkt.core.v1.FlowRuntimeMetadata.v1beta1:type_name -> dtkt.flow.v1beta1.Runtime
	137, // 54: dtkt.core.v1.FlowGroupsMetadata.v1beta1:type_name -> dtkt.flow.v1beta1.Groups
	138, // 55: dtkt.core.v1.FlowSpecMetadata.v1beta1:type_name -> dtkt.flow.v1beta1.Flow
	139, // 56: dtkt.core.v1.FlowGraphMetadata.v1beta1:type_name -> dtkt.flow.v1beta1.Graph
	140, // 57: dtkt.core.v1.PackageSpecMetadata.v1beta1:type_name -> dtkt.shared.v1beta1.Package
	120, // 58: dtkt.core.v1.PackageBuildMetadata.runtime:type_name -> dtkt.core.v1.PackageBuildMetadata.Runtime
	121, // 59: dtkt.core.v1.PackageBuildMetadata.platform:type_name -> dtkt.core.v1.PackageBuildMetadata.Platform
	122, // 60: dtkt.core.v1.PackageBuildMetadata.env:type_name -> dtkt.core.v1.PackageBuildMetadata.EnvEntry
	2,   // 61: dtkt.core.v1.BuildOperationMetadata.state:type_name -> dtkt.core.v1.BuildOperationMetadata.State
	131, // 62: dtkt.core.v1.BuildOperationMetadata.start_time:type_name -> google.protobuf.Timestamp
	131, // 63: dtkt.core.v1.BuildOperationMetadata.finish_time:type_name -> google.protobuf.Timestamp
	3,   // 64: dtkt.core.v1.RunOperationMetadata.state:type_name -> dtkt.core.v1.RunOperationMetadata.State
	131, // 65: dtkt.core.v1.RunOperationMetadata.start_time:type_name -> google.protobuf.Timestamp
	131, // 66: dtkt.core.v1.RunOperationMetadata.stop_time:type_name -> google.protobuf.Timestamp
	4,   // 67: dtkt.core.v1.SyncOperationMetadata.state:type_name -> dtkt.core.v1.SyncOperationMetadata.State
	131, // 68: dtkt.core.v1.SyncOperationMetadata.start_time:type_name -> google.protobuf.Timestamp
	131, // 69: dtkt.core.v1.SyncOperationMetadata.stop_time:type_name -> google.protobuf.Timestamp
	5,   // 70: dtkt.core.v1.DialMetadata.state:type_name -> dtkt.core.v1.DialMetadata.State
	131, // 71: dtkt.core.v1.DialMetadata.time:type_name -> google.protobuf.Timestamp
	123, // 72: dtkt.core.v1.BatchRunOperationMetadata.failed_requests:type_name -> dtkt.core.v1.BatchRunOperationMetadata.FailedRequestsEntry
	9,   // 73: dtkt.core.v1.EncryptRequest.resource:type_name -> dtkt.core.v1.Resource
	9,   // 74: dtkt.core.v1.EncryptResponse.resource:type_name -> dtkt.core.v1.Resource
	9,   // 75: dtkt.core.v1.DecryptRequest.resource:type_name -> dtkt.core.v1.Resource
	9,   // 76: dtkt.core.v1.DecryptResponse.resource:type_name -> dtkt.core.v1.Resource
	10,  // 77: dtkt.core.v1.GetAutomationResponse.automation:type_name -> dtkt.core.v1.Automation
	10,  // 78: dtkt.core.v1.CreateAutomationRequest.automation:type_name -> dtkt.core.v1.Automation
	10,  // 79: dtkt.core.v1.CreateAutomationResponse.automation:type_name -> dtkt.core.v1.Automation
	40,  // 80: dtkt.core.v1.BatchCreateAutomationsRequest.requests:type_name -> dtkt.core.v1.CreateAutomationRequest
	10,  // 81: dtkt.core.v1.BatchCreateAutomationsResponse.automations:type_name -> dtkt.core.v1.Automation
	10,  // 82: dtkt.core.v1.ListAutomationsResponse.automations:type_name -> dtkt.core.v1.Automation
	124, // 83: dtkt.core.v1.SendAutomationEvent.inputs_event:type_name -> dtkt.core.v1.SendAutomationEvent.InputsEvent
	125, // 84: dtkt.core.v1.SendAutomationEvent.user_response_event:type_name -> dtkt.core.v1.SendAutomationEvent.UserResponseEvent
	128, // 85: dtkt.core.v1.ReceiveAutomationEvent.outputs_event:type_name -> dtkt.core.v1.ReceiveAutomationEvent.OutputsEvent
	129, // 86: dtkt.core.v1.ReceiveAutomationEvent.user_request_event:type_name -> dtkt.core.v1.ReceiveAutomationEvent.UserRequestEvent
	46,  // 87: dtkt.core.v1.SendAutomationEventRequest.event:type_name -> dtkt.core.v1.SendAutomationEvent
	47,  // 88: dtkt.core.v1.ReceiveAutomationEventsResponse.event:type_name -> dtkt.core.v1.ReceiveAutomationEvent
	46,  // 89: dtkt.core.v1.StreamAutomationEventsRequest.event:type_name -> dtkt.core.v1.SendAutomationEvent
	47,  // 90: dtkt.core.v1.StreamAutomationEventsResponse.event:type_name -> dtkt.core.v1.ReceiveAutomationEvent
	12,  // 91: dtkt.core.v1.ListConnectionsResponse.connections:type_name -> dtkt.core.v1.Connection
	12,  // 92: dtkt.core.v1.GetConnectionResponse.connection:type_name -> dtkt.core.v1.Connection
	12,  // 93: dtkt.core.v1.CreateConnectionRequest.connection:type_name -> dtkt.core.v1.Connection
	12,  // 94: dtkt.core.v1.CreateConnectionResponse.connection:type_name -> dtkt.core.v1.Connection
	12,  // 95: dtkt.core.v1.UpdateConnectionRequest.connection:type_name -> dtkt.core.v1.Connection
	141, // 96: dtkt.core.v1.UpdateConnectionRequest.update_mask:type_name -> google.protobuf.FieldMask
	12,  // 97: dtkt.core.v1.UpdateConnectionResponse.connection:type_name -> dtkt.core.v1.Connection
	31,  // 98: dtkt.core.v1.DialConnectionResponse.dial:type_name -> dtkt.core.v1.DialMetadata
	15,  // 99: dtkt.core.v1.SyncDescriptorsResponse.file:type_name -> dtkt.core.v1.File
	15,  // 100: dtkt.core.v1.GetDescriptorsResponse.file:type_name -> dtkt.core.v1.File
	13,  // 101: dtkt.core.v1.ListDeploymentsResponse.deployments:type_name -> dtkt.core.v1.Deployment
	13,  // 102: dtkt.core.v1.GetDeploymentResponse.deployment:type_name -> dtkt.core.v1.Deployment
	13,  // 103: dtkt.core.v1.CreateDeploymentRequest.deployment:type_name -> dtkt.core.v1.Deployment
	13,  // 104: dtkt.core.v1.UpdateDeploymentRequest.deployment:type_name -> dtkt.core.v1.Deployment
	141, // 105: dtkt.core.v1.UpdateDeploymentRequest.update_mask:type_name -> google.protobuf.FieldMask
	1,   // 106: dtkt.core.v1.UpdateDeploymentRequest.desired_state:type_name -> dtkt.core.v1.Deployment.State
	11,  // 107: dtkt.core.v1.ListFlowsResponse.flows:type_name -> dtkt.core.v1.Flow
	11,  // 108: dtkt.core.v1.GetFlowResponse.flow:type_name -> dtkt.core.v1.Flow
	11,  // 109: dtkt.core.v1.CreateFlowRequest.flow:type_name -> dtkt.core.v1.Flow
	11,  // 110: dtkt.core.v1.CreateFlowResponse.flow:type_name -> dtkt.core.v1.Flow
	11,  // 111: dtkt.core.v1.UpdateFlowRequest.flow:type_name -> dtkt.core.v1.Flow
	141, // 112: dtkt.core.v1.UpdateFlowRequest.update_mask:type_name -> google.protobuf.FieldMask
	11,  // 113: dtkt.core.v1.UpdateFlowResponse.flow:type_name -> dtkt.core.v1.Flow
	14,  // 114: dtkt.core.v1.ListIntegrationsResponse.integrations:type_name -> dtkt.core.v1.Integration
	14,  // 115: dtkt.core.v1.GetIntegrationResponse.integration:type_name -> dtkt.core.v1.Integration
	14,  // 116: dtkt.core.v1.CreateIntegrationRequest.integration:type_name -> dtkt.core.v1.Integration
	14,  // 117: dtkt.core.v1.CreateIntegrationResponse.integration:type_name -> dtkt.core.v1.Integration
	27,  // 118: dtkt.core.v1.BuildIntegrationRequest.build:type_name -> dtkt.core.v1.PackageBuildMetadata
	14,  // 119: dtkt.core.v1.BuildIntegrationResponse.integration:type_name -> dtkt.core.v1.Integration
	27,  // 120: dtkt.core.v1.BuildIntegrationResponse.build:type_name -> dtkt.core.v1.PackageBuildMetadata
	14,  // 121: dtkt.core.v1.UpdateIntegrationRequest.integration:type_name -> dtkt.core.v1.Integration
	141, // 122: dtkt.core.v1.UpdateIntegrationRequest.update_mask:type_name -> google.protobuf.FieldMask
	14,  // 123: dtkt.core.v1.UpdateIntegrationResponse.integration:type_name -> dtkt.core.v1.Integration
	18,  // 124: dtkt.core.v1.GetTypeResponse.type:type_name -> dtkt.core.v1.Type
	18,  // 125: dtkt.core.v1.ListTypesResponse.types:type_name -> dtkt.core.v1.Type
	17,  // 126: dtkt.core.v1.GetMethodResponse.method:type_name -> dtkt.core.v1.Method
	16,  // 127: dtkt.core.v1.GetServiceResponse.service:type_name -> dtkt.core.v1.Service
	16,  // 128: dtkt.core.v1.ListServicesResponse.services:type_name -> dtkt.core.v1.Service
	17,  // 129: dtkt.core.v1.ListMethodsResponse.methods:type_name -> dtkt.core.v1.Method
	21,  // 130: dtkt.core.v1.Automation.ConnectionsEntry.value:type_name -> dtkt.core.v1.FlowConnectionMetadata
	132, // 131: dtkt.core.v1.Automation.InputsEntry.value:type_name -> google.protobuf.Any
	27,  // 132: dtkt.core.v1.Deployment.Build.metadata:type_name -> dtkt.core.v1.PackageBuildMetadata
	8,   // 133: dtkt.core.v1.Deployment.Build.context:type_name -> dtkt.core.v1.Context
	8,   // 134: dtkt.core.v1.Deployment.Cloud.context:type_name -> dtkt.core.v1.Context
	33,  // 135: dtkt.core.v1.Deployment.Cloud.config:type_name -> dtkt.core.v1.EncryptedAny
	118, // 136: dtkt.core.v1.Deployment.RuntimeMetadata.native:type_name -> dtkt.core.v1.Deployment.RuntimeMetadata.Native
	119, // 137: dtkt.core.v1.Deployment.RuntimeMetadata.docker:type_name -> dtkt.core.v1.Deployment.RuntimeMetadata.Docker
	142, // 138: dtkt.core.v1.PackageBuildMetadata.Runtime.v1beta1:type_name -> dtkt.shared.v1beta1.Runtime
	143, // 139: dtkt.core.v1.PackageBuildMetadata.Platform.v1beta1:type_name -> dtkt.shared.v1beta1.Platform
	144, // 140: dtkt.core.v1.BatchRunOperationMetadata.FailedRequestsEntry.value:type_name -> google.rpc.Status
	126, // 141: dtkt.core.v1.SendAutomationEvent.InputsEvent.values:type_name -> dtkt.core.v1.SendAutomationEvent.InputsEvent.ValuesEntry
	127, // 142: dtkt.core.v1.SendAutomationEvent.UserResponseEvent.values:type_name -> dtkt.core.v1.SendAutomationEvent.UserResponseEvent.ValuesEntry
	132, // 143: dtkt.core.v1.SendAutomationEvent.InputsEvent.ValuesEntry.value:type_name -> google.protobuf.Any
	132, // 144: dtkt.core.v1.SendAutomationEvent.UserResponseEvent.ValuesEntry.value:type_name -> google.protobuf.Any
	130, // 145: dtkt.core.v1.ReceiveAutomationEvent.OutputsEvent.values:type_name -> dtkt.core.v1.ReceiveAutomationEvent.OutputsEvent.ValuesEntry
	145, // 146: dtkt.core.v1.ReceiveAutomationEvent.UserRequestEvent.user_action:type_name -> dtkt.flow.v1beta1.UserAction
	132, // 147: dtkt.core.v1.ReceiveAutomationEvent.OutputsEvent.ValuesEntry.value:type_name -> google.protobuf.Any
	148, // [148:148] is the sub-list for method output_type
	148, // [148:148] is the sub-list for method input_type
	148, // [148:148] is the sub-list for extension type_name
	148, // [148:148] is the sub-list for extension extendee
	0,   // [0:148] is the sub-list for field type_name
}

func init() { file_dtkt_core_v1_messages_proto_init() }
func file_dtkt_core_v1_messages_proto_init() {
	if File_dtkt_core_v1_messages_proto != nil {
		return
	}
	file_dtkt_core_v1_messages_proto_msgTypes[1].OneofWrappers = []any{}
	file_dtkt_core_v1_messages_proto_msgTypes[3].OneofWrappers = []any{
		(*Resource_Encrypted)(nil),
		(*Resource_Decrypted)(nil),
	}
	file_dtkt_core_v1_messages_proto_msgTypes[4].OneofWrappers = []any{}
	file_dtkt_core_v1_messages_proto_msgTypes[6].OneofWrappers = []any{
		(*Connection_Deployment)(nil),
		(*Connection_CustomProtos)(nil),
		(*Connection_CustomGrpc)(nil),
		(*Connection_DecryptedConfig)(nil),
		(*Connection_EncryptedConfig)(nil),
	}
	file_dtkt_core_v1_messages_proto_msgTypes[8].OneofWrappers = []any{}
	file_dtkt_core_v1_messages_proto_msgTypes[9].OneofWrappers = []any{
		(*File_Protos)(nil),
	}
	file_dtkt_core_v1_messages_proto_msgTypes[13].OneofWrappers = []any{
		(*TypeSchema_V1Beta1)(nil),
	}
	file_dtkt_core_v1_messages_proto_msgTypes[16].OneofWrappers = []any{
		(*FlowRuntimeMetadata_V1Beta1)(nil),
	}
	file_dtkt_core_v1_messages_proto_msgTypes[17].OneofWrappers = []any{
		(*FlowGroupsMetadata_V1Beta1)(nil),
	}
	file_dtkt_core_v1_messages_proto_msgTypes[18].OneofWrappers = []any{
		(*FlowSpecMetadata_V1Beta1)(nil),
	}
	file_dtkt_core_v1_messages_proto_msgTypes[19].OneofWrappers = []any{
		(*FlowGraphMetadata_V1Beta1)(nil),
	}
	file_dtkt_core_v1_messages_proto_msgTypes[20].OneofWrappers = []any{
		(*PackageSpecMetadata_V1Beta1)(nil),
	}
	file_dtkt_core_v1_messages_proto_msgTypes[38].OneofWrappers = []any{}
	file_dtkt_core_v1_messages_proto_msgTypes[40].OneofWrappers = []any{
		(*SendAutomationEvent_InputsEvent_)(nil),
		(*SendAutomationEvent_UserResponseEvent_)(nil),
	}
	file_dtkt_core_v1_messages_proto_msgTypes[41].OneofWrappers = []any{
		(*ReceiveAutomationEvent_OutputsEvent_)(nil),
		(*ReceiveAutomationEvent_UserRequestEvent_)(nil),
	}
	file_dtkt_core_v1_messages_proto_msgTypes[47].OneofWrappers = []any{}
	file_dtkt_core_v1_messages_proto_msgTypes[57].OneofWrappers = []any{
		(*DialConnectionRequest_Name)(nil),
		(*DialConnectionRequest_Address)(nil),
	}
	file_dtkt_core_v1_messages_proto_msgTypes[63].OneofWrappers = []any{}
	file_dtkt_core_v1_messages_proto_msgTypes[68].OneofWrappers = []any{}
	file_dtkt_core_v1_messages_proto_msgTypes[70].OneofWrappers = []any{}
	file_dtkt_core_v1_messages_proto_msgTypes[80].OneofWrappers = []any{}
	file_dtkt_core_v1_messages_proto_msgTypes[94].OneofWrappers = []any{}
	file_dtkt_core_v1_messages_proto_msgTypes[100].OneofWrappers = []any{}
	file_dtkt_core_v1_messages_proto_msgTypes[102].OneofWrappers = []any{}
	file_dtkt_core_v1_messages_proto_msgTypes[111].OneofWrappers = []any{
		(*Deployment_RuntimeMetadata_Native_)(nil),
		(*Deployment_RuntimeMetadata_Docker_)(nil),
	}
	file_dtkt_core_v1_messages_proto_msgTypes[114].OneofWrappers = []any{
		(*PackageBuildMetadata_Runtime_V1Beta1)(nil),
	}
	file_dtkt_core_v1_messages_proto_msgTypes[115].OneofWrappers = []any{
		(*PackageBuildMetadata_Platform_V1Beta1)(nil),
	}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: unsafe.Slice(unsafe.StringData(file_dtkt_core_v1_messages_proto_rawDesc), len(file_dtkt_core_v1_messages_proto_rawDesc)),
			NumEnums:      6,
			NumMessages:   125,
			NumExtensions: 0,
			NumServices:   0,
		},
		GoTypes:           file_dtkt_core_v1_messages_proto_goTypes,
		DependencyIndexes: file_dtkt_core_v1_messages_proto_depIdxs,
		EnumInfos:         file_dtkt_core_v1_messages_proto_enumTypes,
		MessageInfos:      file_dtkt_core_v1_messages_proto_msgTypes,
	}.Build()
	File_dtkt_core_v1_messages_proto = out.File
	file_dtkt_core_v1_messages_proto_goTypes = nil
	file_dtkt_core_v1_messages_proto_depIdxs = nil
}
